...
def _train_lstm(model_path, corpus_path, out_path):
    ...
    MODEL、CORPUS 完全沿用 perplexity-lstm 的 version 2 八键读取、形状、F、
    严格 UTF-8、词表及语料至少 2 码点契约。置 h=h0、c=c0，t 升序以当前字符
    的 V 长 one-hot 为 x，调用装入 W、b 的 LSTMCell.forward(x, h, c)，取
    返回的 h、c 推进状态并缓存每步 cache；标签为下一字符。

    输出层沿用 _train 的公式、下标与累加顺序（隐状态由 n 换为 h）：自
    float 偏置起依 j 升序计算 logit，减最大值的稳定 softmax 得 p，令
    g = p - onehot(y)，按 t 升序累加 dWhy += g⊗h、dby += g，并按 k 升序
    计算 dhs = Whyᵀg。随后以 backward_sequence(dhs, caches) 取得
    dxs、dh0、dc0、dW、db（dxs、dh0、dc0 丢弃）。

    依 dW、db、dWhy、dby 的组序按行序求全局范数，超过 5.0 即统一缩放至
    5.0；四组参数减 0.1 倍梯度，h0、c0 不变。任一中间量或结果非有限均抛
    ValueError。

    OUT 复用 perplexity-lstm 模型的八个键、键序与形状，version 恰为 int 2，
    数组元素均转为 float；文件内容恰为 json.dumps(obj, ensure_ascii=True,
    ...
    # t 升序，以 (h0, c0) 为初态逐步前向，缓存每步返回的 h 与 cache。
    hs = []
    caches = []
    h = list(h0)
    c = list(c0)
    ...
    for t in range(T):
        ...
        # logit：与 _train 相同，自 float 偏置起依 j 升序累加（隐状态为 h）。
        z = [0.0] * V
        for k in range(V):
            acc = float(by[k])
            why_row = Why[k]
...
def main(argv):
    ...
    python seqmodel.py train-lstm MODEL CORPUS OUT：MODEL、CORPUS 沿用
    perplexity-lstm 的 version 2 八键契约。t 升序以 (h0,c0) 为初态用当前
    字符 one-hot 调用装入 W、b 的 LSTMCell.forward 并缓存；输出层 p、g、
    dWhy、dby、dhs 沿用 train 的公式与累加顺序，隐状态换为 h；以
    backward_sequence(dhs,caches) 取得 dW、db。梯度组序 dW、db、dWhy、dby
    求全局范数并按 5.0 裁剪，0.1 更新规则同 train，仅更新 W、b、Why、by，
    h0、c0 不变；写出 version 2 的八键新模型，成功时 stdout 为空并返回 0。
