...
class BidirectionalGRU(object):
    ...
    def forward_self_attn_masked(self, xs, lengths, mask):
        """变长批掩码自注意力前向，返回 (c, w) 并保存 selfattnmasked 缓存。

        xs、lengths 完全沿用 forward_padded 的契约（xs 为非空 B×T×I 的 F
        嵌套列表，lengths 各项为 1<=值<=T 的非 bool int）；mask 须为
        B×T×T 的嵌套 list 且每个元素 type 恰为 bool（True 参与、False
        屏蔽），否则抛 ValueError。任何失败都使既有缓存（含 forward、
        forward_padded、forward_attn_padded、forward_self_attn_padded
        缓存）失效；本前向使用独立的缓存标签，不与其他前向共用缓存。

        先按 forward_padded 的同一前向语义求批双向输出 y（有效前缀为真实
        输出、padding 行为 2H 个 0.0）。mask 的 padding 行、列（下标 >=
        L_b=lengths[b]）同样逐元素校验为 bool 后忽略；样本 b 的有效
        L_b×L_b 块逐行复制，且每行至少一个 True，否则抛 ValueError。对
        样本 b 取 v = y[b][:L_b]（逐行新建的 float 行），调用
        attention(v, v, v, 有效块) 得 (c_b, w_b)（形状分别为 L_b×2H、
        L_b×L_b）。c 为 B×T×2H，c[b] 有效前缀逐行取自 c_b，padding 行是
        2H 个 0.0；w 为 B×T×T，w[b] 的 L_b×L_b 有效块取自 w_b，padding
        行、列均为 0.0。缓存保存批 GRU 缓存及 v、有效 mask 块的快照，足以
        支撑 backward_self_attn_masked。结果均为逐层新建的 float 列表，不
        修改或复用输入与单元参数；任一中间量非有限同样抛 ValueError，实参
        数量错误沿用 Python 自带的 TypeError。
        """
        I, H = self.I, self.H
        ...
        # y 复用 forward_padded 的全部校验与前向计算；成功后其缓存即当前
        # 缓存（标签 "padded"），随后改写为带 v、mask 快照的 selfattnmasked
        # 缓存。其内部失败已自行清空缓存；其后任一步失败也须清掉它留下的
        # padded 缓存。
        y = self.forward_padded(xs, lengths)
        try:
            _, B, T, lengths, cfs, cbs = self._cache

            # mask：B×T×T 的嵌套 list，元素 type 恰为 bool。padding 行列在
            # 此一并校验（type 仍须为 bool），随后忽略；仅逐行复制有效
            # L×L 块入缓存。
                    ...
                    for m in row:
                        if type(m) is not bool:
                            raise ValueError(
                                ...
                                % (m,))
                L = lengths[b]
                block = [[mb[i][j] for j in range(L)] for i in range(L)]
                # 有效块每行至少一个 True（全屏蔽行在调用 attention 前判定）。
                for i in range(L):
                    if not any(block[i]):
                        raise ValueError(
                            "mask row %d is entirely False" % i)
                masks[b] = block
        ...
        self._cache = ("selfattnmasked", B, T, list(lengths), cfs, cbs,
                       vs, masks)
        return c, w
    ...
    def backward_self_attn_masked(self, dc):
        ...
        对样本 b 令 L=lengths[b]，以缓存的有效前缀 v（长 L）与有效 mask
        块对 dc 的有效前缀调用
        attention_backward(v, v, v, dc[:L], 有效块) 得 (gq, gk, gv)（形状
        均为 L×2H）：有效前缀输出梯度 dy 按 t、维升序自 0.0 起依次累加
        ...
        结果均为逐层新建的 float 列表，不修改 dc、缓存或单元参数，重复调用
        结果确定；任一中间量非有限抛 ValueError。
        """
        I, H = self.I, self.H
        cache = self._cache
        if type(cache) is not tuple or len(cache) != 8 \
                or cache[0] != "selfattnmasked":
            raise ValueError(
                "backward_self_attn_masked requires a successful "
                "forward_self_attn_masked pass before it")
        _, B, T, lengths, cfs, cbs, vs, masks = cache
        ...
        for b in range(B):
            ...
            dc_prefix = [dc[b][t] for t in range(L)]
            gq, gk, gv = attention_backward(
                vs[b], vs[b], vs[b], dc_prefix, masks[b])
            dy_b = [[0.0] * (2 * H) for _ in range(L)]
            for t in range(L):
                row = dy_b[t]
                gqt = gq[t]
