...
def _load_rank_inputs(models_path, list_path):
    """读取并校验 rank 系列命令共用的 MODELS/LIST，返回四项解包结果。

    返回 (model_paths, entries, loaded, entry_ids)：
      * model_paths：MODELS 原路径 str 列表（重复项按序保留）；
      * entries：LIST 原 (corpus, window) str 二元组列表（重复项按序保留）；
      * loaded：与 model_paths 同序的 (cell, Why, by, h0, c0) 五元组列表，
        cell 为已装入该模型 W、b 的 LSTMCell；
      * entry_ids：与 entries 同序的语料 id 序列列表。

    MODELS 为严格 UTF-8 的 JSON 数组且至少 2 项，每项须为非空相对路径 str
    （绝对路径非法），路径相对 MODELS 文件的父目录解析；重复项按序保留。
    LIST 为严格 UTF-8 的 JSON 非空数组，每项恰为 [corpus, window] 两个 str：
    corpus 为非空相对路径并按 LIST 文件的父目录解析，window 整串匹配
    [1-9][0-9]*（任意位数均合法，不转 int）；重复项按序保留。各模型的读取
    沿用 _load_perplexity_lstm_model 的 version 2 八键、F、形状与严格 UTF-8
    契约；所有模型 vocab 须逐项同序相等，H 可不同。每个语料至少 2 码点且每
    个码点须在公共 vocab 内。任何读取失败抛 OSError；其余 UTF-8/JSON/校验失
    败抛 ValueError。不跨调用缓存任何状态，亦不写文件。
    """
...
def _total_rank_nll(cell, Why, by, h0, c0, ids, window_text):
    """单个模型在单条语料 id 序列上的总负对数似然。

    每次调用均以 h0、c0、memory=[h0] 独立重置：h=list(h0)、c=list(c0)，
    T=len(ids)-1，t 从 0 升序。第 t 步以 ids[t] 的 V 长 one-hot 调用装入
    W、b 的 LSTMCell.forward(x, h, c) 推进 h、c；M 取 memory 末尾至多
    WINDOW 项（顺序从旧到新），经 _attn_context 求上下文 u（注意力的 d/k
    下标与累加顺序不变）；z 由 _output_logits 依 j 升序求得，随后按 k 升序
    做稳定 log-sum-exp：m=max(z)，d 自 0.0 累加 exp(z[k]-m)，
    nll=m+log(d)-z[ids[t+1]]；total 自 0.0 依 t 升序累加 nll，再向 memory
    追加 h 的 float 副本。window_text 整串匹配 [1-9][0-9]*；ids 须为非空的
    int 列表且相邻索引落在 [0,V) 内。任何非法输入或非有限中间结果均抛
    ValueError。不跨调用缓存状态。
    """
    if type(window_text) is not str or not _WINDOW_RE.match(window_text):
        raise ValueError("WINDOW must match [1-9][0-9]*")
    ...
    for ix in ids:
        if type(ix) is not int or ix < 0 or ix >= V:
            raise ValueError("ids entries must be ints within the vocab")

    # 每次均从自身 h0、c0、memory=[h0] 独立重置；h0 保留模型原值（F 允许
    # int），attention 按原值“先乘后加”，attention 不修改其输入。
    memory = [h0]
    h = list(h0)
    c = list(c0)
    total = 0.0
...
def _rank_suite(models_path, list_path):
    ...
    for gi, (_corpus_text, window_text) in enumerate(entries):
        ...
        for mi, (cell, Why, by, h0_m, c0_m) in enumerate(loaded):
            # 每项均从自身 h0、c0、memory=[h0] 独立重置（见 _total_rank_nll）。
...
def _rank_window_sensitivity(models_path, list_path, base_text):
    ...
    for mi, (cell, Why, by, h0_m, c0_m) in enumerate(loaded):
        for gi, (corpus_text, window_text) in enumerate(entries):
            ...
            # 两轨均从自身 h0、c0、memory=[h0] 独立重置（见 _total_rank_nll）。
            L = _total_rank_nll(cell, Why, by, h0_m, c0_m, ids, window_text)
