...
def _sample_transformer_topp(model_path, start, seed_text, start_t_text,
                             end_t_text, top_p_text, length_text):
    """以线性退火温度与 top-p 截断从单层 Transformer 块语言模型采样。

    除 TOP_P 及下述选样外，MODEL、START、SEED、LENGTH 的校验，以及
    prefix 的 one-hot 输入、因果掩码、TransformerBlock.forward、Why/by
    仿射、稳定 softmax、线性温度退火（LENGTH 为 1 时仅用 START_T，为 0
    时不计算温度）、唯一随机源与 prefix 状态更新均沿用
    _sample_transformer_anneal。TOP_P 经 float() 解析，结果须有限且
    0<TOP_P<=1，否则抛 ValueError。整次调用仅初始化一次
    r=random.Random(int(SEED))，不写任何文件。

    每步先按原顺序求 e_k=exp(a_k-max(a))，d 自 0.0 依 k 升序累加。再将
    索引按 (-e_k, k) 升序排列（e 降序、并列时 k 升序），依该序自 0.0
    累加 e，截取首个使累计值 >=TOP_P*d 的最短前缀；s 为该前缀按该序自
    0.0 累加所得之和。令 u=r.random()*s，再按前缀顺序自 0.0 累加 e，
    选首个累计值严格大于 u 的索引；无则取前缀末项。其字符追加到输出，
    索引追加到 prefix。任一新增运算非有限均抛 ValueError。成功返回
    LENGTH 个码点再加一个 LF。
    """
    (vocab, heads, P, Wq, Wk, Wv, Wo, W1, b1, W2, b2, Why, by) = \
        _load_perplexity_transformer_model(model_path)
...
def main(argv):
    ...
    END_T TOP_P LENGTH：除 TOP_P 与选样外，全部参数校验、LENGTH=0/1 语义、
    Transformer 前向、Why/by 仿射、线性温度、唯一随机源、状态更新、有限性
    与错误协议均沿用 sample-transformer-anneal。TOP_P 经 float() 解析，须
    有限且 0<TOP_P<=1。每步沿原次序求 a、m 与 e[k]=exp(a[k]-m)，d 从 0.0
    按 k 升序累加；索引按 (-e[k],k) 升序，从 0.0 依序累加 e，保留首个使
    累计值 >=TOP_P*d 的最短前缀，s 为其累计和。令 u=random()*s，按前缀序
    从 0.0 累加 e，选首个累计值严格大于 u 的索引，无则取末项；字符与索引
    追加到输出和 prefix。新增运算非有限即失败。输出契约与 sample 相同，
    不写文件。
    ...
    try:
        if len(argv) == 4 and argv[1] == "perplexity":
            ...
            output = _sample_transformer_topp(
                argv[2], argv[3], argv[4], argv[5], argv[6], argv[7],
                argv[8])
            sys.stdout.buffer.write(output.encode("utf-8"))
        else:
