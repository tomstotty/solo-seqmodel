...
def _sample_gru_attn_topk_topp(model_path, start, seed_text, start_t_text,
    ...
    除 TOP_P 及核选样外，MODEL、START、SEED、START_T、END_T、LENGTH、
    WINDOW 的校验、LENGTH 的 0/1 语义、GRU 状态、注意力记忆、线性温度、
    Why/by logit、稳定 softmax、随机源生命周期与有限性失败契约均沿用
    _sample_gru_attn_topk；WINDOW 整串匹配 [1-9][0-9]*（任意位数均合法，
    不转 int），安全截取沿用 perplexity-gru-attn，否则抛 ValueError。
    整次调用仅构造一次 r=random.Random(int(SEED))（LENGTH 为 0、1 或选样
    回退时亦然），不写任何文件。
    ...
    引；无则取前缀末项。其字符追加到输出并作为下一输入 x，随后向 memory
    追加 h 的 float 副本。任一新增运算非有限均抛 ValueError。成功返回
    LENGTH 个码点再加一个 LF。
    """
    if not _WINDOW_RE.match(window_text):
        raise ValueError("WINDOW must match [1-9][0-9]*")
...
def main(argv):
    ...
    python seqmodel.py sample-gru-attn-top-k-top-p MODEL START SEED START_T
    END_T TOP_K TOP_P LENGTH WINDOW：除 TOP_P 及核选样外，参数校验、LENGTH
    的 0/1 语义、GRU 状态、注意力记忆、线性温度、Why/by logit、稳定
    softmax、随机源生命周期与有限性失败契约均沿用 sample-gru-attn-top-k
    （整次仅构造一次 random.Random(int(SEED))）。TOP_K 整串匹配
    [1-9][0-9]* 且数学值不超过 V=len(vocab)：先按十进制位数、同长字典
    序与 V 比较，越界即失败，仅通过后才转 int。TOP_P 经 float() 解析，
    须有限且 0<TOP_P<=1，否则失败。每步先按原顺序算 e_k=exp(a_k-
    max(a))，d 自 0.0 依 k 升序累加，再将索引按 (-e_k,k) 升序取前 K 项，
    sK 从 0.0 依序累加 e，令 target=TOP_P*sK，再从 0.0 累加 e，保留首
    个使累计值 >=target 的最短前缀，s 为其累计和；每步令 u=random()*s，
    再按前缀序自 0.0 累加 e，选首个累计值严格大于 u 的索引，无则取前
    缀末项，其字符作为下一输入；输出契约与 sample 相同，不写文件。
