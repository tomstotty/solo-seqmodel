...
class TransformerBlock(object):
        ...
        try:
            if type(cache) is not tuple or len(cache) != 8 \
                ...
                raise ValueError(
                    ...
                    "forward_relative pass before it")
            (_, T, D, P, a, f, W1, W2) = cache
            # 缓存的 a、f、W1、W2 须分别为 T×D、T×P、P×D、D×P 的 F 矩阵；
            # 缓存遭篡改或状态非法时与其他失败一样清空缓存。
            a = _check_matrix(a, T, D, "cached a")
            f = _check_matrix(f, T, P, "cached f")
            W1 = _check_matrix(W1, P, D, "cached W1")
            W2 = _check_matrix(W2, D, P, "cached W2")
            dy = _check_matrix(dy, T, D, "dy")
            dy = [[float(z) for z in row] for row in dy]
    ...
    def forward_cross_relative(self, qx, kvx, qpos, kpos, bias, mask=None):
        """相对位置偏置 Transformer 块交叉注意力前向，返回 (y, w) 并缓存快照。
        ...
        Tq×D、w 为 heads×Tq×Tk 的逐层新建 float 列表并缓存，不修改或复用
        输入、bias 与参数。任一中间量非有限抛 ValueError；任何失败都清空
        本块缓存。实参数量错误沿用 Python 自带的 TypeError。
        """
        D, P = self.D, self.P
        # 任何失败的 forward_cross_relative 都使既有缓存失效。
        self._cache = None
    ...
    def backward_cross_relative(self, dy):
        ...
        否则抛 ValueError；dy 须为与前向 y 同形的 Tq×D F 矩阵，否则抛
        ValueError；缓存的 a、f、W1、W2 须分别为 Tq×D、Tq×P、P×D、D×P 的
        F 矩阵，缓存遭篡改或状态非法时同样清空缓存并抛 ValueError。按
        backward_cross 相同的公式与次序（求和均自 0.0 按下标升序累加）求
        dW2、db2、df、dz、dW1、db1 与 da，再调用
        ...
        重复调用结果相同。任一中间量非有限、输入/属性/缓存/状态非法或反向
        失败均抛 ValueError 并清空缓存。实参数量错误沿用 Python 自带的
        TypeError。
        """
        cache = self._cache
        try:
            if type(cache) is not tuple or len(cache) != 8 \
                ...
                raise ValueError(
                    ...
                    "forward_cross_relative pass before it")
            (_, Tq, D, P, a, f, W1, W2) = cache
            # 缓存的 a、f、W1、W2 须分别为 Tq×D、Tq×P、P×D、D×P 的 F 矩阵；
            # 缓存遭篡改或状态非法时与其他失败一样清空缓存。
            a = _check_matrix(a, Tq, D, "cached a")
            f = _check_matrix(f, Tq, P, "cached f")
            W1 = _check_matrix(W1, P, D, "cached W1")
            W2 = _check_matrix(W2, D, P, "cached W2")
            dy = _check_matrix(dy, Tq, D, "dy")
            dy = [[float(z) for z in row] for row in dy]
            ...
            dqa, dkvx, dWq, dWk, dWv, dWo, dbias = \
                self.attn.backward_cross_relative(da)
