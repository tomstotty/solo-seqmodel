...
def _sample_lstm_attn(model_path, start, seed_text, temperature_text,
    ...
    MODEL、START、SEED、TEMPERATURE、LENGTH 的读取、词法、形状、F 与确定性
    抽样契约完全沿用 _sample_lstm；WINDOW 整串匹配 [1-9][0-9]*（任意位数均
    合法，不转 int），安全截取规则沿用 _perplexity_lstm_attn，否则抛
    ValueError。整次调用仅初始化一次 r=random.Random(int(SEED))，不写任何
    文件。

    置 h=h0、c=c0、x=START 索引、memory=[h0]。循环 LENGTH 次：以 x 的 V 长
    one-hot 调用装入 W、b 的 LSTMCell.forward(x, h, c)，取前两项更新 h、c；
    M 取 memory 末尾至多 WINDOW 项（顺序从旧到新），以
    attention([h], M, M, None) 返回首项 ctx，按 i 升序令
    u[i] = h[i] + ctx[0][i]。logit 仅以 u 替代 _sample_lstm 中的 h，温度
    缩放、稳定 softmax、随机阈值与按词表升序累计抽样的公式及运算顺序均与
    _sample_lstm 相同；追加 vocab[k]、令 x=k，再向 memory 追加 h 的 float
    副本。任一中间量非有限均抛 ValueError。成功返回 LENGTH 个码点再加一个
    LF。
    """
    if not _WINDOW_RE.match(window_text):
        raise ValueError("WINDOW must match [1-9][0-9]*")
    ...
    rng = random.Random(seed)
    # 与 _perplexity_lstm_attn 相同：h0 保留模型原值，保证首步“原值先乘后加”。
    memory = [h0]
    h = list(h0)
    c = list(c0)
    x = vocab.index(start)
    ...
    for _t in range(length):
        ...
        # threshold=r.random()*d；自 0.0 依 k 升序累加 e_k，选首个累计值
        # 严格大于 threshold 者；无则取词表末项。
        threshold = rng.random() * d
        if not math.isfinite(threshold):
            raise ValueError("sample threshold became non-finite")
        chosen = V - 1
...
def main(argv):
    ...
    python seqmodel.py sample-lstm-attn MODEL START SEED TEMPERATURE LENGTH
    WINDOW：MODEL、START、SEED、TEMPERATURE、LENGTH 沿用 sample-lstm 的
    version 2 八键、F、形状、词法与确定性抽样契约；WINDOW 沿用
    perplexity-lstm-attn 的词法及任意位数安全截取。置 h=h0、c=c0、
    x=START 索引、memory=[h0]，每步以 x 的 one-hot 调用装入 W、b 的
    LSTMCell.forward 更新 h、c，再以 memory 末尾至多 WINDOW 项（从旧到新）
    为键/值调用 attention，用 h 与首行上下文之和作为 logit 隐状态，随后令
    x 为选中索引并向 memory 追加 h 的 float 副本；输出契约与 sample 相同，
    不写文件。
