...
def _train_gru_attn_tbptt(model_path, corpus_path, out_path, window_text,
                          k_text):
    """带注意力上下文对 GRU 模型做一次截断 BPTT 的全语料 SGD 更新并写入 OUT。

    MODEL、CORPUS、OUT、0.1 更新、5.0 裁剪及成败协议沿用 train-gru；WINDOW
    的词法与任意位数安全截取沿用 perplexity-gru-attn。K 整串匹配
    [1-9][0-9]*（任意位数均合法，不转 int），否则抛 ValueError；T 为预测
    步数，按十进制位数与同长字典序求 Ke=min(K,T)，不把超长 K 先转 int。

    前向完全沿用 perplexity-gru-attn 并保存各步 h、M、u 与 GRU cache。令
    g=p-onehot(y)，按 train-gru 次序以 u 累加 dWhy、dby 并求 du=Whyᵀg。
    dhs 置零，t 降序调用 attention_context_backward(h_t, M_t, du_t)：dn 加
    至 dhs[t]；dmemory 第 p 行令 q=t+1-len(M_t)+p，q=0 丢弃，否则 s=q-1，
    仅当 (T-1-s)//Ke==(T-1-t)//Ke 才按 p、i 升序加至 dhs[s]。随后调用
    GRUCell.backward_sequence(dhs, caches, None, Ke) 取 dW、db；梯度组序
    dW、db、dWhy、dby，h0 不变，OUT 键序/形状/紧凑 JSON 加 LF 及负零口径
    均沿用 train-gru。
    """
    if not _WINDOW_RE.match(window_text):
        raise ValueError("WINDOW must match [1-9][0-9]*")
    ...
    # Ke = min(K, T)：位数多者数值大；同位数则字典序与数值序一致（两者均
    # 无前导零）。仅当 K < T 时才需要实际整数，此时 K 的位数不超过
    # str(T) 的位数，int() 不会触发整数文本位数限制。
    t_text = str(T)
    if len(k_text) > len(t_text) or (
            len(k_text) == len(t_text) and k_text >= t_text):
        ke = T
    else:
        ke = int(k_text)
    ...
    # 前向完全沿用 perplexity-gru-attn：t 升序以 h0 为初态逐步推进 h，
    # M_t 为 memory 末尾至多 WINDOW 项，u_t = h + 首行上下文，随后向
    # memory 追加 h 的 float 副本；另保存各步 h、M、u 与 cache。
    hs = []
    caches = []
    m_list = []
    u_list = []
    ...
    # 注意力残差与记忆的反向：dhs 置零，按 t 降序累加；跨 TBPTT 窗口的
    # 记忆梯度丢弃（窗口自末端对齐，步 x 属第 (T-1-x)//Ke 窗）。
    dhs = [[0.0] * H for _ in range(T)]
    for t in range(T - 1, -1, -1):
        dn, dmemory = attention_context_backward(hs[t], m_list[t], du_list[t])
        ...
        # 再按 M_t 从旧到新、i 升序：q 为该行在完整记忆
        # [h0, h_0, ..., h_{t-1}] 中的下标；q=0 即 h0（梯度丢弃），
        # 否则 s=q-1，仅与 t 同 TBPTT 窗口时加至 dhs[s]。
        base = t + 1 - len(m_list[t])
        for p, dm_row in enumerate(dmemory):
            q = base + p
            if q == 0:
...
def main(argv):
    ...
    WINDOW 沿用 perplexity-gru-attn；K 整串匹配 [1-9][0-9]*，T 为预测
    步数，按位数与同长字典序求 Ke=min(K,T)，不先转换超长 K。前向完全
    沿用 perplexity-gru-attn 并保存各步 h、M、u 与 GRU cache；反向令
    g=p-onehot(y)，按 train-gru 次序以 u 累加 dWhy、dby 并求 du=Whyᵀg，
    dhs 置零后按 t 降序调用 attention_context_backward：dn 加至 dhs[t]，
    dmemory 第 p 行 q=t+1-len(M_t)+p，q=0 丢弃，否则 s=q-1，仅当
    (T-1-s)//Ke==(T-1-t)//Ke 才加至 dhs[s]；再以
    GRUCell.backward_sequence(dhs, caches, None, Ke) 取 dW、db，梯度组序
