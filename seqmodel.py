...
def _sample_gru(model_path, start, seed_text, temperature_text, length_text):
    ...
    MODEL 严格沿用 perplexity-gru 的 version 3 七键顺序、F、形状及
    UTF-8 契约；START、SEED、TEMPERATURE、LENGTH 的词法与校验完全沿用
    _sample。整次调用仅初始化一次 r=random.Random(int(SEED))，不写任何
    文件。

    置 h=list(h0)、x=START 索引。循环 LENGTH 次：以 x 的 V 长 one-hot
    调用装入 W、b 的 GRUCell.forward(x, h)，取首项更新 h；按
    perplexity-gru 的 float 偏置与 j 升序累加计算 logit
    z_k = by_k + Σ_j Why_k,j*h_j；温度缩放、稳定 softmax、随机阈值、
    k 升序累计与选中规则逐项沿用 _sample；追加 vocab[k] 并令 x=k。任一
    中间量非有限均抛 ValueError。成功返回 LENGTH 个码点再加一个 LF。
    """
    ...
    for _t in range(length):
        # 当前字符的 V 长 one-hot 输入，推进 GRU 隐状态，取返回首项。
        xvec = [0.0] * V
        xvec[x] = 1.0
...
def main(argv):
    ...
    MODEL 为 version 3 的 GRU 模型（沿用 perplexity-gru 的七键顺序、F、
    形状及 UTF-8 契约，W 形状 3H×(V+H)、b 形状 3H），置 r=
    random.Random(int(SEED))、h=list(h0)、x=START 索引，每步以 x 的 V 长
    one-hot 调用装入 W、b 的 GRUCell.forward(x, h) 取首项更新 h，logit、
    温度缩放、稳定 softmax 与抽样规则同 sample；输出契约与 sample 相同，
    不写任何文件。
    ...
    try:
        if len(argv) == 4 and argv[1] == "perplexity":
            ...
            raise ValueError(
                ...
                "seqmodel.py sample-gru MODEL START SEED TEMPERATURE LENGTH | "
