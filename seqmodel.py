...
def _beam_lstm_attn_topk_topp(model_path, start, start_t_text, end_t_text,
                              ...
                              length_text, window_text):
    """以线性退火温度、top-k 截断后再 top-p 截取、带注意力上下文的束搜索。

    除 TOP_K、TOP_P 及下述每束候选截取外，MODEL、START、LENGTH、WINDOW、
    LSTM 状态、注意力记忆、Why/by logit、稳定 softmax、线性温度退火
    （LENGTH 为 1 时仅用 START_T，为 0 时不计算温度）与有限性失败契约均
    沿用 _beam_lstm_attn；WINDOW 词法与安全截取沿用
    perplexity-lstm-attn。本命令无 SEED、无随机源且不写任何文件。

    TOP_K 整串匹配 [1-9][0-9]*，且数学值 K 不超过 V=len(vocab)：先按十进
    制位数及同长度字典序与 V 的十进制文本比较，越界即失败，仅通过后才转
    int，任意位数文本都不触发整数转换异常。TOP_P 经 float() 解析，结果须
    有限且 0<TOP_P<=1，否则抛 ValueError。BEAM 整串匹配 [1-9][0-9]*
    （任意位数均合法），每轮仅当其数学值小于候选数时转 int，否则保留全部
    候选。

    初始束为 (0.0, "", h0, c0, START 索引, [h0])，生成索引元组为空。第 t
    轮逐束按既有温度推进 h、c 并求缩放 logit a、m=max(a)、
    e[k]=exp(a[k]-m)；将索引按 (-e[k], k) 升序排列（e 降序、并列时 k
    升序）并取前 K 项，sK 自 0.0 依该序累加，令 target=TOP_P*sK，再从
    0.0 依该序累加 e，取首个使累计值 >=target 的最短前缀，s 为其累计
    和。仅为前缀中每个 k 生成子束：分数加 a[k]-m-log(s)，文本追加
    vocab[k]，置 x=k，memory 追加 h 的 float 副本。全部子束按 (-分数,
    生成索引元组) 升序排列，保留前 min(BEAM, 候选数) 项；同轮等长，使用
    原始累计分数，长度归一化因子固定为 1。任一新增乘法、累加、log 或分
    数非有限均抛 ValueError。LENGTH 为 0 时仅输出 LF，否则输出最终首束
    文本加一个 LF。相同输入输出逐字节相同。
    """
    if not _WINDOW_RE.match(window_text):
        raise ValueError("WINDOW must match [1-9][0-9]*")
    ...
    for t in range(length):
        ...
        for bi in range(len(beams)):
            ...
            # 再从 0.0 按候选序累加 e，保留首个累计值 >=target 的最短前
            # 缀；s 为此前缀累计和。因累加顺序相同，末项累计恰为
            # sK>=target（TOP_P<=1），前缀必存在且非空。
            prefix = []
            s = 0.0
            for idx in candidates:
                prefix.append(idx)
            ...
            if not math.isfinite(ls):
                raise ValueError("log prefix mass became non-finite")
...
def main(argv):
    ...
    python seqmodel.py beam-lstm-attn-top-k-top-p MODEL START START_T END_T
    TOP_K TOP_P BEAM LENGTH WINDOW：以线性退火温度、top-k 截断后再 top-p
    截取、带注意力上下文做确定性束搜索。除 TOP_K、TOP_P 及每束候选截取
    外，参数校验、LENGTH 的 0/1 语义、线性温度、LSTM 状态、注意力记忆、
    Why/by logit、稳定 softmax 与有限性失败契约均沿用 beam-lstm-attn；本
    命令无 SEED、无随机源且不写文件。TOP_K 整串匹配 [1-9][0-9]* 且数学
    值不超过 V（先按十进制位数及同长度字典序比较，通过后才转 int）；
    TOP_P 经 float() 解析且有限、0<TOP_P<=1；BEAM 整串匹配
    [1-9][0-9]*，每轮仅当其数学值小于候选数时转 int，否则保留全部候选。
    初始束同 beam-lstm-attn；每轮逐束求 a、m=max(a)、e_k=exp(a_k-m)，索
    引按 (-e_k,k) 升序取前 K 项，sK 自 0.0 依序累加，target=TOP_P*sK，
    再取累计值首次 >=target 的最短前缀，s 为其累计和；仅为前缀中每个 k
    生成子束，分数加 a[k]-m-log(s)，文本、x、h、c、memory 更新同
    beam-lstm-attn；全部子束按 (-分数, 生成索引元组) 升序，保留前
    min(BEAM, 候选数) 项。LENGTH 为 0 时仅输出 LF，否则输出最终首束文
    本加 LF。
    ...
    try:
        if len(argv) == 4 and argv[1] == "perplexity":
            ...
            sys.stdout.buffer.write(output.encode("utf-8"))
        elif len(argv) == 11 and argv[1] == "beam-lstm-attn-top-k-top-p":
            output = _beam_lstm_attn_topk_topp(
