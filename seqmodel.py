...
def _sample_lstm_attn_topk_topp_run(model_path, start, seed_text,
    ...
    校验、状态推进、温度退火、top-k 截断后再 top-p 截取、随机源初始化与
    消费、选索引规则及有限性失败契约均与 _sample_lstm_attn_topk_topp 文档
    一致。返回 (chars, logprobs, total)：chars 为生成码点列表；logprobs
    为每步选中索引 k 对应的 a[k]-m-log(s)（s 为 top-p 前缀质量）；total
    自 0.0 按 t 升序累加各 lp。任一 lp 或 total 非有限均抛 ValueError。
    ...
    for t in range(length):
        ...
        # 选中索引 k 后，以既有 a、m 与最终前缀质量 s 计算选中项对数概率，
        # total 自 0.0 按 t 升序累加；任一结果非有限即失败。
        lp = a[chosen] - m - math.log(s)
        if not math.isfinite(lp):
            raise ValueError("selected log-prob became non-finite")
...
def _sample_lstm_attn_topk_topp_scored(model_path, start, seed_text,
                                       start_t_text, end_t_text, top_k_text,
                                       top_p_text, length_text, window_text):
    """sample-lstm-attn-top-k-top-p-scored：输出文本、逐步对数概率与累计。

    全部校验、状态推进、温度退火、top-k 后 top-p 截取、随机源初始化与消
    费、选索引规则及错误协议均沿用 sample-lstm-attn-top-k-top-p；同参须
    消费相同随机序列并生成与原入口一致的 text。每步选中索引 k 后，以既
    有 a、m 和最终前缀质量 s 计算 lp=a[k]-m-log(s)；total 从 0.0 按 t 升
    序累加 lp，任一结果非有限即失败。stdout 恰为单个 JSON 对象加 LF，键
    序 text,logprobs,total_logprob；text 为生成字符串（不含尾随 LF），
    logprobs 为 LENGTH 长字符串列表、第 t 项为 format(lp,'.17g')，
...
def main(argv):
    ...
    python seqmodel.py sample-lstm-attn-top-k-top-p-scored MODEL START SEED
    START_T END_T TOP_K TOP_P LENGTH WINDOW：全部校验、状态推进、温度退
    火、top-k 后 top-p 截取、随机消费及错误协议均沿用
    sample-lstm-attn-top-k-top-p；同参须消费相同随机序列并生成与原入口
    一致的 text。每步选中 k 后，以已有 a、m、最终前缀质量 s 算
    lp=a[k]-m-log(s)，total 从 0.0 按 t 升序累加 lp，任一结果非有限即失
    败。stdout 恰为单个 JSON 对象加 LF，键序
    text,logprobs,total_logprob；text 为生成字符串（不含 LF），logprobs
    为 LENGTH 长字符串列表、第 t 项为 format(lp,'.17g')，total_logprob
    为 format(total,'.17g')；LENGTH 为 0 时三值依次为 ""、[]、"0"。序
    列化恰用 json.dumps(obj,ensure_ascii=True,separators=(',',':'),
