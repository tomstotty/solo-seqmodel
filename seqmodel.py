...
def _sample_lstm_attn_anneal(model_path, start, seed_text, start_t_text,
    ...
    """以线性退火温度带注意力上下文从 LSTM 语言模型采样 LENGTH 个码点。

    MODEL、START、SEED、LENGTH、WINDOW 的词法与校验、LSTM 状态推进、
    memory 记忆与窗口截取、attention 上下文、Why/by logit、稳定 softmax、
    随机阈值与词表升序抽样规则均逐项沿用 _sample_lstm_attn；仅以退火温度
    替换原固定温度，退火规则同 _sample_lstm_anneal。START_T、END_T 各经
    float() 解析，结果须有限且严格大于 0，否则抛 ValueError。整次调用仅
    初始化一次 r=random.Random(int(SEED))，不写任何文件。

    LENGTH 为 0 时不计算温度，仅输出 LF；为 1 时仅用 START_T；否则 t 自 0
    升序，第 t 步温度严格按 Python 表达式
    START_T+(END_T-START_T)*t/(LENGTH-1) 求值，结果非有限或不大于 0 即抛
    ValueError。每步沿用同一 h、c、x 与 memory，生成后令 x=k 并向 memory
    追加 h 的 float 副本。任一中间量非有限均抛 ValueError。成功返回 LENGTH
    个码点再加一个 LF。
    """
    if not _WINDOW_RE.match(window_text):
        raise ValueError("WINDOW must match [1-9][0-9]*")
...
def main(argv):
    ...
    END_T LENGTH WINDOW：除温度外，MODEL、START、SEED、LENGTH、WINDOW、
    LSTM 状态、注意力记忆、Why/by logit、稳定 softmax 与词表升序阈值抽样
    均沿用 sample-lstm-attn；START_T、END_T 经 float() 解析且有限、严格
    大于 0。整次仅初始化一次随机源。LENGTH 为 0 时不求温度且仅输出 LF，
    为 1 时仅用 START_T，否则第 t 步温度严格按
    START_T+(END_T-START_T)*t/(LENGTH-1) 求值，每步须有限且大于 0。每步
    以该温度替换固定温度，沿用同一 h、c、x 与 memory，生成后更新 x 并追加
    h 的 float 副本；输出契约与 sample 相同，不写文件。
    ...
    try:
        if len(argv) == 4 and argv[1] == "perplexity":
            ...
            output = _sample_lstm_attn_anneal(
