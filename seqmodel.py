...
def _sample_lstm_mha_topp_run(model_path, start, seed_text, start_t_text,
    ...
    校验、LSTM/MHA 状态推进、温度退火、top-p 前缀截取、随机源初始化与消
    费、选索引规则及有限性失败契约均与 _sample_lstm_mha_topp 文档一致。
    返回 (chars, logprobs, total)：chars 为生成码点列表；logprobs 为每步
    选中索引 k 对应的 a[k]-m-log(s)（s 为 top-p 前缀质量）；total 自 0.0
    按 t 升序累加各 lp。任一 lp 或 total 非有限均抛 ValueError。
...
def _sample_lstm_mha_topp(model_path, start, seed_text, start_t_text,
                          end_t_text, top_p_text, length_text, window_text):
    """sample-lstm-mha-top-p：成功返回 LENGTH 个码点再加一个 LF。
...
def _sample_lstm_mha_topp_scored(model_path, start, seed_text, start_t_text,
    ...
    全部校验、LSTM/MHA 状态推进、温度退火、top-p 前缀截取、随机源初始化
    与消费、选索引规则及错误协议均沿用 sample-lstm-mha-top-p；同参须消费
    相同随机序列并生成与原入口一致的 text。每步选中索引 k 后，以既有 a、
    m 和前缀质量 s 计算 lp=a[k]-m-log(s)；total 从 0.0 按 t 升序累加 lp，
    任一结果非有限即失败。stdout 恰为单个 JSON 对象加 LF，键序
    text,logprobs,total_logprob；text 为生成字符串（不含尾随 LF），
    logprobs 为 LENGTH 长字符串列表、第 t 项为 format(lp,'.17g')，
    total_logprob 为 format(total,'.17g')。LENGTH 为 0 时三者分别为
    ""、[]、"0"。序列化恰用
    json.dumps(obj,ensure_ascii=True,separators=(',',':'),
    allow_nan=False)+'\\n' 的 UTF-8 字节。不写文件。
    """
