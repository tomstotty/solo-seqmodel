...
def _load_rank_inputs(models_path, list_path):
    """读取并校验 rank-window-* 共用的 MODELS、LIST 及模型与语料。

    返回四元组 (model_paths, entries, loaded, entry_ids)：
    model_paths 为 MODELS 原文 str 列表（至少 2 项的非空相对路径，重复项
    按序保留）；entries 为 LIST 原文 (corpus, window) 二元组列表（非空，
    corpus 为非空相对路径、window 整串匹配 [1-9][0-9]*，重复项按序保
    留）；loaded 按 MODELS 原序为 (cell, Why, by, h0, c0)，cell 为装入
    该模型 W、b 的 LSTMCell，所有模型 vocab 须逐项同序相等（H 可不
    同）；entry_ids 按 LIST 原序为每个语料的整数 id 列表（语料至少 2 码
    点且每个码点在 vocab）。MODELS、LIST 均为严格 UTF-8 的 JSON；模型
    相对 MODELS 父目录、语料相对 LIST 父目录解析。任一读取、UTF-8、
    JSON 或校验失败均抛 ValueError（或 OSError），不写文件、不缓存任何
    跨调用状态。
    """
    with open(models_path, "rb") as f:
        model_paths = json.loads(f.read().decode("utf-8"))
...
def _total_rank_nll(cell, Why, by, h0, c0, ids, window_text):
    """rank-window-* 逐项总 NLL：从 h0、c0、memory=[h0] 重置后按 t 累加。

    每次调用均以该模型自身 h0、c0 独立重置，memory 自 [h0] 起；按 t 升序
    推进单步 LSTM、窗口截取（window_text 为已通过词法校验的
    [1-9][0-9]*，不转 int）、注意力与输出 logit，稳定 softmax 的 t/j/k
    下标与浮点累加次序完全沿用 perplexity-lstm-attn，总量自 0.0 按 t 升
    序累加，任一步或总量非有限均抛 ValueError。不保留跨调用状态。
    """
    V = len(by)

    def _step_nll(h, c, memory, t):
        """perplexity-lstm-attn 单步：推进状态并返回 (nll, h, c, memory)。"""
        y = ids[t + 1]
    ...
    for t in range(len(ids) - 1):
        nll, h, c, memory = _step_nll(h, c, memory, t)
        total += nll
