...
def _lstm_cell_loaded(V, H, W, b):
    ...
    装载 version 4 模型参数的入口因此构造一个与采样无关的随机源；此处以
    object.__new__ 跳过该随机初始化，仅设置 forward/backward 所需的 I、H、
    W、b，令这类入口不构造任何随机源（采样入口对 random.Random 的唯一一次
    构造恰为 random.Random(int(SEED))）。W 逐行复制为新列表、b 复制为新
    列表，行为与“LSTMCell(V,H) 后覆盖 W、b”逐值一致。
    """
...
def _mha_loaded(H, heads, Wq, Wk, Wv, Wo):
    """不经随机初始化构造一个已装入四组投影的 MHA。

    MHA.__init__ 本身不使用随机源，但为使“装载 version 4 模型参数不得构造
    任何随机源”的装载路径单一且不依赖各构造器的实现细节，此处与
    _lstm_cell_loaded 同样以 object.__new__ 跳过 __init__，仅设置
    forward_cross/backward_cross 所需的 D、heads、四组投影与缓存槽。四组
    投影逐行复制为新列表，行为与“MHA(H,heads) 后覆盖 Wq/Wk/Wv/Wo”逐值
    一致。
    """
...
def _perplexity_lstm_mha(model_path, corpus_path, window_text):
    ...
    # 以 model 参数直接装入 cell/MHA，跳过构造器中随即被覆盖的随机 W 初始化。
...
def _sample_lstm_mha_anneal(model_path, start, seed_text, start_t_text,
                            end_t_text, length_text, window_text):
    """以线性退火温度从 version 4 的 LSTM-MHA 模型采样 LENGTH 个码点。

    MODEL、START、SEED、LENGTH、WINDOW 的校验，以及 LSTM 状态、MHA 窗口、
    Why/by logit、稳定 softmax、词表升序阈值抽样的次序均完全沿用
    _sample_lstm_mha；模型 LSTM、MHA 参数经 _lstm_cell_loaded/_mha_loaded
    直接装入，不构造任何随机源，整次调用对 random.Random 的唯一一次构造
    恰为 r=random.Random(int(SEED))，不写任何文件。

    START_T、END_T 各经 float() 解析，结果须有限且严格大于 0。LENGTH 为 0
    时不计算温度，仅输出一个 LF；为 1 时仅用 START_T；否则 t 自 0 升序，
    第 t 步温度严格按 Python 表达式
    START_T+(END_T-START_T)*t/(LENGTH-1) 求值，结果非有限或不大于 0 即抛
    ValueError，并以该温度替换 _sample_lstm_mha 的固定温度。置 h=h0、
    c=c0、x=START 索引、memory=[h0]，h、c、x、memory 与随机源均跨步连续，
    生成后更新 x 并向 memory 追加 h 的 float 副本。任一中间量非有限均抛
    ValueError。成功返回 LENGTH 个码点再加一个 LF。
    """
    if not _WINDOW_RE.match(window_text):
        raise ValueError("WINDOW must match [1-9][0-9]*")
    ...
    # 整次调用唯一的随机源。
    rng = random.Random(seed)
    # 与 _sample_lstm_mha 相同：h0 保留模型原值，forward_cross 会先复制
    # 再投影；各步 h 本就是 float。
    memory = [h0]
    h = list(h0)
    c = list(c0)
...
def main(argv):
    ...
    END_T LENGTH WINDOW：MODEL、START、SEED、LENGTH、WINDOW 的校验，以及
    LSTM 状态、MHA 窗口、Why/by、softmax 与抽样次序完全沿用
    sample-lstm-mha；模型 LSTM、MHA 参数直接装入，不构造任何随机源，整次
    仅构造一次 random.Random(int(SEED))。START_T、END_T 各经 float()
    解析且须有限、>0。LENGTH 为 0 不求温度且仅输出 LF，为 1 仅用
    START_T；否则第 t 步严格以
    START_T+(END_T-START_T)*t/(LENGTH-1) 求温度，结果须有限、>0，并替换
    sample-lstm-mha 的固定温度；h、c、x、memory 与随机源跨步连续。成功
    返回 0、stderr 空，stdout 恰为 LENGTH 个 UTF-8 码点加 LF，不写文件；
    参数数量、词法、I/O、UTF-8/JSON、模型或非有限计算错误均返回 2、
    stdout 空、stderr 恰为 error\n 且无回溯。
