...
def _sample_lstm_mha_topp(model_path, start, seed_text, start_t_text,
                          end_t_text, top_p_text, length_text, window_text):
    """以线性退火温度、top-p 核采样、带投影多头交叉注意力从 version 4
    的 LSTM-MHA 模型采样。

    除 TOP_P 与选样外，MODEL、START、SEED、START_T、END_T、LENGTH、WINDOW
    的校验、LSTM 状态、MHA 窗口、Why/by logit、稳定 softmax、线性温度退火
    （LENGTH 为 1 时仅用 START_T，为 0 时不计算温度）、唯一随机源、模型
    参数直接装入不经随机初始化及有限性失败契约均完全沿用
    sample-lstm-mha-anneal，不写任何文件。

    TOP_P 经 float() 解析，结果须有限且 0<TOP_P<=1，否则抛 ValueError。
    每步先按原顺序求 a[k]、m=max(a)、e[k]=exp(a[k]-m)，d 自 0.0 按 k 升
    序累加；索引按 (-e[k], k) 升序排列（e 降序、并列时 k 升序），依该序
    自 0.0 累加 e，保留首个使累计值 >=TOP_P*d 的最短前缀，s 为该前缀按
    该序自 0.0 累加所得之和。令 u=random()*s，再按前缀顺序自 0.0 累加 e，
    选首个累计值严格大于 u 的索引；无则取前缀末项。随后追加 vocab[k]、更新
    x、向 memory 追加 h 的 float 副本。任一新增运算非有限均抛 ValueError。
    ...
    for t in range(length):
        ...
        # 依该序自 0.0 累加 e，保留首个累计值 >=TOP_P*d 的最短前缀；s 为
...
def main(argv):
    ...
    END_T TOP_P LENGTH WINDOW：以线性退火温度、top-p 核采样、带投影多头
    交叉注意力从 version 4 的 LSTM-MHA 模型采样。除 TOP_P 与选样外，
    MODEL、START、SEED、START_T、END_T、LENGTH、WINDOW 的校验、LSTM/MHA
    状态推进、Why/by logit、稳定 softmax、线性温度（LENGTH 为 0 时不求温度
    且仅输出 LF，为 1 时仅用 START_T）、唯一随机源及有限性契约均沿用
    sample-lstm-mha-anneal；模型参数直接装入、不经随机初始化，整次仅构造
    一次 random.Random(int(SEED))，不写文件。TOP_P 经 float() 解析，须
    有限且 0<TOP_P<=1。每步沿原顺序求 a[k]、m=max(a)、e[k]=exp(a[k]-m)，
    d 自 0.0 按 k 升序累加；索引按 (-e[k],k) 升序，依序累加 e 并保留首个
    使累计值 >=TOP_P*d 的最短前缀，s 为其和；令 u=random()*s，按前缀序
    累加 e，选首个累计值严格大于 u 的索引，无则取末项，再更新 x 并追加 h
    副本。成功返回 LENGTH 个 UTF-8 码点加 LF。
