...
def _sample_lstm_attn_topk_topp(model_path, start, seed_text, start_t_text,
                                ...
                                length_text, window_text):
    """以线性退火温度、top-k 截断后再 top-p（核）抽样、带注意力上下文采样。

    除 TOP_P 及下述选样外，参数校验、LENGTH 的 0/1 语义、线性温度、LSTM
    状态、注意力记忆、Why/by logit、稳定 softmax、有限性失败契约均沿用
    _sample_lstm_attn_topk；WINDOW 词法与安全截取沿用
    perplexity-lstm-attn。TOP_K 整串匹配 [1-9][0-9]* 且数学值 K 不超过
    V=len(vocab)：先按十进制位数及同长度字典序与 V 比较，越界即失败，仅
    通过后转 int，任意位数文本不得触发整数转换异常。TOP_P 经 float() 解
    析，结果须有限且 0<TOP_P<=1，否则抛 ValueError。整次调用仅初始化一次
    r=random.Random(int(SEED))，不写任何文件。

    每步先按原顺序求温度缩放后的 e_k=exp(a_k-max(a))，d 自 0.0 依 k 升
    序累加。再将索引按 (-e_k, k) 升序排列（e 降序、并列时 k 升序），候
    选恰为该序前 K 项；sK 自 0.0 按候选序累加 e，令 target=TOP_P*sK，
    再自 0.0 按候选序累加 e，保留首个使累计值 >=target 的最短前缀，s 为
    该前缀自 0.0 依前缀序累加所得之和。令 u=r.random()*s，再按前缀顺序
    自 0.0 累加 e，选首个累计值严格大于 u 的索引，无则取前缀末项。其字
    符追加到输出并作为下一输入 x，随后向 memory 追加 h 的 float 副本。
    任一新增乘法或累加非有限均抛 ValueError。成功返回 LENGTH 个码点再加
    一个 LF。
    """
    if not _WINDOW_RE.match(window_text):
        raise ValueError("WINDOW must match [1-9][0-9]*")
    ...
    for t in range(length):
        ...
        # 再自 0.0 按候选序累加 e，截取首个累计值 >=target 的最短前缀；s
        # 为其自 0.0 依候选序累加所得之和。与 top-p 同理，全量累计恰为
        # sK>=target，数学上必能达到；未达到时以全部候选为前缀。
        prefix = []
        s = 0.0
        reached = False
        for idx in candidates:
...
def main(argv):
    ...
    python seqmodel.py sample-lstm-attn-top-k-top-p MODEL START SEED START_T
    END_T TOP_K TOP_P LENGTH WINDOW：除 TOP_P 及下述选样外，参数校验、
    LENGTH 的 0/1 语义、线性温度、LSTM 状态、注意力记忆、Why/by logit、
    稳定 softmax 与有限性失败契约均沿用 sample-lstm-attn-top-k；TOP_K
    词法与按位数/字典序的越界检查亦同。TOP_P 经 float() 解析，须有限且
    0<TOP_P<=1。每步先按原顺序算 e_k=exp(a_k-max(a))，索引按 (-e_k,k)
    升序并取前 K 项为候选；sK 自 0.0 按候选序累加 e，令
    target=TOP_P*sK，再自 0.0 按候选序累加 e，保留首个使累计值
    >=target 的最短前缀，s 为此前缀累计和；整次仅初始化一次
    random.Random(int(SEED))，每步令 u=random()*s，再按前缀顺序自 0.0
    累加 e，选首个累计值严格大于 u 的索引，无则取前缀末项，其字符作为
    下一输入；任一新增乘法或累加非有限即失败；输出契约与 sample 相同，
    不写文件。
    ...
    try:
        if len(argv) == 4 and argv[1] == "perplexity":
            ...
            output = _sample_lstm_attn_topk_topp(
