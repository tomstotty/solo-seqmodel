...
def _load_rank_inputs(models_path, list_path):
    """读取并校验 rank-window-* 三命令共用的 MODELS、LIST、模型与语料。

    MODELS 为严格 UTF-8 的 JSON 数组，至少 2 项；每项须为非空相对路径
    str（绝对路径非法，重复项按序保留），按 MODELS 文件的父目录解析。
    LIST 为严格 UTF-8 的 JSON 非空数组，每项恰为两个 str 组成的
    [corpus, window]：corpus 须为非空相对路径（重复项按序保留），按 LIST
    文件的父目录解析；window 须整串匹配 [1-9][0-9]*（任意位数均合法，
    不转 int）。各模型沿用 compare-lstm-attn 的 version 2 八键、F、形状
    与严格 UTF-8 契约，所有模型 vocab 须逐项同序相等，H 可不同。每个语
    料须为严格 UTF-8、至少 2 码点且每个码点在词表中。

    返回 (model_paths, entries, loaded, entry_ids)：model_paths 为 MODELS
    原文 str 列表；entries 为 LIST 原文 (corpus, window) 元组列表；
    loaded 与 model_paths 同序，每项为 (cell, Why, by, h0, c0)，cell 为
    装入该模型 W、b 副本的 LSTMCell；entry_ids 与 entries 同序，每项为
    同序整数 id 列表。MODELS/LIST/模型/语料缺失或不可读抛 OSError；其余
    解析、形状、词表或非有限错误抛 ValueError。
    """
    with open(models_path, "rb") as f:
        model_paths = json.loads(f.read().decode("utf-8"))
    if not isinstance(model_paths, list) or len(model_paths) < 2:
    ...
    # 每个清单项的语料只读一次并转成同序 id 序列；计算结果与重复读取逐位
    # 相同，重复清单按序保留各自的同一 id 序列副本。
    entry_ids = []
    for corpus_text, _window_text in entries:
        corpus_path = os.path.join(base_dir, corpus_text)
        with open(corpus_path, "rb") as f:
...
def _total_rank_nll(cell, Why, by, h0, c0, ids, window_text):
    """单模型在单个语料 id 序列上按 t 升序累加的总负对数似然。

    每次均以该模型自身的 h0、c0、memory=[h0] 独立重置（h、c 取 h0、c0
    的新列表副本，memory 仅含 h0 引用），t 升序推进：x 为 ids[t] 的 V
    长 one-hot，经 LSTMCell.forward 更新 h、c，M 取 memory 末尾至多
    WINDOW 项（从旧到新），u 为注意力上下文，z=Why·u+by 依 j 升序累
    加，稳定 softmax 的 m=max(z)、d 依 k 升序累加，
    nll=m+log(d)-z[ids[t+1]]，total 自 0.0 按 t 升序累加，随后向
    memory 追加 h 的 float 副本。LSTM、attention 窗口及 t/j/k 累加顺序
    与 perplexity-lstm-attn 完全一致；任一计算非有限即抛 ValueError。
    """
    V = len(by)
    memory = [h0]
    h = list(h0)
...
def _rank_window_stability(models_path, list_path, bases_text):
    ...
    保序完全沿用 rank-window-sensitivity（MODELS 为至少 2 个模型，vocab
    同序，H 可不同）。BASES 为严格 UTF-8 的 JSON 非空数组，每项 type 恰
    为 str 且整串匹配 [1-9][0-9]*（任意位数均合法，全程不转 int），重复
    项按序保留，否则失败。
