...
def _sample_lstm_mha_relative_topk(model_path, start, seed_text,
    ...
    u=random()*s，再按候选序自 0.0 累加 e，选首个累计值严格大于 u 的索
    引；无则取候选末项。其字符追加到输出并作为下一输入 x，随后向 memory
    追加 h 的 float 副本、向 pos 追加 t+1。任一新增运算非有限均抛
    ValueError。LENGTH 为 0 时不计算温度；为 1 时仅用 START_T。成功返回
    LENGTH 个码点再加一个 LF。
    """
    if not _WINDOW_RE.match(window_text):
        raise ValueError("WINDOW must match [1-9][0-9]*")
    ...
    rng = random.Random(seed)
    # 与 sample-lstm-mha-relative-anneal 相同：h0 保留模型原值，
    # forward_cross_relative 会先复制再投影；各步 h 本就是 float。
    memory = [h0]
    pos = [0]
    h = list(h0)
...
def main(argv):
    ...
    START_T END_T TOP_K LENGTH WINDOW：以线性退火温度与 top-k 抽样、带相
    对位置偏置多头交叉注意力从 version 5 的模型采样。除 TOP_K 与选样外，
    模型与参数校验、LENGTH 为 0/1 的处理、线性温度退火、LSTM/相对位置
    MHA 状态推进、Why/by logit、稳定 softmax、唯一随机源与有限性失败契
    约均沿用 sample-lstm-mha-relative-anneal；TOP_K 整串匹配
    [1-9][0-9]* 且数学值 K<=V，先按十进制位数及同长字典序与 V 比较，越
    界失败，通过后才转 int。每步求 a[k]、m=max(a)、e[k]=exp(a[k]-m)，
    索引按 (-e[k],k) 升序，候选取前 K 项，s 自 0.0 按候选序累加；
    u=random()*s，再从 0.0 依序累加 e，选首个累计值严格大于 u 的索引，
    无则取末项，更新 x 并追加 h 副本与 t+1 到 pos。Unicode 按 UTF-8 输
    出，不写文件。
