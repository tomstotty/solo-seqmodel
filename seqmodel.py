...
def _sample_lstm_attn_topp_scored(model_path, start, seed_text, start_t_text,
                                  ...
                                  window_text):
    """行为同 _sample_lstm_attn_topp，但输出带每步对数概率的 JSON 对象。

    全部校验、状态推进、温度退火、top-p 前缀、随机选中及错误协议均沿用
    _sample_lstm_attn_topp；同参须消费相同随机序列并生成与原入口一致的
    text。每步选中索引 k 后，以既有 a、m 和前缀质量 s 计算
    lp=a[k]-m-log(s)；total 从 0.0 按 t 升序累加 lp，任一结果非有限即抛
    ValueError。成功时返回单个 JSON 对象加 LF：键序恰为
    text,logprobs,total_logprob；text 为生成字符串，logprobs 为 LENGTH
    长字符串列表，第 t 项为 format(lp,'.17g')，total_logprob 为
    format(total,'.17g')。序列化恰用
    json.dumps(obj,ensure_ascii=True,separators=(',',':'),
    allow_nan=False)+'\\n'，不写任何文件。
    """
    if not _WINDOW_RE.match(window_text):
    ...
    for t in range(length):
        ...
        # 选中索引的前缀内对数概率 lp=a[k]-m-log(s)，total 按 t 升序累加。
...
def main(argv):
    ...
    top-p 前缀、随机选中及错误协议均沿用 sample-lstm-attn-top-p；同参
    须消费相同随机序列并生成与原入口一致的 text。每步选中索引 k 后，以
    既有 a、m 和前缀质量 s 计算 lp=a[k]-m-log(s)；total 从 0.0 按 t 升
    序累加 lp，任一结果非有限即失败。stdout 恰为单个 JSON 对象加 LF，
    键序 text,logprobs,total_logprob；text 为生成字符串，logprobs 为
    LENGTH 长字符串列表，第 t 项为 format(lp,'.17g')，total_logprob 为
    format(total,'.17g')。序列化恰用
    json.dumps(obj,ensure_ascii=True,separators=(',',':'),
    allow_nan=False)+'\\n'，不写文件。
    ...
    try:
        if len(argv) == 4 and argv[1] == "perplexity":
            ...
            output = _sample_lstm_attn_topp_scored(argv[2], argv[3], argv[4],
                                                   argv[5], argv[6], argv[7],
                                                   argv[8], argv[9])
            sys.stdout.buffer.write(output.encode("utf-8"))
