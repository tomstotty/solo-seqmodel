...
def _sample_gru_attn(model_path, start, seed_text, temperature_text,
    ...
    抽样及失败契约完全沿用 _sample_gru；WINDOW 的词法与任意位数安全截取
    完全沿用 _perplexity_gru_attn（整串匹配 [1-9][0-9]*，不转 int），否则
    抛 ValueError。整次调用仅初始化一次 r=random.Random(int(SEED))，不写
    任何文件。

    置 h=h0、x=START 索引、memory=[h0]。循环 LENGTH 次：以 x 的 V 长
    one-hot 调用装入 W、b 的 GRUCell.forward(x, h)，取首项更新 h；M 取
    memory 末尾至多 WINDOW 项（顺序从旧到新），以 attention([h], M, M,
    None) 返回首项 ctx，按 i 升序令 u[i] = h[i] + ctx[0][i]。logit 仅以 u
    替代 _sample_gru 中的 h，温度缩放、稳定 softmax、随机阈值、k 升序累计
    ...
    rng = random.Random(seed)
    # 与 _perplexity_gru_attn 相同：h0 保留模型原值，attention 不修改其
    # 输入，故直接共享行即可。
    memory = [h0]
    h = list(h0)
    x = vocab.index(start)
    ...
    for _t in range(length):
        ...
        # 阈值=r.random()*d；自 0.0 依 k 升序累加 e_k，选首个累计值严格
        # 大于阈值者；无则取词表末项。
...
def main(argv):
    ...
    WINDOW 沿用 perplexity-gru-attn 的 [1-9][0-9]* 词法及任意位数安全
    截取。置 h=h0、x=START 索引、memory=[h0]，每步以 x 的 V 长 one-hot
    调用装入 W、b 的 GRUCell.forward 取首项更新 h，M 取 memory 末尾至多
    WINDOW 项（从旧到新）调用 attention([h],M,M,None)，按 i 升序令
    u[i]=h[i]+ctx[0][i]，以 u 替代 h 计算 Why/by logit；温度缩放、稳定
    softmax、整次唯一随机源与词表升序抽样均沿用 sample-gru；生成字符
    作为下一 x，再向 memory 追加 h 的 float 副本。任一中间量非有限即
    失败；输出契约与 sample 相同，不写任何文件。
