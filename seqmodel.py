...
def _sample_lstm_attn_topk_topp_batch(model_path, start, seeds_path,
                                      start_t_text, end_t_text, top_k_text,
                                      top_p_text, length_text, window_text):
    """sample-lstm-attn-top-k-top-p-batch：SEEDS 文件中多种子独立采样。

    SEEDS 为严格 UTF-8 的 JSON 非空数组文件；每个元素 type 恰为 str，且
    整串匹配 _INT_RE（0|-?[1-9][0-9]*，禁止空白、+ 前缀、前导零、下划
    线），重复项按原序保留；文件读取、UTF-8/JSON 解析或元素非法即按既
    有协议失败。MODEL、START、START_T、END_T、TOP_K、TOP_P、LENGTH、
    WINDOW 与其余校验、退火、top-k 后 top-p 截取、随机消费、逐步得分、
    total 累加及错误协议均沿用 sample-lstm-attn-top-k-top-p-scored。
    ...
    累加；medoid 按 (d_i,-total_i,i) 升序取首项下标，total 使用累加所
    得的未格式化 float。不写任何文件。
...
def main(argv):
    ...
    python seqmodel.py sample-lstm-attn-top-k-top-p-batch MODEL START SEEDS
    START_T END_T TOP_K TOP_P LENGTH WINDOW：SEEDS 为严格 UTF-8 的 JSON
    非空数组文件，每个元素 type 恰为 str 且整串匹配 0|-?[1-9][0-9]*，重
    复项按原序保留；文件读取、UTF-8/JSON 解析与元素非法均按既有错误协
    议失败。其余校验、退火、top-k 后 top-p 截取、随机消费、逐步得分与
    错误协议均沿用 sample-lstm-attn-top-k-top-p-scored。按 SEEDS 原序逐
    项独立采样，每项逐值等同于以该 seed 单独调用原入口，项间不共享随
    机源或任何状态。令 d_i=Σ_j H(text_i,text_j)，H 为两串逐码点不等位
    置数，j 按原序累加；medoid 按 (d_i,-total_i,i) 升序取首项下标，
    total 使用未格式化 float。stdout 恰为单个 JSON 对象加 LF，键序
    runs,medoid；runs[i] 恰为 [SEEDS[i],text_i,logs_i,
    format(total_i,'.17g'),d_i]，logs_i 为逐步 format(lp,'.17g') 字符串
    列表，medoid 为所选 int 下标。序列化恰用 json.dumps(obj,
    ensure_ascii=True,separators=(',',':'),allow_nan=False)+'\\n' 的
    UTF-8 字节。失败时 stdout 为空且不写文件。
