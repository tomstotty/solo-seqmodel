...
_LSTM_MHA_RELATIVE_MODEL_KEYS = [
    "version", "vocab", "W", "b", "Wq", "Wk", "Wv", "Wo", "heads", "bias",
    "Why", "by", "h0", "c0"]


def _load_perplexity_lstm_mha_relative_model(path):
    """读取并校验 perplexity-lstm-mha-relative 模型，返回解包后的十三元组。

    契约完全沿用 _load_perplexity_lstm_mha_model，唯一区别为顶层键恰为
    version、vocab、W、b、Wq、Wk、Wv、Wo、heads、bias、Why、by、h0、c0
    且按此顺序出现（十四键）：version 的 type 恰为非 bool int 且值为 5；
    bias 须为 heads×(2R+1) 的 F 矩阵（R>=0），即行数恰为 heads、列数为
    正奇数。其余键规则与 perplexity-lstm-mha 完全相同。
    """
    with open(path, "rb") as f:
        raw = f.read()
    text = raw.decode("utf-8")
    ...
    # bias：heads×(2R+1) 的 F 矩阵，列数须为正奇数。
    bias = model["bias"]
    if type(bias) is not list or len(bias) != heads or len(bias) == 0:
        raise ValueError(
            "bias must be a non-empty list of shape %d×(2R+1)" % heads)
    first_brow = bias[0]
    if (type(first_brow) is not list or len(first_brow) == 0
            or len(first_brow) % 2 != 1):
        raise ValueError(
            ...
            % (first_brow,))
    bias_cols = len(first_brow)
    for brow in bias:
        if type(brow) is not list or len(brow) != bias_cols:
            raise ValueError("bias must be a list of shape %d×%d"
                             % (heads, bias_cols))
        for v in brow:
            if not _is_f(v):
                raise ValueError("bias entries must be finite numbers, got %r"
                                 % (v,))
...
def _perplexity_lstm_mha_relative(model_path, corpus_path, window_text):
    """相对位置偏置的 LSTM-MHA 字符困惑度，返回待写出的字符串。

    MODEL 沿用 perplexity-lstm-mha 契约，顶层键恰为
    version、vocab、W、b、Wq、Wk、Wv、Wo、heads、bias、Why、by、h0、c0
    且按此顺序出现；version 恰为非 bool int 5；bias 为 heads×(2R+1) 的
    F 矩阵（R>=0），列数为正奇数；其余键规则与 perplexity-lstm-mha 相同。
    CORPUS 沿用严格 UTF-8 全文码点、词表及至少 2 码点契约；WINDOW 整串
    匹配 [1-9][0-9]*（任意位数均合法，不转 int），否则抛 ValueError。

    置 h=h0、c=c0、memory=[h0]、pos=[0]，t 升序：以当前字符的 V 长
    one-hot 为 x，调用装入 W、b 的 LSTMCell.forward(x, h, c)，取前两项
    更新 h、c；M 取 memory 末尾至多 WINDOW 项（顺序从旧到新），P 取 pos
    的同一切片，装入四组投影构造 MHA(H, heads)，调用
    forward_cross_relative([h], M, [t+1], P, bias, None)，按 i 升序令
    u[i] = h[i] + ctx[0][i]。logit 仅以 u 替代 perplexity-lstm-mha 中的
    h，其余 Why/by 仿射、稳定 log-sum-exp 及下一字符负对数似然的下标与
    累加顺序均与 perplexity-lstm-mha 相同；随后向 memory 追加 h 的 float
    副本并向 pos 追加 t+1。任一运算非有限（含最终 exp(L/T) 溢出）均抛
    ValueError。成功返回 format(exp(L/T), '.17g') + '\\n'。
    """
    if not _WINDOW_RE.match(window_text):
        raise ValueError("WINDOW must match [1-9][0-9]*")
    ...
    # 记忆序列 [h0, h_0, ..., h_{t-1}] 与对应位置 [0, 1, ..., t]；h0
    # 保留模型原值（F 允许 int），forward_cross_relative 会先复制再投影。
    memory = [h0]
    pos = [0]
    h = list(h0)
    c = list(c0)
    ...
    for t in range(T):
        ...
        # M、P 取同一尾部窗口切片（顺序从旧到新），查询位置为 t+1。
        M = _window_tail(memory, window_text)
        P = pos[len(pos) - len(M):]
...
def main(argv):
    ...
    python seqmodel.py perplexity-lstm-mha-relative MODEL CORPUS WINDOW：
    MODEL 沿用 perplexity-lstm-mha 契约，顶层键仅依次为
    version、vocab、W、b、Wq、Wk、Wv、Wo、heads、bias、Why、by、h0、c0；
    version 恰为非 bool int 5；bias 为 heads×(2R+1) 的 F 矩阵且列数为
    正奇数，其余键规则与 perplexity-lstm-mha 相同；CORPUS、WINDOW 完整
    沿用 perplexity-lstm-mha 契约。置 h=h0、c=c0、memory=[h0]、pos=[0]，
    t 升序推进 LSTM；M 取 memory 末尾至多 WINDOW 项，P 取 pos 同一切片，
    调用 MHA.forward_cross_relative([h], M, [t+1], P, bias, None)，令
    u=h+ctx[0]；以 u 复用 perplexity-lstm-mha 的 Why/by 仿射、稳定
    log-sum-exp 与下一字符负对数似然次序，随后追加 h 的 float 副本与
    t+1；输出契约与 perplexity 相同，不写文件。
    ...
    try:
        if len(argv) == 4 and argv[1] == "perplexity":
            ...
            output = _perplexity_lstm_mha_relative(
