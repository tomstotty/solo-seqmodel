...
class GRUCell(object):
    ...
    def backward_sequence(self, dhs, caches, dh_last=None,
                          tbptt_steps=None):
        """沿整条序列反向传播（可选截断 BPTT），固定返回
        (dxs, dh0, dW, db)。

        caches 须为非空的时序 list，长度确定 T，每项均为 dict 且严格符合
        forward 的公开缓存契约（同 backward 对单个 cache 的九键有序要求）；
        dhs 须为与 caches 同长的 T×H 的 F 列表；dh_last 为 None（等价全零
        末端梯度）或长度 H 的 F 列表；tbptt_steps 为 None 或非 bool 的正
        整数 K。任一校验失败抛 ValueError；实参数量错误沿用 Python 自带的
        TypeError。

        置 ph = dh_last 或零，自 t=T-1 至 0 依次调用现有
        backward(dhs[t]+ph, caches[t])：dx 放回 dxs 的 t 位，返回的
        dh_prev 成为下一步的 ph；dW、db 各元素自 0.0 起按 t 降序、行列
        升序累加。给定 K 时窗口从末端对齐：每处理完 K 步且尚有更早的步，
        便将 ph 清零，故跨窗状态梯度为零。

        四个返回值形状依次为 T×I、H、3H×(I+H)、3H；dh0 即处理 t=0
        一步后所得的状态梯度。所有结果均为全新 float 列表（矩阵逐层
        新建），不修改输入、缓存或参数；任一运算或输出非有限同样抛
        ValueError。
        """
        I, H = self.I, self.H

        # caches：非空时序 list，长度确定 T；各项的九键缓存契约由
        # backward 在处理该步时严格校验（多、少、乱序、形状或元素非法
        # 均抛 ValueError）。
        if type(caches) is not list or len(caches) == 0:
            raise ValueError("caches must be a non-empty list")
        T = len(caches)
        ...
        for t in range(T - 1, -1, -1):
            # dhs[t] + ph：交由 backward 再做 F 校验（溢出即 ValueError）。
            din = [dhs[t][i] + ph[i] for i in range(H)]
            dx, dh_prev, step_dW, step_db = self.backward(din, caches[t])
            dxs[t] = [float(v) for v in dx]

            # 各参数梯度元素自 0.0 起按 t 降序、行列升序累加，累加后须
            # 仍有限。
            for k in range(3 * H):
                row = dW[k]
                srow = step_dW[k]
