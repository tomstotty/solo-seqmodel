...
def _rank_suite_details(models_path, list_path):
    """多个 LSTM-attn 模型在一组 (语料, 窗口) 上的逐项 NLL 明细与排名。

    MODELS、LIST 的读取、校验、相对路径解析与重复项按序保留契约完全沿用
    rank-suite（至少 2 个模型，所有模型 vocab 逐项同序相等，H 可不同）。

    按 MODELS 原下标 mi、LIST 原下标 gi 升序遍历每个 (模型, 清单项) 组合；
    每项均从该模型自身的 h0、c0、memory=[h0] 重置，单步状态推进、注意力、
    logit 与稳定 log-sum-exp 完全沿用 rank-suite。每项 NLL 总和 L 从 0.0
    按 t 升序累加（任一计算非有限即抛 ValueError）；模型总分 S 从 0.0 按
    gi 升序累加未格式化的 L，累加结果非有限即抛 ValueError。

    stdout 恰为单个 JSON 对象加 LF，顶层键序恰为 items,ranking。items 按
    (mi,gi) 展平，每项恰为 [model,corpus,window,steps,item_nll]：model、
    corpus、window 依次为 MODELS、LIST 原文 str，steps 为语料码点数减 1
    的 int，item_nll 为 format(L,'.17g') 字符串。ranking 按 (S,mi) 升序，
    每项恰为 [model,format(S,'.17g')]，与同参 rank-suite 的 ranking 完全
    一致。序列化恰用 json.dumps(obj,ensure_ascii=True,separators=
    (',',':'),allow_nan=False)+'\\n'。整个对象先完整构造再返回，故失败时
    不产生任何部分输出，不写文件。
    """
    with open(models_path, "rb") as f:
    ...
    # 按 (mi, gi) 升序遍历；每项从该模型自身 h0、c0、memory=[h0] 重置。
    items = []
    scores = [0.0] * len(loaded)
    for mi, (cell, Why, by, h0_m, c0_m) in enumerate(loaded):
...
def main(argv):
    ...
    python seqmodel.py rank-suite-details MODELS LIST：MODELS、LIST 的
    读取、校验、相对路径与重复项契约完全沿用 rank-suite。按 MODELS 原
    下标 mi、LIST 原下标 gi 升序遍历每个 (模型, 清单项)，每项均从该
    模型自身 h0、c0、memory=[h0] 重置并逐步计算 NLL（沿用 rank-suite）。
    每项 L 从 0.0 按 t 升序累加；模型总分 S 从 0.0 按 gi 升序累加未
    格式化的 L，任一计算非有限即失败。stdout 恰为单个 JSON 对象加 LF，
    顶层键序恰为 items,ranking：items 按 (mi,gi) 展平，每项恰为
    [model,corpus,window,steps,item_nll]（前三项为 MODELS/LIST 原文
    str，steps 为语料码点数减 1 的 int，item_nll 为
    format(L,'.17g')）；ranking 按 (S,mi) 升序，每项恰为
    [model,format(S,'.17g')]，与同参 rank-suite 的 ranking 一致。
    序列化恰用 json.dumps(obj,ensure_ascii=True,separators=(',',':'),
    allow_nan=False)+'\\n'；失败时不输出任何部分行，不写文件。
