...
def _rank_window_sensitivity(models_path, list_path, base_text):
    """各模型清单窗口相对基准窗 BASE 的逐项 NLL 偏移总量排名。

    MODELS、LIST 的读取、校验、相对路径解析与重复项保序完全沿用
    rank-suite-details（MODELS 为至少 2 项的非空相对路径 str 数组；
    LIST 为非空 [corpus, window] 数组；所有模型 vocab 须逐项同序相等，
    H 可不同）。BASE 整串匹配 [1-9][0-9]*（任意位数均合法，不转 int），
    窗口截取沿用 _window_tail，否则抛 ValueError。

    按模型原下标 mi、清单原下标 gi 升序遍历 (mi, gi)；每项分别以该清单
    项的 window 与 BASE 从该模型的 h0、c0、memory=[h0] 独立重置，两条
    轨道的单步状态推进、注意力、logit 与稳定 log-sum-exp 完全沿用
    rank-suite-details。每项 window 轨 NLL 总和 L、BASE 轨 NLL 总和 B
    各从 0.0 按 t 升序累加；令 delta=L-B，模型敏感度 A 从 0.0 按 gi
    升序累加 abs(delta)，任一计算非有限即抛 ValueError。整个对象先完
    整构造再返回，失败时不产生任何部分输出，不写文件。

    stdout 恰为单个 JSON 对象加 LF，顶层键序 items,ranking。items 按
    (mi, gi) 展平，每项恰为
    [model,corpus,window,steps,item_nll,base_nll,delta]：前三项依次为
    MODELS 原文 str、LIST 原文 corpus str、LIST 原文 window str；steps
    为语料码点数减 1 的 int；item_nll、base_nll、delta 依次为
    format(L,'.17g')、format(B,'.17g')、format(delta,'.17g') 字符串。
    ranking 按 (-A, mi) 升序（A 以 float 精确比较，下标决胜保序），每
    项恰为 [model, format(A,'.17g')]。序列化恰用
    ...
    items = []
    # 每个模型的敏感度 A 从 0.0 起，按 gi 升序累加 abs(delta)。
    sensitivities = [0.0] * len(loaded)
    for mi, (cell, Why, by, h0_m, c0_m) in enumerate(loaded):
        for gi, (corpus_text, window_text) in enumerate(entries):
            ids = entry_ids[gi]
            T = len(ids) - 1

            # 每项分别以 window、BASE 从该模型 h0、c0、memory=[h0] 独立重置。
            memory_w = [h0_m]
            h_w = list(h0_m)
            c_w = list(c0_m)
            memory_b = [h0_m]
            ...
            B = 0.0
            for t in range(T):
                nll_w, h_w, c_w, memory_w = _step_nll(
                    cell, Why, by, h_w, c_w, memory_w, ids, t, window_text)
                L += nll_w
                if not math.isfinite(L):
                    raise ValueError(
                        "item total NLL accumulated non-finitely")
                nll_b, h_b, c_b, memory_b = _step_nll(
                    cell, Why, by, h_b, c_b, memory_b, ids, t, base_text)
                B += nll_b
                if not math.isfinite(B):
                    raise ValueError(
                        "base total NLL accumulated non-finitely")

            delta = L - B
            if not math.isfinite(delta):
                raise ValueError("item delta is non-finite")
            sensitivities[mi] += abs(delta)
            if not math.isfinite(sensitivities[mi]):
                raise ValueError(
    ...
    order = sorted(range(len(loaded)),
                   key=lambda mi: (-sensitivities[mi], mi))
    ranking = [[model_paths[mi], format(sensitivities[mi], ".17g")]
               for mi in order]
...
def main(argv):
    ...
    rank-suite-details（至少 2 个模型，vocab 同序，H 可不同），既有接
    口不变。BASE 整串匹配 [1-9][0-9]*（任意位数安全处理，不转 int）。
    按模型原下标 mi、清单原下标 gi 升序遍历，每项分别以该清单项的
    window 与 BASE 从该模型的 h0、c0、memory=[h0] 独立重置并沿用其逐
    步 NLL。每项 window 轨总和 L、BASE 轨总和 B 各从 0.0 按 t 升序累
    加；令 delta=L-B，模型敏感度 A 从 0.0 按 gi 升序累加 abs(delta)，
    任一计算非有限即失败。stdout 恰为键序 items,ranking 的单个 JSON
    对象加 LF。items 按 (mi,gi) 展平，每项恰为
    [model,corpus,window,steps,item_nll,base_nll,delta]，前三项是
    MODELS/LIST 原文 str，steps 是语料码点数减 1 的 int，后三项依次
    format(L,'.17g')、format(B,'.17g')、format(delta,'.17g')；
    ranking 按 (-A,mi) 升序，每项恰为 [model,format(A,'.17g')]。
    序列化与失败协议沿用 rank-suite-details；失败不部分输出、不写文件。
