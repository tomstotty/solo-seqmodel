...
def _score_lstm_attn(model_path, start, text, temperature_text, window_text):
    """确定性地对给定候选 TEXT 打总对数概率分，返回待写出的字符串。

    MODEL、START、TEMPERATURE、WINDOW 的读取、词法、形状与 F 校验完全沿用
    _sample_lstm_attn；本命令无 SEED、无随机源且不写任何文件。TEXT 为可空
    Unicode 字符串，其每个码点都须在词表中，否则抛 ValueError。

    置 h=h0、c=c0、x=START 索引、memory=[h0]、total=0.0。按 t 升序遍历
    TEXT：每步沿用 _sample_lstm_attn 的 LSTM 推进与窗口注意力得到 u，按原
    顺序计算 a[k]=z[k]/TEMPERATURE、m=max(a)、e[k]=exp(a[k]-m)，d 从 0.0
    按 k 升序累加 e[k]；不抽样，令 y 为 TEXT[t] 的词表索引，
    lp=a[y]-m-math.log(d)，total 按 t 升序累加 lp，置 x=y，并向 memory 追
    加 h 的 float 副本。任一计算非有限即抛 ValueError。成功返回
    format(total, '.17g') + '\\n'；TEXT 为空时 total 为 0.0，输出 "0\\n"。
    """
    if not _WINDOW_RE.match(window_text):
        raise ValueError("WINDOW must match [1-9][0-9]*")
    ...
    # TEXT：可空 Unicode 字符串，每个码点都须在词表中（按码点迭代）。
    if type(text) is not str or any(ch not in vocab for ch in text):
        raise ValueError("TEXT must be a possibly-empty string of in-vocab "
                         "codepoints")
    ...
    for ch in text:
        ...
        # logit 仅以 u 替代 h；下标与累加顺序同 _sample_lstm_attn。
        z = _output_logits(Why, by, u)
...
def main(argv):
    ...
    python seqmodel.py score-lstm-attn MODEL START TEXT TEMPERATURE WINDOW：
    确定性地为指定候选 TEXT 打总对数概率分，无 SEED、无随机源且不写文件。
    MODEL、START、TEMPERATURE、WINDOW 沿用 sample-lstm-attn；TEXT 为可空
    Unicode 字符串，每个码点都须在词表中，否则失败。置 h=h0、c=c0、
    x=START 索引、memory=[h0]、total=0.0，按 t 升序遍历 TEXT：每步以 x 的
    V 长 one-hot 推进 h、c，经窗口注意力得到 u，按原顺序计算
    a[k]=z[k]/TEMPERATURE、m=max(a)、e[k]=exp(a[k]-m)，d 从 0.0 按 k 升序
    累加 e[k]；不抽样，令 y 为 TEXT[t] 索引，
    lp=a[y]-m-math.log(d)，total 按 t 升序累加 lp，置 x=y，memory 追加 h
    的 float 副本；任一计算非有限即失败。成功时 stdout 恰为
    format(total,'.17g')+'\\n' 的 ASCII 字节（TEXT 为空时为 "0\\n"），
    stderr 为空、返回 0；相同输入逐字节一致。
