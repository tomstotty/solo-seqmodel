...
def _lstm_attn_mean_lag(model_path, corpus_path, window_text):
    """perplexity-lstm-attn 每步注意力的平均滞后，返回待写出的字符串。
    ...
    q=start+j、r=t+1-q，自 0.0 累加 lag+=w[0][j]*float(r)，再令 A+=lag；
    各乘加或 A/T 非有限均抛 ValueError。构造 JSON 对象：顶层键序恰为
    version,items,mean_lag，值依次为 int 1、T 长列表、
    format(A/T,'.17g') 字符串；items 第 t 项键序恰为 t,start,lag，值依次
    为 int t、int start、format(lag,'.17g') 字符串。返回
    json.dumps(obj,ensure_ascii=True,separators=(',',':'),
    allow_nan=False)+'\\n'，不写文件。
    """
    if not _WINDOW_RE.match(window_text):
    ...
    for t in range(T):
        ...
        # 第 t 步滞后：lag 自 0.0 起按 j 升序以 q=start+j、r=t+1-q
        # 累加 w[0][j]*float(r)。
        lag = 0.0
