...
def _score_lstm_mha_relative(model_path, start, text, temperature_text,
                             window_text):
    """确定性评分指定候选 TEXT（LSTM-MHA 相对位置），返回待写出的字符串。
    ...
    LSTMCell.forward 更新 h、c；M 取 memory 末尾至多 WINDOW 项（顺序从
    旧到新），P 取 pos 的同一切片，装入 Wq、Wk、Wv、Wo 构造
    MHA(H, heads)，以 forward_cross_relative([h], M, [t+1], P, bias,
    None) 返回首项 ctx，按 i 升序令 u[i]=h[i]+ctx[0][i]；按原 Why/by
    次序求 z，再计算 a[k]=z[k]/TEMPERATURE、m=max(a)、
    e[k]=exp(a[k]-m)，d 自 0.0 依 k 升序累加 e[k]；不抽样，令 y 为
    TEXT[t] 索引，lp=a[y]-m-math.log(d)，total 按 t 升序累加 lp，置
    x=y，memory 追加 h 的 float 副本、pos 追加 t+1。任一计算非有限均
    抛 ValueError。成功返回 format(total, '.17g') + '\\n'（TEXT 为空时
    为 "0\\n"）。
    """
    if not _WINDOW_RE.match(window_text):
        raise ValueError("WINDOW must match [1-9][0-9]*")
...
def main(argv):
    ...
    每步以 x 的 one-hot 推进 h、c，M 取 memory 末尾至多 WINDOW 项（从
    旧到新），P 取 pos 的同一切片，以装入四组投影的 MHA 调用
    forward_cross_relative([h], M, [t+1], P, bias, None)，首行 ctx 与 h
    逐项求和得 u，经 Why/by 得 z，按原顺序计算 a[k]=z[k]/TEMPERATURE、
    m=max(a)、e[k]=exp(a[k]-m)，d 自 0.0 依 k 升序累加 e[k]；不抽样，
    令 y 为 TEXT[t] 索引，lp=a[y]-m-math.log(d)，total 按 t 升序累加
    lp，置 x=y，memory 追加 h 的 float 副本、pos 追加 t+1。任一计算非
    有限即失败。成功时 stdout 恰为 format(total,'.17g')+'\\n' 的 ASCII
    字节（空 TEXT 为 "0\\n"），stderr 为空并返回 0。
    ...
    try:
        if len(argv) == 4 and argv[1] == "perplexity":
            ...
            output = _score_lstm_mha_relative(argv[2], argv[3], argv[4],
                                              argv[5], argv[6])
            sys.stdout.buffer.write(output.encode("ascii"))
