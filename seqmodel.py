...
def _load_transformer_resume_state(path, sha_hex, expected_args, vocab_size,
    ...
    sha 为 MODEL 字节的小写 SHA-256 且逐字符相等；args 为 3 个原文 str
    组成的数组且恰等于 [START,SEED,TEMPERATURE]；p 为非空词表索引列表，
    每项均为 [0,V) 内的非 bool int 且首项恰为 START 索引；r 符合
    _resume_getstate_from_json 的结构，并能通过 random.Random.setstate
    还原。任何读取、UTF-8、JSON、类型、形状、摘要、args 或随机状态校验
    ...
    if type(prefix) is not list or len(prefix) == 0:
        raise ValueError("resume p must be a non-empty list of indices")
    ...
    if prefix[0] != start_index:
        raise ValueError("resume p must start with the START index")
...
def _sample_transformer_resume(model_path, start, seed_text,
    ...
    MODEL、START、SEED、TEMPERATURE、LENGTH 的词法与校验、模型十四键
    装载、prefix 的 one-hot 输入、因果 mask、TransformerBlock.forward、
    Why/by logit、温度缩放、稳定 softmax、词表升序阈值抽样、唯一随机源
    及有限性失败契约均逐项沿用 sample-transformer（模型参数直接装入、
    不经随机初始化）；STATE 为状态文件路径。

    STATE 缺失时置 prefix=[START 索引]、rng=random.Random(int(SEED))；
    STATE 存在时读取严格 UTF-8 JSON，顶层键依次且仅为 v,sha,args,p,r：
    v 为 int 1；sha 为 MODEL 原始字节的小写 SHA-256；args 为保留原文的
    [START,SEED,TEMPERATURE] 字符串数组；p 为非空词表索引列表，元素是
    [0,V) 内非 bool int 且首项为 START 索引；r 为 getstate 递归将 tuple
    转 list 所得 [3,[625 个非 bool int],null]。类型、形状、摘要、args
    或随机状态不符均抛 ValueError。
    ...
    存在）状态，stdout 仅为 LF。先跑 p 步再续跑 q 步的文本拼接与终态
    字节，须与一次跑 p+q 步逐字节相同。
    """
...
def main(argv):
    ...
    TEMPERATURE LENGTH STATE：前五参数及生成行为逐项沿用
    sample-transformer，另以 STATE 原子读写续跑状态。STATE 缺失时从
    prefix=[START 索引] 与唯一随机源 random.Random(int(SEED)) 初始化；
    STATE 存在时读取严格 UTF-8 JSON，顶层键依次且仅为 v,sha,args,p,r：
    v 为 int 1，sha 为 MODEL 原始字节的小写 SHA-256，args 为保留原文的
    [START,SEED,TEMPERATURE] 字符串数组，p 为非空词表索引 list（元素是
    [0,V) 内非 bool int 且首项为 START 索引），r 为 getstate 递归将
    tuple 转 list 所得 [3,625 个非 bool int,null]。类型、形状、摘要、
    args 或状态不符均失败。生成 LENGTH 步后状态写为 ASCII 转义、禁非
    有限数的紧凑 JSON UTF-8 字节加 LF；先写全 STATE 同目录临时文件并
    os.replace 后，stdout 才写本段文本加 LF。分段 p、q 步的文本与终态
    须与一次 p+q 步逐字节相同；LENGTH=0 仅创建或校验 STATE 且 stdout 仅
