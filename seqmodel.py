...
class TransformerBlock(object):
    ...
    def forward_relative(self, x, bias, mask=None):
        """相对位置偏置的 Transformer 块前向，返回 (y, w) 并缓存反向快照。

        x、mask 与 W1/b1/W2/b2 的契约完全沿用 forward；bias 须为
        heads×(2R+1) 的 F 矩阵（R>=0），即行数恰为 heads、列数为正奇数，
        否则抛 ValueError。先取 (c, w) = attn.forward_relative(x, bias,
        mask)，再求 a = x + c、f = tanh(b1 + W1*a)、y = a + b2 + W2*f。
        y 为 T×D、w 为 heads×T×T 的逐层新建 float 列表，不修改或复用
        输入与参数。任一中间量非有限抛 ValueError；任何失败都清空本块
        缓存。实参数量错误沿用 Python 自带的 TypeError。
        """
        D, P = self.D, self.P
        # 任何失败的 forward_relative 都使既有缓存失效。
        self._cache = None
    ...
    def backward_relative(self, dy):
        """相对位置偏置的 Transformer 块反向，返回十元组梯度。

        须紧随一次成功的 forward_relative（其后缓存未被失败清空），否则
        抛 ValueError；dy 须为与前向 y 同形的 T×D F 矩阵，否则抛
        ValueError。按 backward 相同的公式与次序（求和均自 0.0 按下标
        升序累加）求 dW2、db2、df、dz、dW1、db1 与 da，再调用
        attn.backward_relative(da) 得 (dxa, dWq, dWk, dWv, dWo, dbias)，
        令
            dx[t][j] = da[t][j] + dxa[t][j]
        返回 (dx, dWq, dWk, dWv, dWo, dW1, db1, dW2, db2, dbias)。dx 为
        T×D，四个注意力梯度为 D×D，dW1 为 P×D、db1 长 P，dW2 为 D×P、
        ...
        try:
            ...
            # 相对位置注意力子层（含其四个投影与 bias）的反向沿用
            # MHA.backward_relative。
            dxa, dWq, dWk, dWv, dWo, dbias = self.attn.backward_relative(da)
