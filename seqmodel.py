...
def _sample_transformer_top_k(model_path, start, seed_text, start_t_text,
    ...
    """以线性退火温度、top-k 抽样从单层 Transformer 块语言模型采样。

    除 TOP_K 与候选选样外，MODEL、START、SEED、START_T、END_T、LENGTH 的校
    验，以及 LENGTH 的 0/1 语义、prefix 的 one-hot 输入、因果 mask、
    TransformerBlock.forward、Why/by 仿射、线性温度退火、稳定 softmax、唯一
    随机源、prefix 状态更新与有限性失败契约均沿用 _sample_transformer_anneal
    ；不写任何文件。
    ...
    每步先按原顺序求 a[k]、m=max(a)、e[k]=exp(a[k]-m)。再将索引按
    (-e[k], k) 升序排列（e 降序、并列时 k 升序），候选恰为该序前 K 项；
    s 自 0.0 按候选序累加 e。令 u=random()*s，再从 0.0 依候选序累加 e，
    选择首个累计值严格大于 u 的索引；无则取候选末项。选中字符追加到输
    出，其索引追加到 prefix。任一新增运算非有限均抛 ValueError。成功返回
    LENGTH 个码点再加一个 LF。
    """
    (vocab, heads, P, Wq, Wk, Wv, Wo, W1, b1, W2, b2, Why, by) = \
        _load_perplexity_transformer_model(model_path)
    ...
    for t in range(length):
        ...
        # a_k=z_k/T，m=max(a)，e_k=exp(a_k-m)。
        a = [0.0] * V
        m = None
        for k in range(V):
            ak = z[k] / temperature
...
def main(argv):
    ...
    END_T TOP_K LENGTH：除 TOP_K 与候选选样外，MODEL、START、SEED、
    START_T、END_T、LENGTH 的校验，以及 LENGTH 的 0/1 语义、Transformer
    前向、Why/by 仿射、线性温度、稳定 softmax、唯一随机源、prefix 状态更新
    与有限性错误协议均沿用 sample-transformer-anneal。TOP_K 整串匹配
    [1-9][0-9]* 且数学值 K<=V=len(vocab)：先按十进制位数及同长字典序与 V
    比较，越界失败，通过后才转 int，超长文本不得触发整数转换异常。每步先
    按原次序求 a[k]、m=max(a)、e[k]=exp(a[k]-m)；索引按 (-e[k],k) 升序，
    候选为前 K 项，s 从 0.0 按候选序累加。令 u=random()*s，再依序累加 e，
    选首个累计值严格大于 u 的索引，无则取候选末项；字符及索引追加到输出
    和 prefix；新增运算非有限即失败。输出契约与 sample 相同，不写文件。
