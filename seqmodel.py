...
class MHA(object):
    ...
    def forward_relative(self, x, bias, mask=None):
        """相对位置偏置多头自注意力前向，返回 (y, w) 并缓存反向快照。

        x、mask 与四个投影参数的契约完全沿用 forward；bias 须为
        heads×(2R+1) 的 F 矩阵（R>=0），即行数恰为 heads、列数为正奇数，
        否则抛 ValueError。依次求 q=xWqᵀ、k=xWkᵀ、v=xWvᵀ，令 d=D/heads；
        对第 r 头、查询 i、键 j，分数 s 从 0.0 按 a 升序累加
        q[i][r*d+a]*k[j][r*d+a]，再取 s/sqrt(d)+
        bias[r][max(-R,min(R,j-i))+R]。mask、softmax、值加权与 y=cWoᵀ
        均沿用 forward（同一 mask 作用于各头）。y 为 T×D、w 为
        heads×T×T 的逐层新建 float 列表并缓存，不修改或复用输入与投影
        属性。任一中间量非有限抛 ValueError；任何失败都清空缓存。实参
        数量错误沿用 Python 自带的 TypeError。
        """
        D, heads = self.D, self.heads
        ...
        try:
            xc = self._check_x(x)
            T = len(xc)
            if type(bias) is not list or len(bias) != heads or len(bias) == 0:
                raise ValueError(
                    "bias must be a non-empty list of shape %d×(2R+1)"
                    % heads)
            first_brow = bias[0]
            if type(first_brow) is not list or len(first_brow) == 0 \
                    or len(first_brow) % 2 != 1:
                raise ValueError(
                    "bias must have a positive odd number of columns, "
                    "got %r" % (first_brow,))
            W = 2 * (len(first_brow) // 2) + 1
            R = W // 2
            for brow in bias:
                if type(brow) is not list or len(brow) != W:
                    raise ValueError(
                        "bias must be a list of shape %d×%d" % (heads, W))
                for bv in brow:
                    if not _is_f(bv):
                        raise ValueError(
                            "bias entries must be finite numbers, got %r"
                            % (bv,))
            bias_c = [[float(bv) for bv in brow] for brow in bias]
            Wq = _check_matrix(self.Wq, D, D, "Wq")
            Wk = _check_matrix(self.Wk, D, D, "Wk")
            Wv = _check_matrix(self.Wv, D, D, "Wv")
            Wo = _check_matrix(self.Wo, D, D, "Wo")
            ...
            # mask 校验沿用 _attention_check 的同一契约（形状、bool 类型、
            # 全屏蔽行），各头共用同一快照。
            _, _, _, _, active = _attention_check(q, k, v, mask)

            hd = D // heads
            scale = math.sqrt(float(hd))
            c = [[0.0] * D for _ in range(T)]
            w = []
            for r in range(heads):
                ...
                for i in range(T):
                    ...
                    # s[i][j] 从 0.0 按 a 升序累加头内点积，再除 sqrt(d)
                    # 并加相对位置偏置；仅 True 位参与 softmax。
                    s_row = [0.0] * T
                    for j in range(T):
                        kj = k[j]
                        acc = 0.0
                        for a in range(hd):
                            try:
                                acc += qi[off + a] * kj[off + a]
                            except OverflowError:
                                raise ValueError(
                                    "score dot product accumulated to a "
                                    "non-finite value")
                            if not math.isfinite(acc):
                                raise ValueError(
                                    "score dot product accumulated to a "
                        ...
                        if not math.isfinite(sval):
                            raise ValueError(
                                "score became non-finite after scaling "
                                "and bias")
                        s_row[j] = sval

                    # 行最大值只在 True 位取（每行至少一个 True）。
                    m = None
                    for j in range(T):
                        if acti[j]:
                            sj = s_row[j]
                    ...
                    # e = exp(s - m) 仅在 True 位计算；分母按 j 升序累加。
                    e_row = [0.0] * T
                    denom = 0.0
                    for j in range(T):
                        if acti[j]:
                            try:
                                ...
                                raise ValueError("softmax exp overflowed")
                            if not math.isfinite(ev):
                                raise ValueError("softmax exp became "
                                                 "non-finite")
                            e_row[j] = ev
                            denom += ev
                            if not math.isfinite(denom):
                                raise ValueError(
                                    "softmax denominator accumulated to a "
                                    "non-finite value")
            ...
            # c[i][r*d+a] 从 0.0 按 j 升序累加 w[r][i][j]*v[j][r*d+a]。
            for r in range(heads):
                off = r * hd
                wr = w[r]
                for i in range(T):
        ...
        self._cache = ("mha_relative", T, D, heads, R, xc, q, k, v, c, w,
                       active, mask_c, Wq, Wk, Wv, Wo, bias_c)
        return y, w

    def backward_relative(self, dy):
        """相对位置偏置多头自注意力反向，返回
        (dx, dWq, dWk, dWv, dWo, dbias)。

        须紧随一次成功的 forward_relative（其后缓存未被失败清空），否则
        抛 ValueError；dy 须为与前向 y 同形的 T×D F 矩阵，否则抛
        ValueError。dWo 与 dc 沿用 backward 的 Wo 路径；各头值路径
            dv[j][a] = Σ_i w[r][i][j]*dc[i][a]
        沿用 backward 的注意力值路径（屏蔽位 w 为 0.0）。令
            dw[i][j] = Σ_a dc[i][a]*v[j][a]（屏蔽位取 0.0）
            ds[i][j] = w[i][j]*(dw[i][j]-Σ_k w[i][k]*dw[i][k])
        （屏蔽位取 0.0；和按 k 升序自 0.0 累加），则头内
            dq[i][a] = Σ_j (ds[i][j]/sqrt(d))*k[j][a]
            dk[j][a] = Σ_i (ds[i][j]/sqrt(d))*q[i][a]
        （j、i 分别升序）；dWq/dWk/dWv 与 dx 的投影梯度沿用 backward 的
        次序合并三路径。dbias 为与 bias 同形的零矩阵，按 r、i、j 升序把
        ds[i][j] 累加至偏移桶 max(-R,min(R,j-i))+R。所有结果逐层新建
        float 列表，不修改 dy、缓存或投影属性；成功时缓存保留，重复调用
        结果相同。任一中间量非有限或反向失败均抛 ValueError 并清空缓存。
        实参数量错误沿用 Python 自带的 TypeError。
        """
        cache = self._cache
        try:
            if type(cache) is not tuple or len(cache) != 18 \
                    or cache[0] != "mha_relative":
                raise ValueError(
                    "backward_relative requires a successful "
                    "forward_relative pass before it")
            (_, T, D, heads, R, xc, q, k, v, c, w, active, mask_c,
             Wq, Wk, Wv, Wo, bias_c) = cache
            dy = _check_matrix(dy, T, D, "dy")
            dy = [[float(z) for z in row] for row in dy]
            ...
            # 按 r、i、j 升序累加 dbias；其余每头梯度按头序拼回。
            for r in range(heads):
                off = r * hd
                wr = w[r]
                dbrow = dbias[r]
                for i in range(T):
                    ...
                    # dw[i][j] = Σ_a dc[i][off+a]*v[j][off+a]，a 升序，
                    # 仅 True 位。
                    dw = [0.0] * T
                    for j in range(T):
                        if acti[j]:
                    ...
                    # rsum = Σ_k w[i][k]*dw[i][k]，k 升序（屏蔽位 w 为
                    # 0.0）。
                    rsum = 0.0
                    for kk in range(T):
                        try:
                            ...
                            raise ValueError(
                                "rsum accumulated to a non-finite value")
                        if not math.isfinite(rsum):
                            raise ValueError(
                                "rsum accumulated to a non-finite value")

                    # ds[i][j] = w*(dw-rsum)，仅 True 位；随即除以 sqrt(d)
                    # 用于 dq/dk，并按偏移桶累加 dbias。
                    dq_i = dq[i]
                    for j in range(T):
                        if acti[j]:
                            try:
                                dsij = wri[j] * (dw[j] - rsum)
                            except OverflowError:
                                raise ValueError(
                                    "ds became non-finite")
                            if not math.isfinite(dsij):
                                raise ValueError("ds became non-finite")
                            bucket = max(-R, min(R, j - i)) + R
                            dbrow[bucket] += dsij
                            if not math.isfinite(dbrow[bucket]):
                                raise ValueError(
                                    "dbias accumulated to a non-finite value")
                            g = dsij / scale
                            if not math.isfinite(g):
                                raise ValueError(
                                    "ds became non-finite after scaling")
                            kj = k[j]
                            for a in range(hd):
                                try:
                                    dq_i[off + a] += g * kj[off + a]
                                except OverflowError:
                                    raise ValueError(
                                        "dq accumulated to a non-finite value")
                                if not math.isfinite(dq_i[off + a]):
                                    raise ValueError(
                                        "dq accumulated to a non-finite value")
                            dk_j = dk[j]
                            qi = q[i]
                            for a in range(hd):
                                try:
                                    dk_j[off + a] += g * qi[off + a]
                                except OverflowError:
                                    raise ValueError(
                                        "dk accumulated to a non-finite value")
                                if not math.isfinite(dk_j[off + a]):
                                    raise ValueError(
                                        "dk accumulated to a non-finite value")

                # dv[j][off+a] = Σ_i w[r][i][j]*dc[i][off+a]，i 升序。
                for j in range(T):
                    dvj = dv[j]
                    for a in range(hd):
                        acc = 0.0
    ...
    def forward_cross_relative(self, qx, kvx, qpos, kpos, bias, mask=None):
        """相对位置偏置多头交叉注意力前向，返回 (y, w) 并缓存反向快照。

        qx、kvx、mask 与四个投影参数的契约完全沿用 forward_cross；qpos、
        kpos 须分别为 Tq、Tk 长 list，各项 type 为非 bool 的 int 且严格递增；
        bias 须为 heads×(2R+1) 的 F 矩阵（R>=0），即行数恰为 heads、列数为
        正奇数，否则抛 ValueError。依次求 q=qxWqᵀ、k=kvxWkᵀ、v=kvxWvᵀ，令
        d=D/heads；对第 r 头、查询 i、键 j，分数 s 从 0.0 按 a 升序累加
        q[i][r*d+a]*k[j][r*d+a]，再取 s/sqrt(d)+
        bias[r][max(-R,min(R,kpos[j]-qpos[i]))+R]。mask、softmax、值加权与
        y=cWoᵀ 均沿用 forward_cross（同一 mask 作用于各头）。y 为 Tq×D、w
        为 heads×Tq×Tk 的逐层新建 float 列表并缓存，不修改或复用输入与投影
        属性。任一校验或中间量非有限抛 ValueError；任何失败都清空缓存。实参
        数量错误沿用 Python 自带的 TypeError。
        """
        D, heads = self.D, self.heads
        # 任何失败的 forward_cross_relative 都使既有缓存失效。
        self._cache = None
        Tq = 0
        try:
            qxc = self._check_x(qx)
            ...
            # qpos：Tq 长 list，各项为非 bool int 且严格递增。
            if type(qpos) is not list or len(qpos) != Tq:
                raise ValueError(
                    "qpos must be a list of length %d" % Tq)
            qpos_c = []
            for p in qpos:
                if type(p) is bool or type(p) is not int:
                    raise ValueError(
                        "qpos entries must be non-bool integers, got %r"
                        % (p,))
                if qpos_c and p <= qpos_c[-1]:
                    raise ValueError(
                        "qpos must be strictly increasing, got %r after %r"
                        % (p, qpos_c[-1]))
                qpos_c.append(p)

            # kpos：Tk 长 list，各项为非 bool int 且严格递增。
            if type(kpos) is not list or len(kpos) != Tk:
                raise ValueError(
                    "kpos must be a list of length %d" % Tk)
            kpos_c = []
            for p in kpos:
                if type(p) is bool or type(p) is not int:
                    raise ValueError(
                        "kpos entries must be non-bool integers, got %r"
                        % (p,))
                if kpos_c and p <= kpos_c[-1]:
                    raise ValueError(
                        "kpos must be strictly increasing, got %r after %r"
                        % (p, kpos_c[-1]))
                kpos_c.append(p)
        ...
        self._cache = ("mha_cross_relative", Tq, Tk, D, heads, R, qxc, kvxc,
                       qpos_c, kpos_c, q, k, v, c, w, active, mask_c,
                       Wq, Wk, Wv, Wo, bias_c)
        return y, w

    def backward_cross_relative(self, dy):
        """相对位置偏置多头交叉注意力反向，返回
        (dqx, dkvx, dWq, dWk, dWv, dWo, dbias)。

        须紧随一次成功的 forward_cross_relative（其后缓存未被任何其他
        forward 类调用替换或被失败清空），否则抛 ValueError；dy 须为与前向
        y 同形的 Tq×D F 矩阵，否则抛 ValueError。dWo 与 dc 沿用
        backward_cross 的 Wo 路径；各头值路径
            dv[j][a] = Σ_i w[r][i][j]*dc[i][a]
        沿用 backward_cross 的注意力值路径（屏蔽位 w 为 0.0）。令
            dw[i][j] = Σ_a dc[i][a]*v[j][a]（屏蔽位取 0.0）
            ds[i][j] = w[i][j]*(dw[i][j]-Σ_k w[i][k]*dw[i][k])
        （屏蔽位取 0.0；和按 k 升序自 0.0 累加），则头内
            dq[i][a] = Σ_j (ds[i][j]/sqrt(d))*k[j][a]
            dk[j][a] = Σ_i (ds[i][j]/sqrt(d))*q[i][a]
        （j、i 分别升序），公式与次序沿用 backward_relative；dWq、dWk、
        dWv、dqx、dkvx 的投影梯度沿用 backward_cross 的公式与次序（dWq 沿
        qx、dWk/dWv 与 dkvx 沿 kvx）。dbias 为与 bias 同形的零矩阵，按 r、
        i、j 升序把 ds[i][j] 累加至偏移桶
            max(-R,min(R,kpos[j]-qpos[i]))+R。
        所有结果逐层新建 float 列表，不修改 dy、缓存或投影属性；成功时缓存
        保留，重复调用结果相同。任一中间量非有限或反向失败均抛 ValueError
        并清空缓存。实参数量错误沿用 Python 自带的 TypeError。
        """
        cache = self._cache
        try:
            if type(cache) is not tuple or len(cache) != 22 \
                    or cache[0] != "mha_cross_relative":
                raise ValueError(
                    "backward_cross_relative requires a successful "
                    "forward_cross_relative pass before it")
            (_, Tq, Tk, D, heads, R, qxc, kvxc, qpos_c, kpos_c, q, k, v,
             c, w, active, mask_c, Wq, Wk, Wv, Wo, bias_c) = cache
            dy = _check_matrix(dy, Tq, D, "dy")
            dy = [[float(z) for z in row] for row in dy]
        ...
        return dqx, dkvx, dWq, dWk, dWv, dWo, dbias
...
class TransformerBlock(object):
    """残差多头自注意力加位置前馈的单层 Transformer 块。

    结构（无归一化层）：
        c, w = attn(x, mask)          # attn 为 MHA(D, heads)
        a = x + c                     # 注意力残差
        f = tanh(b1 + W1 @ a)         # W1: P×D，b1: P
        y = a + b2 + W2 @ f           # W2: D×P，b2: D
    初始权重满足 W1[p][j] = W2[j][p] = float(p % D == j)，b1、b2 全 0.0；
    所有参数逐层新建、互不共享。D、P、heads 须为非 bool 的正 int 且 heads
    整除 D，否则抛 ValueError。
    """
    ...
    def __init__(self, D, heads, P):
        ...
        # W1 第 p 行在 p%D 列为 1.0；W2 第 j 行在所有满足 p%D==j 的 p 列
        # 为 1.0。逐行新建，偏置全 0.0。
        self.W1 = [[1.0 if p % D == j else 0.0 for j in range(D)]
                   for p in range(P)]
        self.b1 = [0.0] * P
        self.W2 = [[1.0 if p % D == j else 0.0 for p in range(P)]
                   for j in range(D)]
        self.b2 = [0.0] * D
        self._cache = None
    ...
    def forward(self, x, mask=None):
        ...
        中间量非有限抛 ValueError；任何失败都清空本块缓存。实参数量错误
        ...
        self._cache = None
        try:
            # x、mask 的校验与 attn 自身缓存均由 MHA.forward 保证；c、w
            # 为逐层新建的 float 列表。
        ...
        self._cache = ("transformerblock", T, D, P, a, f, W1, W2)
        return y, w

    def backward(self, dy):
        """Transformer 块反向，返回九元组梯度。

        须紧随一次成功的 forward（其后缓存未被失败清空），否则抛
        ValueError；dy 须为与前向 y 同形的 T×D F 矩阵，否则抛 ValueError。
        按链式求（求和均自 0.0 按下标升序累加）：
        ...
        四个注意力梯度为 D×D，dW1 为 P×D、db1 长 P，dW2 为 D×P、db2 长 D，
        均为逐层新建 float 列表，不修改 dy、缓存或任何参数；成功时缓存
        保留，重复调用结果相同。任一中间量非有限或反向失败均抛 ValueError
        并清空缓存。实参数量错误沿用 Python 自带的 TypeError。
        """
        cache = self._cache
        try:
            ...
            # dz[t][p] = df[t][p]*(1 - f[t][p]^2)，逐元素新建 float。
            ...
            for t in range(T):
                ...
                for p in range(P):
                    ...
                    if not math.isfinite(zv):
                        raise ValueError("dz became a non-finite value")
                    dzt[p] = zv
            ...
            # da[t][j] = dy[t][j] + Σ_p dz[t][p]*W1[p][j]：乘积自 0.0 按 p
            # 升序累加，循环结束后再加 dy[t][j]。
            ...
            # dx[t][j] = da[t][j] + dxa[t][j]，残差两路逐元素相加。
            dx = [[0.0] * D for _ in range(T)]
            for t in range(T):
                dat = da[t]
                dxat = dxa[t]
    ...
    def forward_relative(self, x, bias, mask=None):
        """相对位置偏置 Transformer 块前向，返回 (y, w) 并缓存反向快照。

        x、mask 的契约完全沿用 forward；bias 须为 heads×(2R+1) 的 F 矩阵
        （R>=0），即行数恰为 heads、列数为正奇数，否则抛 ValueError（校验
        由 MHA.forward_relative 保证）。先取 (c, w) =
        attn.forward_relative(x, bias, mask)，再按 forward 次序求
        a = x + c、f = tanh(b1 + W1*a)、y = a + b2 + W2*f（W1/b1/W2/b2
        的校验与快照方式同 forward）。y 为 T×D、w 为 heads×T×T 的逐层新建
        float 列表并缓存，不修改或复用输入、bias 与参数。任一中间量非有限
        抛 ValueError；任何失败都清空本块缓存。实参数量错误沿用 Python
        自带的 TypeError。
        """
        D, P = self.D, self.P
        # 任何失败的 forward_relative 都使既有缓存失效。
        self._cache = None
        try:
            # x、bias、mask 的校验与 attn 自身缓存均由 MHA.forward_relative
            # 保证；c、w 为逐层新建的 float 列表。
            c, w = self.attn.forward_relative(x, bias, mask)
            T = len(c)
        ...
        self._cache = ("transformerblock_relative", T, D, P, a, f, W1, W2)
        return y, w

    def backward_relative(self, dy):
        """相对位置偏置 Transformer 块反向，返回十元组梯度。

        须紧随一次成功的 forward_relative（其后缓存未被失败清空），否则抛
        ValueError；缓存中的 a、f、W1、W2 须分别为 T×D、T×P、P×D、D×P 的
        F 矩阵（T、D、P 为非 bool 正 int），否则清空缓存并抛 ValueError；
        dy 须为与前向 y 同形的 T×D F 矩阵，否则抛 ValueError。
        按 backward 相同的公式与次序（求和均自 0.0 按下标升序累加）求
        dW2、db2、df、dz、dW1、db1 与 da，再调用
        attn.backward_relative(da) 得 (dxa, dWq, dWk, dWv, dWo, dbias)，令
            dx[t][j] = da[t][j] + dxa[t][j]
        返回 (dx, dWq, dWk, dWv, dWo, dW1, db1, dW2, db2, dbias)。dx 为
        T×D，四个注意力投影梯度为 D×D，dW1 为 P×D、db1 长 P，dW2 为 D×P、
        db2 长 D，dbias 为 heads×(2R+1)，均为逐层新建 float 列表，不修改
        dy、缓存或任何参数；成功时缓存保留，重复调用结果相同。任一中间
        量非有限或反向失败均抛 ValueError 并清空缓存。实参数量错误沿用
        Python 自带的 TypeError。
        """
        cache = self._cache
        try:
            if type(cache) is not tuple or len(cache) != 8 \
                    or cache[0] != "transformerblock_relative":
                raise ValueError(
                    "backward_relative requires a successful "
                    "forward_relative pass before it")
            (_, T, D, P, a, f, W1, W2) = cache
            # 缓存维度与 a、f、W1、W2 须仍为契约形状的 F 矩阵，否则缓存已
            # 被篡改或损坏：清空缓存并抛 ValueError。
            if type(T) is bool or type(T) is not int or T <= 0 \
                    or type(D) is bool or type(D) is not int or D <= 0 \
                    or type(P) is bool or type(P) is not int or P <= 0:
                raise ValueError("cached dimensions are invalid")
            _check_matrix(a, T, D, "cached a")
            _check_matrix(f, T, P, "cached f")
            _check_matrix(W1, P, D, "cached W1")
            _check_matrix(W2, D, P, "cached W2")
            dy = _check_matrix(dy, T, D, "dy")
            dy = [[float(z) for z in row] for row in dy]
            ...
            # 相对位置注意力子层（含其四个投影与 dbias）的反向沿用
            # MHA.backward_relative。
            dxa, dWq, dWk, dWv, dWo, dbias = \
                self.attn.backward_relative(da)
        ...
        return dx, dWq, dWk, dWv, dWo, dW1, db1, dW2, db2, dbias

    def forward_cross(self, qx, kvx, mask=None):
        """Transformer 块交叉注意力前向，返回 (y, w) 并缓存反向所需快照。

        qx、kvx、mask 的契约完全沿用 MHA.forward_cross：先取 (c, w) =
        attn.forward_cross(qx, kvx, mask)，再求 a = qx + c、
        f = tanh(b1 + W1*a)、y = a + b2 + W2*f（W1/b1/W2/b2 的校验与
        快照方式同 forward）。y 为 Tq×D、w 为 heads×Tq×Tk 的逐层新建
        float 列表，不修改或复用输入与参数。任一中间量非有限抛
        ValueError；任何失败都清空本块缓存。实参数量错误沿用 Python
        自带的 TypeError。
        """
        D, P = self.D, self.P
        # 任何失败的 forward_cross 都使既有缓存失效。
        self._cache = None
        try:
            # qx、kvx、mask 的校验与 attn 自身缓存均由 MHA.forward_cross
            # 保证；c、w 为逐层新建的 float 列表。
            c, w = self.attn.forward_cross(qx, kvx, mask)
            Tq = len(c)
            ...
            # a[t][j] = qx[t][j] + c[t][j]，逐元素新建 float。
            a = [[0.0] * D for _ in range(Tq)]
            for t in range(Tq):
                qxt = qx[t]
                ct = c[t]
                at = a[t]
        ...
        self._cache = ("transformerblock_cross", Tq, D, P, a, f, W1, W2)
        return y, w

    def backward_cross(self, dy):
        """Transformer 块交叉注意力反向，返回十元组梯度。

        须紧随一次成功的 forward_cross（其后缓存未被失败清空），否则抛
        ValueError；dy 须为与前向 y 同形的 Tq×D F 矩阵，否则抛
        ValueError。按 backward 相同的公式与次序（求和均自 0.0 按下标
        升序累加）求 dW2、db2、df、dz、dW1、db1 与 da，再调用
        attn.backward_cross(da) 得 (dqa, dkvx, dWq, dWk, dWv, dWo)，令
            dqx[t][j] = da[t][j] + dqa[t][j]
        返回 (dqx, dkvx, dWq, dWk, dWv, dWo, dW1, db1, dW2, db2)。dqx
        为 Tq×D、dkvx 为 Tk×D，四个注意力梯度为 D×D，dW1 为 P×D、db1
        长 P，dW2 为 D×P、db2 长 D，均为逐层新建 float 列表，不修改
        dy、缓存或任何参数；成功时缓存保留，重复调用结果相同。任一中间
        量非有限或反向失败均抛 ValueError 并清空缓存。实参数量错误沿用
        Python 自带的 TypeError。
        """
        ...
        try:
            if type(cache) is not tuple or len(cache) != 8 \
                    or cache[0] != "transformerblock_cross":
                raise ValueError(
                    "backward_cross requires a successful forward_cross "
                    "pass before it")
            (_, Tq, D, P, a, f, W1, W2) = cache
            dy = _check_matrix(dy, Tq, D, "dy")
            dy = [[float(z) for z in row] for row in dy]
            ...
            # 交叉注意力子层（含其四个投影）的反向沿用 MHA.backward_cross。
            dqa, dkvx, dWq, dWk, dWv, dWo = self.attn.backward_cross(da)

            # dqx[t][j] = da[t][j] + dqa[t][j]，残差两路逐元素相加。
            dqx = [[0.0] * D for _ in range(Tq)]
            for t in range(Tq):
                dat = da[t]
                dqat = dqa[t]
        ...
        return dqx, dkvx, dWq, dWk, dWv, dWo, dW1, db1, dW2, db2

    def forward_cross_relative(self, qx, kvx, qpos, kpos, bias, mask=None):
        """相对位置偏置 Transformer 块交叉注意力前向，返回 (y, w) 并缓存
        反向快照。

        qx、kvx、qpos、kpos、bias、mask 的契约完全沿用
        MHA.forward_cross_relative：先取 (c, w) =
        attn.forward_cross_relative(qx, kvx, qpos, kpos, bias, mask)，再按
        forward_cross 次序求 a = qx + c、f = tanh(b1 + W1*a)、
        y = a + b2 + W2*f（W1/b1/W2/b2 的校验与快照方式同 forward）。y 为
        Tq×D、w 为 heads×Tq×Tk 的逐层新建 float 列表，不修改或复用输入、
        bias 与参数。任一中间量非有限抛 ValueError；任何失败都清空本块缓存。
        实参数量错误沿用 Python 自带的 TypeError。
        """
        D, P = self.D, self.P
        # 任何失败的 forward_cross_relative 都使既有缓存失效。
        self._cache = None
        try:
            # qx、kvx、qpos、kpos、bias、mask 的校验与 attn 自身缓存均由
            # MHA.forward_cross_relative 保证；c、w 为逐层新建的 float 列表。
            c, w = self.attn.forward_cross_relative(
                qx, kvx, qpos, kpos, bias, mask)
            Tq = len(c)
        ...
        self._cache = ("transformerblock_cross_relative", Tq, D, P,
                       a, f, W1, W2)
        return y, w

    def backward_cross_relative(self, dy):
        """相对位置偏置 Transformer 块交叉注意力反向，返回十一元组梯度。

        须紧随一次成功的 forward_cross_relative（其后缓存未被失败清空），
        否则抛 ValueError；缓存中的 a、f、W1、W2 须分别为 T×D、T×P、
        P×D、D×P 的 F 矩阵（T、D、P 为非 bool 正 int），否则清空缓存并抛
        ValueError；dy 须为与前向 y 同形的 Tq×D F 矩阵，否则抛
        ValueError。按 backward_cross 相同的公式与次序（求和均自 0.0 按
        下标升序累加）求 dW2、db2、df、dz、dW1、db1 与 da，再调用
        attn.backward_cross_relative(da) 得
        (dqa, dkvx, dWq, dWk, dWv, dWo, dbias)，令
            dqx[t][j] = da[t][j] + dqa[t][j]
        返回 (dqx, dkvx, dWq, dWk, dWv, dWo, dW1, db1, dW2, db2, dbias)。
        dqx 为 Tq×D、dkvx 为 Tk×D，四个注意力投影梯度为 D×D，dW1 为
        P×D、db1 长 P，dW2 为 D×P、db2 长 D，dbias 为 heads×(2R+1)，均为
        逐层新建 float 列表，不修改 dy、缓存或任何参数；成功时缓存保留，
        重复调用结果相同。任一中间量非有限或反向失败均抛 ValueError 并
        清空缓存。实参数量错误沿用 Python 自带的 TypeError。
        """
        cache = self._cache
        try:
            if type(cache) is not tuple or len(cache) != 8 \
                    or cache[0] != "transformerblock_cross_relative":
                raise ValueError(
                    "backward_cross_relative requires a successful "
                    "forward_cross_relative pass before it")
            (_, Tq, D, P, a, f, W1, W2) = cache
            # 缓存维度与 a、f、W1、W2 须仍为契约形状的 F 矩阵，否则缓存已
            # 被篡改或损坏：清空缓存并抛 ValueError。
            if type(Tq) is bool or type(Tq) is not int or Tq <= 0 \
                    or type(D) is bool or type(D) is not int or D <= 0 \
                    or type(P) is bool or type(P) is not int or P <= 0:
                raise ValueError("cached dimensions are invalid")
            _check_matrix(a, Tq, D, "cached a")
            _check_matrix(f, Tq, P, "cached f")
            _check_matrix(W1, P, D, "cached W1")
            _check_matrix(W2, D, P, "cached W2")
            dy = _check_matrix(dy, Tq, D, "dy")
            dy = [[float(z) for z in row] for row in dy]
            ...
            # 相对位置交叉注意力子层（含其四个投影与 dbias）的反向沿用
            # MHA.backward_cross_relative。
            (dqa, dkvx, dWq, dWk, dWv, dWo,
             dbias) = self.attn.backward_cross_relative(da)
        ...
        return (dqx, dkvx, dWq, dWk, dWv, dWo, dW1, db1, dW2, db2, dbias)

    def forward_padded(self, xs, lengths, mask=None):
        """变长批 Transformer 块前向，返回 (y, w) 并保存各样本独立快照。

        xs 须为非空 B×T×D 的 F 嵌套列表（B、T 均正，各样本等长 T、每行等长
        D，padding 位同样须为 F）；lengths 须为 B 长 list，各项 type 为非
        bool 的 int 且 1<=值<=T；mask 须为 None 或 B×T×T 的嵌套 list，元素
        type 恰为 bool，padding 行、列仅校验后忽略；样本 b 令 L=lengths[b]，
        其有效 L×L 块每行至少一个 True（None 等价全 True），否则抛
        ...
        x=xs[b][:L] 与有效 L×L mask 块调用现有 forward 语义），其缓存连同
        注意力子层缓存原样存为该样本的独立快照，结果与逐样本单独 forward
        完全一致。y 为 B×T×D，w 为 B×heads×T×T：有效区写入各样本结果，
        padding 行（及 w 的 padding 列）均为 0.0。输出均为逐层新建的 float
        列表，不修改或复用输入与参数；任一中间量非有限抛 ValueError。实参
        数量错误沿用 Python 自带的 TypeError。
        """
        D, heads = self.D, self.heads
        # 任何失败的 forward_padded 都使既有缓存（含普通 forward 缓存）失效。
        ...
        try:
            ...
            for b in range(B):
                ...
                # 现有 forward 完成注意力子层与前馈子层的全部计算，并自行
                # 校验参数与有效块每行至少一个 True；成功后本块缓存连同
                # attn 子层缓存即该样本的独立快照。其内部失败已自行清缓存。
                yb, wb = self.forward(xb, block)
                ...
                for t in range(L):
                    for a in range(D):
                        ...
                        if not math.isfinite(yv):
                            raise ValueError(
                                "padded transformer output became non-finite")
                        yb_row[t][a] = yv
                    for h in range(heads):
                        wbht = wb[h][t]
                        wrow = wb_pad[h][t]
    ...
    def backward_padded(self, dy):
        ...
        逐样本以其独立快照（含注意力子层快照）调用现有 backward：dx 为
        B×T×D（有效前缀写入各样本 dx，padding 行为 D 个 0.0）；dWq、dWk、
        dWv、dWo 四个 D×D 梯度、dW1（P×D）、db1（P）、dW2（D×P）、db2（D）
        八组参数梯度均自 0.0 起按 b、行、列升序累加。所有结果均为逐层新建
        的 float 列表，不修改 dy、缓存或任何参数；成功时缓存保留，可重复
        调用且结果确定。任一中间量非有限或反向失败均抛 ValueError 并清空
        缓存。
        """
        D, P = self.D, self.P
        cache = self._cache
        try:
            ...
            # 反传期间临时改挂的注意力子层快照在结束后恢复，保证可重复。
            attn_cache_restore = self.attn._cache
            for b in range(B):
                L = lengths_c[b]
                dyb = [checked_dy[b][t] for t in range(L)]

                # 以该样本的独立快照调用现有 backward；失败时其内部已清空
                # 本块与子层缓存，由本方法外层 except 统一处理。
                block_snap, attn_snap = snaps[b]
                self._cache = block_snap
                self.attn._cache = attn_snap
                (dxb, gWq, gWk, gWv, gWo,
                ...
                # 注意力四组 D×D 梯度自 0.0 起按 b、行、列升序累加。
                for a in range(D):
                    dWqa, dWka, dWva, dWoa = (
                        dWq[a], dWk[a], dWv[a], dWo[a])
                    gWqa, gWka, gWva, gWoa = (
                ...
                # dW2/db2（D×P 与 D）自 0.0 起按 b、行、列升序累加。
                for j in range(D):
                    db2[j] += float(gb2[j])
                    if not math.isfinite(db2[j]):
                        raise ValueError(
                ...
                # dW1/db1（P×D 与 P）自 0.0 起按 b、行、列升序累加。
                for p in range(P):
                    db1[p] += float(gb1[p])
                    if not math.isfinite(db1[p]):
                        raise ValueError(
            ...
            # 恢复批缓存与子层缓存，保证可重复反向。
            self._cache = cache
            self.attn._cache = attn_cache_restore
        except ValueError:
            self._cache = None
            self.attn._cache = None
            raise
    ...
    def forward_cross_padded(self, qxs, kvxs, q_lengths, kv_lengths,
                             mask=None):
        """变长批 Transformer 块交叉注意力前向，返回 (y, w) 并保存各样本
        独立快照。

        qxs、kvxs 须为非空 B×Tq×D、B×Tk×D 的 F 嵌套列表（B、Tq、Tk 均正，
        两者 B 相同，各样本等长、每行等长 D，padding 位同样须为 F）；
        q_lengths、kv_lengths 须为 B 长 list，各项 type 为非 bool 的 int
        且分别在 [1, Tq]、[1, Tk]；mask 须为 None 或 B×Tq×Tk 的嵌套
        list，元素 type 恰为 bool，padding 行、列仅校验后忽略；样本 b 令
        Lq=q_lengths[b]、Lk=kv_lengths[b]，其有效 Lq×Lk 块每行至少一个
        ...
        样本按 b 升序、仅对有效前缀逐样本执行现有 forward_cross（即对
        qx=qxs[b][:Lq]、kvx=kvxs[b][:Lk] 与有效 Lq×Lk mask 块调用现有
        forward_cross 语义），其缓存连同注意力子层缓存原样存为该样本的
        独立快照，结果与逐样本单独 forward_cross 完全一致。y 为 B×Tq×D，
        w 为 B×heads×Tq×Tk：有效区写入各样本结果，padding 行（及 w 的
        padding 列）均为 0.0。输出均为逐层新建的 float 列表，不修改或复用
        输入与参数；任一中间量非有限抛 ValueError。实参数量错误沿用
        Python 自带的 TypeError。
        """
        D, heads = self.D, self.heads
        # 任何失败的 forward_cross_padded 都使既有缓存失效。
        ...
        try:
            ...
            for b in range(B):
                ...
                # 现有 forward_cross 完成注意力子层与前馈子层的全部计算，
                # 并自行校验参数与有效块每行至少一个 True；成功后本块缓存
                # 连同 attn 子层缓存即该样本的独立快照。其内部失败已自行
                # 清缓存。
                yb, wb = self.forward_cross(qxb, kvxb, block)
                snaps[b] = (self._cache, self.attn._cache)
                ...
                for t in range(Lq):
                    for a in range(D):
                        ...
                        if not math.isfinite(yv):
                            raise ValueError(
                                "padded transformer cross output became "
                                "non-finite")
                        yb_row[t][a] = yv
                    for h in range(heads):
                        wbht = wb[h][t]
        ...
        self._cache = ("transformerblock_cross_padded", B, Tq, Tk,
                       list(q_lengths), list(kv_lengths), snaps)
        return y, w

    def backward_cross_padded(self, dy):
        """变长批 Transformer 块交叉注意力反向，返回十元组梯度
        (dqxs, dkvxs, dWq, dWk, dWv, dWo, dW1, db1, dW2, db2)。
        ...
        逐样本以其独立快照（含注意力子层快照）调用现有 backward_cross：
        dqxs 为 B×Tq×D、dkvxs 为 B×Tk×D（有效前缀写入各样本结果，padding
        行为 D 个 0.0）；dWq、dWk、dWv、dWo 四个 D×D 梯度、dW1（P×D）、
        db1（P）、dW2（D×P）、db2（D）八组参数梯度均自 0.0 起按 b、行、
        列升序累加。所有结果均为逐层新建的 float 列表，不修改 dy、缓存或
        任何参数；成功时缓存保留，可重复调用且结果确定。任一中间量非有限
        ...
        try:
            if type(cache) is not tuple or len(cache) != 7 \
                    or cache[0] != "transformerblock_cross_padded":
                raise ValueError(
                    "backward_cross_padded requires a successful "
                    "forward_cross_padded pass before it")
            _, B, Tq, Tk, q_lengths_c, kv_lengths_c, snaps = cache
            ...
            for b in range(B):
                ...
                # 已清空本块与子层缓存，由本方法外层 except 统一处理。
                block_snap, attn_snap = snaps[b]
                self._cache = block_snap
                self.attn._cache = attn_snap
                (dqxb, dkvxb, gWq, gWk, gWv, gWo,
                 gW1, gb1, gW2, gb2) = self.backward_cross(dyb)
        ...
        return dqxs, dkvxs, dWq, dWk, dWv, dWo, dW1, db1, dW2, db2
...
_LSTM_MHA_RELATIVE_MODEL_KEYS = ["version", "vocab", "W", "b", "Wq", "Wk",
                                 "Wv", "Wo", "heads", "bias", "Why", "by",
                                 "h0", "c0"]
...
def _load_perplexity_lstm_mha_relative_model(path):
    """读取并校验 perplexity-lstm-mha-relative 模型文件，返回十三元组。

    文件须为 UTF-8 编码的 JSON 对象，顶层键恰为
    version、vocab、W、b、Wq、Wk、Wv、Wo、heads、bias、Why、by、h0、c0
    且按此顺序出现（重复或多余均非法）：version 的 type 恰为 int 且值为
    5；bias 为 heads×(2R+1) 的 F 矩阵，即行数恰为 heads、列数为正奇数；
    其余各键的契约同 perplexity-lstm-mha。任何读取、UTF-8、JSON 或校验
    失败均抛 ValueError（或 OSError）。
    """
    with open(path, "rb") as f:
        raw = f.read()
    ...
    if type(pairs) is not list \
            or len(pairs) != len(_LSTM_MHA_RELATIVE_MODEL_KEYS):
        raise ValueError("model must be a JSON object with exactly 14 keys")
    for pair, key in zip(pairs, _LSTM_MHA_RELATIVE_MODEL_KEYS):
        if type(pair) is not tuple or len(pair) != 2 or pair[0] != key:
            raise ValueError(
                "model keys must be exactly %r in order"
                % _LSTM_MHA_RELATIVE_MODEL_KEYS)
    model = dict(pairs)

    version = model["version"]
    if type(version) is bool or type(version) is not int or version != 5:
        raise ValueError("version must be exactly non-bool int 5, got %r"
                         % (version,))
    ...
    if type(first_brow) is not list or len(first_brow) == 0 \
            or len(first_brow) % 2 != 1:
        raise ValueError(
            "bias must have a positive odd number of columns, got %r"
            % (first_brow,))
    bias_width = len(first_brow)
    for brow in bias:
        if type(brow) is not list or len(brow) != bias_width:
            raise ValueError(
                "bias must be a list of shape %d×%d" % (heads, bias_width))
        for v in brow:
            if not _is_f(v):
                raise ValueError(
    ...
    return (vocab, W, b, projections["Wq"], projections["Wk"],
            projections["Wv"], projections["Wo"], heads, bias, Why, by,
            h0, c0)
...
def _load_perplexity_transformer_model(path):
    """读取并校验 perplexity-transformer 模型文件，返回解包后的十三元组。

    文件须为 UTF-8 编码的 JSON 对象，顶层键恰为
    version、vocab、heads、P、Wq、Wk、Wv、Wo、W1、b1、W2、b2、Why、by
    且按此顺序出现（重复或多余均非法）：version 的 type 恰为 int 且值为
    6；vocab 的契约与 perplexity-lstm 相同（非空列表，每项是恰含一个码点
    的 str，元素唯一且按码点严格升序）；heads、P 为非 bool 的正 int 且
    heads 整除 V=len(vocab)；其余十项为 F 列表，形状依次为 V×V、V×V、
    V×V、V×V、P×V、P、V×P、V、V×V、V。任何读取、UTF-8、JSON 或校验失败
    均抛 ValueError（或 OSError）。
    """
    with open(path, "rb") as f:
        raw = f.read()
    # 先按严格 UTF-8 解码，再交由 json 解析（object_pairs_hook 保留键序与
    ...
    if type(version) is not int or version != 6:
        raise ValueError("version must be exactly int 6, got %r" % (version,))
    ...
    def check_matrix(key, rows, cols):
        mat = model[key]
        if type(mat) is not list or len(mat) != rows:
            raise ValueError(
                "%s must be a list of shape %d×%d" % (key, rows, cols))
        for row in mat:
            if type(row) is not list or len(row) != cols:
                raise ValueError(
                    "%s must be a list of shape %d×%d" % (key, rows, cols))
    ...
    def check_vector(key, size):
        vec = model[key]
        if type(vec) is not list or len(vec) != size:
            raise ValueError("%s must be a list of length %d" % (key, size))
        for v in vec:
            if not _is_f(v):
                raise ValueError("%s entries must be finite numbers, got %r"
                                 % (key, v))
    ...
    Wq = check_matrix("Wq", V, V)
    Wk = check_matrix("Wk", V, V)
    Wv = check_matrix("Wv", V, V)
    Wo = check_matrix("Wo", V, V)
    W1 = check_matrix("W1", P, V)
    b1 = check_vector("b1", P)
    W2 = check_matrix("W2", V, P)
    b2 = check_vector("b2", V)
    Why = check_matrix("Why", V, V)
    by = check_vector("by", V)

    return vocab, heads, P, Wq, Wk, Wv, Wo, W1, b1, W2, b2, Why, by


def _perplexity_transformer(model_path, corpus_path):
    """计算单层 Transformer 块语言模型在给定语料上的困惑度，返回字符串。

    MODEL 为 version 6 的十四键严格 JSON（见
    _load_perplexity_transformer_model）；CORPUS 沿用 perplexity-lstm 的
    严格 UTF-8 全文码点、至少 2 码点及无表外字符契约。

    令 T=len(CORPUS)-1，x 为前 T 个字符的 T×V one-hot 矩阵，mask 为 T×T
    且 mask[i][j]=(j<=i)；将 Wq、Wk、Wv、Wo、W1、b1、W2、b2 装入
    TransformerBlock(V, heads, P) 后以 (x, mask) 前向取 y。t 升序以
    y[t] 替代 perplexity-lstm 中的 h，按相同的下标、float 偏置与升序累加
    规则计算 z_k = by_k + Σ_j Why_k,j*y[t][j]，并以减最大值的
    log-sum-exp 累加下一字符的负对数似然。任一中间量非有限（含最终
    exp(L/T) 溢出）均抛 ValueError。成功返回
    format(exp(L/T), '.17g') + '\\n'。
    """
...
def _train_transformer(model_path, corpus_path, out_path):
    ...
    MODEL、CORPUS 完全沿用 perplexity-transformer（十四键严格 JSON、
    version 恰为 int 6、F、形状、heads 整除 V 及语料严格 UTF-8、至少 2
    码点、无表外字符契约）。以该入口相同的前 T 个字符 T×V one-hot 与
    T×T 因果 mask（mask[i][j]=(j<=i)）调用装入 Wq、Wk、Wv、Wo、W1、b1、
    W2、b2 的 TransformerBlock(V, heads, P).forward 取 y。

    t 升序沿用 perplexity-transformer 的 Why/by 仿射下标、float 偏置与
    softmax 次序求 z、p，令 g=p-onehot(下一字符)：按 train-lstm-mha 的
    t/k/j 次序累加 dWhy += g⊗y、dby += g，并对每个 j 自 0.0 按 k 升序
    求 dy[t][j]=Σ_k Why_k,j*g_k。全部 t 完成后以 T×V 的 dy 调用
    block.backward 得 (dx, dWq, dWk, dWv, dWo, dW1, db1, dW2, db2)。

    梯度组序为 dWq、dWk、dWv、dWo、dW1、db1、dW2、db2、dWhy、dby，按
    组、行、列序自 0.0 累加平方和求全局范数，超过 5.0 即 scale=5.0/norm
    统一缩放，否则 scale=1.0；十组参数更新为 float(old)-0.1*(grad*scale)，
    任一梯度或更新结果非有限均抛 ValueError。

    OUT 复用 MODEL 十四键的顺序与形状（version、vocab、heads、P、Wq、Wk、
    Wv、Wo、W1、b1、W2、b2、Why、by），version、heads、P 不变，数组元素
    均转为 float；字节格式、负零保留与同目录临时文件原子替换均沿用
    train-lstm-mha。任一阶段失败都保持 OUT 原状态（或不存在）并清理临时
    文件。成功时 stdout 为空并返回 0。
    """
    (vocab, heads, P, Wq, Wk, Wv, Wo, W1, b1, W2, b2, Why, by) = \
        _load_perplexity_transformer_model(model_path)
    V = len(vocab)
    ...
    dby = [0.0] * V
    dy = [[0.0] * V for _ in range(T)]

    for t in range(T):
        h = y[t]
        target = ids[t + 1]

        # z_k = by_k + Σ_j Why_k,j*h_j，自 float 偏置起依 j 升序累加。
        z = [0.0] * V
        for k in range(V):
            acc = float(by[k])
            why_row = Why[k]
        ...
        # 按 t 升序累加 dWhy += g⊗h、dby += g（组内下标顺序同
        # train-lstm-mha）。
        for k in range(V):
            gk = g[k]
            dby[k] += gk
    ...
    # 一次反向覆盖全序列：TransformerBlock.backward 内部自 0.0 按 t、行、
    # 列升序累加八组参数梯度。
    (_dx, dWq, dWk, dWv, dWo, dW1, db1, dW2, db2) = block.backward(dy)

    # 依 dWq、dWk、dWv、dWo、dW1、db1、dW2、db2、dWhy、dby 的组、行、列
    # 序自 0.0 累加平方和求全局范数；任一梯度非有限即失败。
    ...
    # 十组参数减 0.1 倍（裁剪后的）梯度；version、vocab、heads、P 不变。
    # 结果非有限即失败。
    ...
    # 原子写出：在 OUT 同目录建临时文件，写全并关闭后以 os.replace 替
    # 换；失败时 OUT 保持原状态，临时文件尽量清理。
...
def _train_transformer_window(model_path, corpus_path, out_path,
                               window_text):
    """对 version 6 单层 Transformer 字符模型以截断上下文做一次全语料 SGD。

    MODEL、CORPUS、OUT 契约完全沿用 train-transformer（version 6 十四键
    严格 JSON、F、形状、heads 整除 V、严格 UTF-8 语料至少 2 码点且无表外
    字符、0.1 更新、5.0 全局裁剪、十四键顺序、数组转 float、紧凑 ASCII
    转义 JSON 加 LF、负零保留与同目录临时文件原子替换）；WINDOW 整串匹配
    [1-9][0-9]*（任意位数均合法，不先转无界整数），否则抛 ValueError。

    令 T=语料码点数-1。以十进制位数与同长字典序安全比较 WINDOW 与 T：
    WINDOW 数学值不小于 T 时截断窗口对任何一步都不裁剪可见上下文，直接沿
    用 train-transformer 的整序列前向与一次 backward，OUT 与其逐字节相同
    （超长 WINDOW 不先转 int）。

    WINDOW<T 时，t 升序以安全比较求 n=min(WINDOW,t+1)、s=t+1-n，以语料
    索引 [s:t+1] 构造 n×V one-hot 与 n×n 因果 mask（mask[i][j]=(j<=i)），
    将八组块参数装入同一个 TransformerBlock(V, heads, P) 后独立调用
    forward，仅取末行 y[n-1]（对应全局位置 t）预测下一字符；输出层
    Why/by 仿射、稳定 softmax、g=p-onehot(下一字符) 及 dWhy、dby 的
    t/k/j 累加次序完全沿用 train-transformer。建 n×V 零 dy，仅末行按
    j、k 升序自 0.0 写 Whyᵀg，调用 block.backward，并将八组块梯度
    dWq、dWk、dWv、dWo、dW1、db1、dW2、db2 按 t、组、行、列升序自 0.0
    累加。每个 t 仅以窗口内序列前后向，不传播或复用任何窗外状态。

    十组梯度的全局范数、5.0 裁剪、0.1 更新、非有限即失败、OUT 十四键与
    原子写出协议均沿用 train-transformer。任一阶段失败都保持 OUT 原状
    态（或不存在）并清理临时文件。成功时 stdout 为空并返回 0。
    """
    if not _WINDOW_RE.match(window_text):
        raise ValueError("WINDOW must match [1-9][0-9]*")
    ...
    # WINDOW>=T 时截断不裁剪任何一步的可见上下文：直接走整序列入口，OUT
    # 与 train-transformer 逐字节相同；_safe_window_min 以位数与同长字典
    # 序比较，超长 WINDOW 不触发无界整数转换。
    if _safe_window_min(window_text, T) == T:
        return _train_transformer(model_path, corpus_path, out_path)
    ...
    # 十组累加器均自 0.0 新建；前八组为块梯度，后两组为输出层梯度。
    dWq = [[0.0] * V for _ in range(V)]
    dWk = [[0.0] * V for _ in range(V)]
    dWv = [[0.0] * V for _ in range(V)]
    dWo = [[0.0] * V for _ in range(V)]
    dW1 = [[0.0] * V for _ in range(P)]
    db1 = [0.0] * P
    dW2 = [[0.0] * P for _ in range(V)]
    db2 = [0.0] * V
    dWhy = [[0.0] * V for _ in range(V)]
    dby = [0.0] * V
    ...
    for t in range(T):
        ...
        # 安全求 n=min(WINDOW,t+1)、s=t+1-n（此处 WINDOW<T，其位数不大
        # 于 str(T)，转换同样有界）。
        n = _safe_window_min(window_text, t + 1)
        s = t + 1 - n
        ...
        # n×V 零 dy，仅末行写 Whyᵀg：每个 j 独立以 0.0 起按 k 升序累加。
        dy = [[0.0] * V for _ in range(n)]
        dy_last = dy[n - 1]
        for j in range(V):
            acc = 0.0
            for k in range(V):
        ...
        # 本窗口独立反向；八组块梯度按组、行、列序累加入全局（t 升序由
        # 外层循环保证），dx 丢弃。
        (_dx, b_dWq, b_dWk, b_dWv, b_dWo, b_dW1, b_db1, b_dW2,
         b_db2) = block.backward(dy)
        block_groups = (b_dWq, b_dWk, b_dWv, b_dWo, b_dW1, b_db1,
                        b_dW2, b_db2)
        acc_groups = (dWq, dWk, dWv, dWo, dW1, db1, dW2, db2)
        for dst, src in zip(acc_groups, block_groups):
            if type(dst[0]) is list:
                for r in range(len(dst)):
                    dst_row = dst[r]
                    src_row = src[r]
                    for c in range(len(dst_row)):
                        dst_row[c] += src_row[c]
                        if not math.isfinite(dst_row[c]):
                            raise ValueError(
                                "block gradient accumulated to a "
                                "non-finite value")
            else:
                for r in range(len(dst)):
                    dst[r] += src[r]
                    if not math.isfinite(dst[r]):
                        raise ValueError(
                            "block gradient accumulated to a "
                            "non-finite value")
    ...
    # 换；失败时 OUT 保持原状态，临时文件尽量清理。
    out_dir = os.path.dirname(os.path.abspath(out_path))
    tmp_fd, tmp_path = tempfile.mkstemp(prefix=".tmp-train-transformer-window-",
                                        dir=out_dir)
    replaced = False
    try:
        try:
...
def _sample_transformer_window(model_path, start, seed_text,
                               temperature_text, length_text, window_text):
    """有限上下文版本的 sample-transformer，返回待写出的字符串。

    MODEL、START、SEED、TEMPERATURE、LENGTH 的词法与校验、模型十四键装
    载、装参 TransformerBlock、唯一随机源、Why/by logit、温度缩放、稳定
    softmax、词表升序阈值抽样与有限性失败契约均逐项沿用
    _sample_transformer；不写任何文件。
    ...
    并仅取末行；每步重新截断、独立前向，不复用任何窗外状态。Why/by、温
    度 softmax 与 k 升序阈值抽样的下标与累加次序与 _sample_transformer
    完全一致（见 _transformer_sample_steps）；选中字符追加到输出、其索引
    追加到完整 prefix。

    当 WINDOW 每步都不小于 prefix 长度（数学值不小于 LENGTH+1）时，各步
    末行与完整 prefix 前向的对应行逐元素相同（注意力对每一行只依赖该行及
    其之前的行，FFN 逐行独立），故输出与同参 sample-transformer 逐字节
    相同。LENGTH=0 时不执行任何前向，仅返回 LF。任一中间量非有限即失败。
    """
    if not _WINDOW_RE.match(window_text):
        raise ValueError("WINDOW must match [1-9][0-9]*")
...
def _sample_transformer_anneal(model_path, start, seed_text, start_t_text,
    ...
    除温度外，MODEL、START、SEED、LENGTH 的校验，以及 prefix 的 one-hot
    输入、因果掩码、TransformerBlock.forward、Why/by 仿射、稳定 softmax、
    词表升序阈值抽样与 prefix 状态更新均沿用 _sample_transformer。START_T、
    END_T 各经 float() 解析，结果须有限且严格大于 0，否则抛 ValueError。
    整次调用仅初始化一次 r=random.Random(int(SEED))，不写任何文件。

    LENGTH 为 0 时不求温度、不执行前向，仅输出 LF；为 1 时仅用 START_T；
    否则 t 自 0 升序，第 t 步温度严格按 Python 表达式
    START_T+(END_T-START_T)*t/(LENGTH-1) 求值，结果非有限或不大于 0 即抛
    ValueError。以该温度替换固定温度，其余运算次序与 _sample_transformer
    完全一致；随机源与 prefix 跨步连续。任一中间量非有限均抛 ValueError。
...
def _sample_transformer_window_anneal(model_path, start, seed_text,
                                      ...
                                      window_text):
    """有限上下文加线性退火版本的 sample-transformer，返回待写出的字符串。

    除窗口外，MODEL、START、SEED、START_T、END_T、LENGTH 的词法与校验、
    模型十四键装载、装参 TransformerBlock、唯一随机源、Why/by logit、线性
    温度退火（含 LENGTH 的 0/1 语义与 START_T+(END_T-START_T)*t/
    (LENGTH-1) 的严格求值次序）、稳定 softmax、词表升序阈值抽样、prefix
    状态更新与有限性失败契约均逐项沿用 _sample_transformer_anneal；不写
    任何文件。
    ...
    安全求 n=min(WINDOW,len(prefix))，仅以末尾 n 项构造 n×V one-hot 与
    n×n 因果 mask（mask[i][j]=(j<=i)），调用装参 TransformerBlock.forward
    并仅取末行；每步重新截断、独立前向，不复用任何窗外状态。Why/by、温度
    softmax 与 k 升序阈值抽样的下标与累加次序与 _sample_transformer_anneal
    完全一致；选中字符追加到输出、其索引追加到完整 prefix。

    当 WINDOW 每步都不小于 prefix 长度（数学值不小于 LENGTH+1）时，各步
    末行与完整 prefix 前向的对应行逐元素相同（注意力对每一行只依赖该行及
    其之前的行，FFN 逐行独立），故输出与同参 sample-transformer-anneal
    逐字节相同。LENGTH=0 时不求温度、不执行任何前向，仅返回 LF。任一中间
    量非有限即失败。
    """
    if not _WINDOW_RE.match(window_text):
        raise ValueError("WINDOW must match [1-9][0-9]*")
...
def _sample_transformer_top_p(model_path, start, seed_text, start_t_text,
    ...
    除 TOP_P 与核选样外，MODEL、START、SEED、START_T、END_T、LENGTH 的校
    验，以及 LENGTH 的 0/1 语义、prefix 的 one-hot 输入、因果 mask、
    TransformerBlock.forward、Why/by 仿射、线性温度退火、稳定 softmax、唯一
    随机源、prefix 状态更新与有限性失败契约均沿用 _sample_transformer_anneal
    ；不写任何文件。TOP_P 经 float() 解析，结果须有限且 0<TOP_P<=1，否则抛
    ValueError。
    ...
    个累计值严格大于 u 的索引；无则取前缀末项。选中字符追加到输出，其索引
    追加到 prefix。任一新增运算非有限均抛 ValueError。成功返回 LENGTH 个码
    点再加一个 LF。
    """
    (vocab, heads, P, Wq, Wk, Wv, Wo, W1, b1, W2, b2, Why, by) = \
        _load_perplexity_transformer_model(model_path)
...
def _sample_transformer_top_k(model_path, start, seed_text, start_t_text,
    ...
    除 TOP_K 与选样外，MODEL、START、SEED、START_T、END_T、LENGTH 的校验，
    以及 LENGTH 的 0/1 语义、prefix 的 one-hot 输入、因果 mask、
    TransformerBlock.forward、Why/by 仿射、线性温度退火、唯一随机源、prefix
    状态更新与有限性失败契约均沿用 _sample_transformer_anneal；不写任何文件
    。TOP_K 整串匹配 [1-9][0-9]*，且数学值 K 不超过 V=len(vocab)，否则抛
    ValueError。

    TOP_K 先按十进制位数及同长度字典序与 V 的十进制文本比较：位数更长、或
    同位数且字典序更大即越界失败；仅比较通过后才转 int，任意位数文本都不
    触发整数转换异常。

    每步先按原顺序求 a_k=z_k/T、m=max(a)、e_k=exp(a_k-m)，d 自 0.0 依 k
    升序累加。再将索引按 (-e_k, k) 升序排列（e 降序、并列时 k 升序），候
    选恰为该序前 K 项；s 自 0.0 按候选序累加 e。令 u=r.random()*s，再按候
    选顺序自 0.0 累加 e，选首个累计值严格大于 u 的索引；无则取候选末项。
    选中字符追加到输出，其索引追加到 prefix。任一新增运算非有限均抛
...
def _sample_transformer_top_k_top_p_run(model_path, start, seed_text,
                                        ...
                                        top_k_text, top_p_text, length_text):
    """sample-transformer-top-k-top-p 与 -scored 共享的采样核心。

    校验、one-hot 输入、因果 mask、TransformerBlock.forward、Why/by 仿射、
    线性温度退火、top-k 截断后 top-p 前缀截取、随机源初始化与消费、选索引
    规则、prefix 状态更新及有限性失败契约均与
    _sample_transformer_top_k_top_p 文档一致。返回 (text, logprobs,
    total)：text 为生成字符串（不含尾随 LF）；logprobs 为每步选中索引 k
    对应的 a[k]-m-log(s)（s 为最终前缀质量）；total 自 0.0 按 t 升序累加
    各 lp。任一 lp 或 total 非有限均抛 ValueError。

    除 TOP_K、TOP_P 及下述选样外，MODEL、START、SEED、START_T、END_T、
    LENGTH 的校验，以及 LENGTH 的 0/1 语义、稳定 softmax、线性温度退火、
    唯一随机源与有限性失败契约均沿用 _sample_transformer_top_k；不写任何
    文件。TOP_K 整串匹配 [1-9][0-9]*，且数学值 K 不超过 V=len(vocab)：
    先按十进制位数及同长度字典序与 str(V) 比较，越界即失败，通过后才转
    ...
    末项累计恰为 sK>=target，前缀必然存在）。令 u=r.random()*s，按前缀
    顺序自 0.0 累加 e，选首个累计值严格大于 u 的索引；无则取前缀末项。
    选中字符追加到输出，其索引追加到 prefix。任一新增运算非有限均抛
    ValueError。
    """
    (vocab, heads, P, Wq, Wk, Wv, Wo, W1, b1, W2, b2, Why, by) = \
    ...
    for t in range(length):
        ...
        # a_k=z_k/T，m=max(a)，e_k=exp(a_k-m)，均按 k 升序依原次序求值。
        a = [0.0] * V
        m = None
        for k in range(V):
            ak = z[k] / temperature
        ...
        # 按候选序自 0.0 累加 e，保留首个使累计值 >=target 的最短前缀；s 为
        # 其累计和。与 sK 同序累加，末项累计恰为 sK>=target，前缀必然存在。
        chosen_prefix = []
        s = 0.0
        for idx in candidates:
            chosen_prefix.append(idx)
...
def _sample_transformer_top_k_top_p_scored(model_path, start, seed_text,
    ...
    全部校验、Transformer 前向、温度退火、top-k 截断后 top-p 前缀截取、
    随机源初始化与消费、选索引规则及错误协议均沿用
...
def _beam_transformer(model_path, start, start_t_text, end_t_text,
    ...
    # 七个模型矩阵（Wq、Wk、Wv、Wo、W1、W2、Why）的外层容器类型须恰为
    # list（拒绝 tuple 等非 list 序列），否则失败；合法模型的矩阵经装载器
    # 本就为 list，故此校验不改变任何合法行为。
    for _mat, _name in ((Wq, "Wq"), (Wk, "Wk"), (Wv, "Wv"), (Wo, "Wo"),
                        (W1, "W1"), (W2, "W2"), (Why, "Why")):
        if type(_mat) is not list:
            raise ValueError("%s outer container must be a list" % _name)
...
def _perplexity_lstm_mha_relative(model_path, corpus_path, window_text):
    """LSTM 加相对位置偏置多头注意力的字符困惑度，返回待写出的字符串。

    MODEL 的顶层键恰为
    version、vocab、W、b、Wq、Wk、Wv、Wo、heads、bias、Why、by、h0、c0
    且按此顺序出现，version 恰为非 bool int 5，bias 为 heads×(2R+1) 的
    F 矩阵（列数为正奇数），其余各键契约同 perplexity-lstm-mha；CORPUS、
    WINDOW 完全沿用 perplexity-lstm-mha 的契约。

    置 h=h0、c=c0、memory=[h0]、pos=[0]、L=0.0、T=len(CORPUS)-1，t 升序：
    以当前字符的 V 长 one-hot 为 x，调用装入 W、b 的 LSTMCell.forward
    更新 h、c；M 取 memory 末尾至多 WINDOW 项（顺序从旧到新），P 取 pos
    的同一切片，以装入四组投影构造的 MHA(H, heads) 调用
    forward_cross_relative([h], M, [t+1], P, bias, None)，取首项 ctx，
    按 i 升序令 u[i] = h[i] + ctx[0][i]。logit 仅以 u 替代
    perplexity-lstm-mha 中的 h，其余 Why/by 仿射、稳定 log-sum-exp 及下一
    字符负对数似然的下标与累加顺序均与之相同；随后向 memory 追加 h 的
    float 副本、向 pos 追加 t+1。任一运算非有限（含最终 exp(L/T) 溢出）
    均抛 ValueError。成功返回 format(exp(L/T), '.17g') + '\\n'。
    """
    if not _WINDOW_RE.match(window_text):
        raise ValueError("WINDOW must match [1-9][0-9]*")
    ...
    # 记忆序列 [h0, h_0, ..., h_{t-1}] 与对应位置 [0, 1, ..., t]；h0 保留
    # 模型原值（F 允许 int），forward_cross_relative 会先复制再投影。
    memory = [h0]
    pos = [0]
    h = list(h0)
    c = list(c0)
...
def _mha_relative_bucket_stats(model_path, corpus_path, window_text):
    """perplexity-lstm-mha-relative 各头每步相对距离桶质量，返回字符串。

    MODEL、CORPUS、WINDOW 的校验、每步 LSTM 推进、memory/pos 尾窗截取与
    mha.forward_cross_relative([h], M, [t+1], P, bias, None) 调用完全沿用
    _perplexity_lstm_mha_relative，取其权重 w（heads×1×len(M)）但不计算
    Why、by 输出层与负对数似然。令 R=(len(bias[0])-1)//2、
    T=len(CORPUS)-1；每步每头 r 置 2R+1 个 0.0，按 j 升序令
    b=max(-R,min(R,P[j]-(t+1)))+R 并累加 mass[b]+=w[r][0][j]；总量按
    t、r、b 升序从 0.0 累加，任一累加或 总量/T 非有限均抛 ValueError。
    构造 JSON 对象：顶层键序恰为 version,radius,steps,items,mean；
    version、radius、steps 为 int 1、R、T；items 按 t 升序，每项为
    [t, rows]，rows 按 r 升序，每项为 [r, masses]；mean 按 r 升序，每项
    为 [r, masses]；两处 masses 均按 b 升序，为单步质量或 总量/T 的
    format(x,'.17g') 字符串列表。返回
    json.dumps(obj,ensure_ascii=True,separators=(',',':'),
    allow_nan=False)+'\\n'，不写文件。
    """
    if not _WINDOW_RE.match(window_text):
    ...
    c = list(c0)
    T = len(ids) - 1
    totals = [[0.0] * width for _ in range(heads)]
    items = []
    for t in range(T):
        x = [0.0] * V
        x[ids[t]] = 1.0
        ...
        # 第 t 步第 r 头：mass 自 2R+1 个 0.0 起按 j 升序把 w[r][0][j] 累入
        # 相对距离桶 b=max(-R,min(R,P[j]-(t+1)))+R；随后按 b 升序把单步质量
        # 累入总量（整体即按 t、r、b 升序自 0.0 累加）。
        rows = []
        for r in range(heads):
            wr = w[r][0]
            masses = [0.0] * width
            for j in range(n):
                ...
                masses[bj] += wr[j]
                if not math.isfinite(masses[bj]):
                    raise ValueError(
            ...
            for bj in range(width):
                ...
                if not math.isfinite(tr[bj]):
                    raise ValueError(
                        "total bucket mass accumulated to a non-finite value")
            rows.append([r, [format(v, ".17g") for v in masses]])
        items.append([t, rows])
...
def _sample_lstm_mha_relative(model_path, start, seed_text,
                              temperature_text, length_text, window_text):
    """带相对位置偏置多头交叉注意力从 version 5 的模型采样 LENGTH 个码点。

    MODEL、WINDOW 的读取、词法、十四键、bias、F、形状与安全截取完全沿用
    perplexity-lstm-mha-relative；START、SEED、TEMPERATURE、LENGTH 的校
    验、唯一随机源、温度 softmax、词表升序阈值抽样、输出及错误协议均沿
    用 sample-lstm-mha。整次调用仅初始化一次
    r=random.Random(int(SEED))，不写任何文件。

    置 h=h0、c=c0、x=START 索引、memory=[h0]、pos=[0]。第 t 步（t 自 0
    升序）循环 LENGTH 次：以 x 的 V 长 one-hot 调用装入 W、b 的
    LSTMCell.forward(x, h, c)，取前两项更新 h、c；M 取 memory 末尾
    min(WINDOW, len(memory)) 项（顺序从旧到新），P 取 pos 的同一切片，
    以装入 Wq、Wk、Wv、Wo 构造的 MHA(H, heads) 调用
    forward_cross_relative([h], M, [t+1], P, bias, None)，取首项 ctx，
    按 i 升序令 u[i] = h[i] + ctx[0][i]。logit 仅以 u 替代
    sample-lstm-mha 中的 u，温度缩放、稳定 softmax、随机阈值、k 升序累
    计与选中规则逐项沿用；追加 vocab[k]，令 x=k，再向 memory 追加 h 的
    float 副本、向 pos 追加 t+1。任一中间量非有限均抛 ValueError。成功
...
def _resume_getstate_from_json(rs):
    """校验状态中的 r 节点并还原为 random.Random.setstate 所需三元组。

    须为长度 3 的列表：首项为非 bool int 且恰为 3；次项恰含 625 个非 bool
    int，其中前 624 个为 Mersenne Twister 字（[0,2^32)），末个为下标
    （[0,624]）——与 getstate 的合法输出逐值一致；末项为 None。结构、类型
    或取值不符均抛 ValueError，返回 (3, tuple(625 个 int), None)。
    """
    if type(rs) is not list or len(rs) != 3:
        raise ValueError("resume r must be a list of length 3")
    version, words, gauss = rs
    if (type(version) is bool or type(version) is not int
            or version != 3):
        raise ValueError("resume r version must be non-bool int 3")
    if type(words) is not list or len(words) != 625:
        raise ValueError(
            "resume r internal state must list exactly 625 ints")
    for i, w in enumerate(words):
        if type(w) is bool or type(w) is not int:
            raise ValueError("resume r words must be non-bool ints")
        # getstate 的前 624 项为无符号 32 位 MT 字，末项为下标 [0,624]。
        if i == 624:
            in_range = 0 <= w <= 624
        else:
            in_range = 0 <= w < (1 << 32)
        if not in_range:
            raise ValueError("resume r word %d out of range" % i)
    if gauss is not None:
        raise ValueError("resume r gauss slot must be null")
    return (3, tuple(words), None)


def _load_lstm_mha_resume_state(path, sha_hex, expected_args, vocab_size, H):
    """读取并严格校验 sample-lstm-mha-resume 状态文件。

    文件须为 UTF-8 编码的 JSON 对象，顶层键依次且仅为
    v、sha、args、c、x、m、r（重复或多余均非法）：v 为非 bool int 且值
    为 1；sha 为 MODEL 字节的小写 SHA-256 且逐字符相等；args 为 4 个原文
    str 组成的数组且恰等于 [START,SEED,TEMPERATURE,WINDOW]；c 为 H 长
    float 列表且每项有限；x 为 [0,V) 内的非 bool int；m 为非空、行宽 H
    的 float 矩阵（旧到新，末行即当前 h）且每项有限；r 符合
    _resume_getstate_from_json 的结构，并能通过 random.Random.setstate
    还原。任何读取、UTF-8、JSON、类型、形状、有限性、摘要、args 或随机
    状态校验失败均抛 ValueError。文件不存在时返回 None；其余 OSError
    原样传播。成功返回 (c, x, m, rng)，其中 rng 已 setstate 还原。
    """
    try:
        with open(path, "rb") as f:
            raw = f.read()
    ...
    pairs = json.loads(text, object_pairs_hook=list)
    if (type(pairs) is not list
            or len(pairs) != len(_RESUME_STATE_KEYS)):
        raise ValueError(
            "resume state must be a JSON object with exactly 7 keys")
    for pair, key in zip(pairs, _RESUME_STATE_KEYS):
        if type(pair) is not tuple or len(pair) != 2 or pair[0] != key:
            raise ValueError(
                "resume state keys must be exactly %r in order"
                % (_RESUME_STATE_KEYS,))
    ...
    if (type(args) is not list or len(args) != 4
            ...
            or args != expected_args):
        raise ValueError(
            "resume args must equal [START,SEED,TEMPERATURE,WINDOW]")

    c = st["c"]
    if type(c) is not list or len(c) != H:
        raise ValueError("resume c must be a list of length %d" % H)
    for v in c:
        if type(v) is not float or not math.isfinite(v):
            raise ValueError(
                "resume c entries must be finite floats, got %r" % (v,))

    x = st["x"]
    if (type(x) is bool or type(x) is not int
            or not 0 <= x < vocab_size):
        raise ValueError("resume x must be a non-bool int in [0,V)")

    m = st["m"]
    if type(m) is not list or len(m) == 0:
        raise ValueError("resume m must be a non-empty list of rows")
    for row in m:
        if type(row) is not list or len(row) != H:
            raise ValueError("resume m rows must each list %d floats" % H)
        for v in row:
            if type(v) is not float or not math.isfinite(v):
                raise ValueError(
                    "resume m entries must be finite floats, got %r" % (v,))
...
def _tuples_to_lists(obj):
    """递归将 tuple 转为 list（其余对象原样返回），用于序列化 getstate。"""
...
def _atomic_replace_bytes(path, payload):
    """将 payload 写入 path 同目录临时文件后以 os.replace 原子替换。

    临时文件写全并 flush、fsync 后才 replace；replace 前任何异常都会删除
    临时文件（含 KeyboardInterrupt 等 BaseException），原 path 字节在替换
    成功前保持不变。
    """
    directory = os.path.dirname(os.path.abspath(path))
    fd, tmp_path = tempfile.mkstemp(prefix=".seqmodel-state-", dir=directory)
    try:
...
def _sample_lstm_mha_resume(model_path, start, seed_text, temperature_text,
    ...
    """sample-lstm-mha 的可续跑采样：生成 LENGTH 步并原子写出续跑状态。

    MODEL、START、SEED、TEMPERATURE、LENGTH、WINDOW 的词法与校验、模型
    十三键装载、LSTM/MHA 状态推进、Why/by logit、温度缩放、稳定
    softmax、词表升序阈值抽样、唯一随机源及有限性失败契约均沿用
    sample-lstm-mha（模型参数直接装入、不经随机初始化）；STATE 为状态
    文件路径。

    STATE 缺失时置 h=h0、c=c0、x=START 索引、memory=[h0]、
    rng=random.Random(int(SEED))（h0、c0 各值转为 float）；STATE 存在时
    读取 UTF-8 JSON，顶层键依次且仅为 v,sha,args,c,x,m,r：v 为 int 1；
    sha 为 MODEL 字节的小写 SHA-256；args 为原文 str 数组
    [START,SEED,TEMPERATURE,WINDOW]；c 为 H 长有限 float 列表；x 为
    [0,V) 内非 bool int；m 为旧到新的非空 H 列有限 float 矩阵，末行即
    当前 h；r 为 getstate 递归将 tuple 转 list 所得
    [3,[625 个非 bool int],null]。类型、形状、有限性、sha 摘要、args 或
    随机状态不符均抛 ValueError。

    生成 LENGTH 步后，状态以
    json.dumps(ensure_ascii=True,separators=(',',':'),allow_nan=False)
    序列化为 ASCII 转义、紧凑、禁非有限数且保留 -0.0 的 JSON，编码为
    UTF-8 字节并加 LF；先写全 STATE 同目录临时文件并 os.replace 原子替换
    后，stdout 才写本段生成文本加 LF。LENGTH 为 0 时仅创建（STATE 缺失）
    或校验（STATE 存在）状态，stdout 仅为 LF。先跑 p 步再续跑 q 步的文本
    拼接与终态字节，须与一次跑 p+q 步逐字节相同。
    """
    if not _WINDOW_RE.match(window_text):
        raise ValueError("WINDOW must match [1-9][0-9]*")
    ...
    restored = _load_lstm_mha_resume_state(
        state_path, sha_hex, expected_args, V, H)
    if restored is None:
        # 全新初态：对 random.Random 的唯一一次构造即 random.Random(seed)。
        rng = random.Random(seed)
        ...
        c, x, memory, rng = restored
        # m 末行即当前 h；各行在采样中只读不写，可直接引用。
        h = [float(v) for v in memory[-1]]
    ...
    # ensure_ascii 保证非 ASCII 以 \\uXXXX 转义输出纯 ASCII 文本；allow_nan
    # 禁止 NaN/Infinity；紧凑分隔；float -0.0 由序列化器保留为 "-0.0"。
    payload = (json.dumps(state_obj, ensure_ascii=True,
                          separators=(",", ":"), allow_nan=False)
               .encode("utf-8") + b"\n")
    # 状态原子落盘成功后，调用方才写 stdout。
...
def _sample_lstm_mha_relative_anneal(model_path, start, seed_text,
    ...
    的 LSTM 推进、相对位置 MHA 窗口、Why/by logit、稳定 softmax、词表升序
    阈值抽样、状态更新与有限性检查均完全沿用 sample-lstm-mha-relative；
    START_T、END_T 的解析与逐步退火温度均沿用 sample-lstm-mha-anneal；
    模型装载不经随机初始化，整次调用仅初始化一次
    r=random.Random(int(SEED))，不写任何文件。
    ...
    每步以该温度替换 sample-lstm-mha-relative 的固定温度，h、c、x、
    memory、pos 与随机源跨步连续。任一中间量非有限均抛 ValueError。成功
    返回 LENGTH 个码点再加一个 LF。
    """
    if not _WINDOW_RE.match(window_text):
        raise ValueError("WINDOW must match [1-9][0-9]*")
...
def _sample_lstm_mha_relative_topp(model_path, start, seed_text,
                                   start_t_text, end_t_text, top_p_text,
                                   length_text, window_text):
    """以线性退火温度与 top-p（核）抽样、带相对位置偏置多头交叉注意力从
    version 5 的 LSTM-MHA 模型采样。

    除 TOP_P 与核选样外，MODEL、START、SEED、START_T、END_T、LENGTH、
    WINDOW 的校验、version 5 模型的 LSTM 推进、相对位置 MHA 窗口、Why/by
    logit、稳定 softmax、线性温度退火、唯一随机源与有限性失败契约均完全沿
    用 sample-lstm-mha-relative-anneal；模型装载不经随机初始化，整次调用
    仅初始化一次 r=random.Random(int(SEED))，不写任何文件。TOP_P 经
    ...
    升序），依该序自 0.0 累加 e，保留首个使累计值 >=TOP_P*d 的最短前缀；
    s 为该前缀按该序自 0.0 累加所得之和。令 u=random()*s，再按前缀顺序自
    0.0 累加 e，选首个累计值严格大于 u 的索引；无则取前缀末项。其字符追加
    到输出并作为下一输入 x，随后向 memory 追加 h 的 float 副本、向 pos 追
    加 t+1。任一新增运算非有限均抛 ValueError。LENGTH 为 0 时不计算温度；
    为 1 时仅用 START_T。成功返回 LENGTH 个码点再加一个 LF。
    """
    if not _WINDOW_RE.match(window_text):
        raise ValueError("WINDOW must match [1-9][0-9]*")
...
def _sample_lstm_mha_relative_topk(model_path, start, seed_text,
                                   start_t_text, end_t_text, top_k_text,
                                   length_text, window_text):
    """以线性退火温度与 top-k 抽样、带相对位置偏置多头交叉注意力从
    version 5 的 LSTM-MHA 模型采样。

    除 TOP_K 与选样外，MODEL、START、SEED、START_T、END_T、LENGTH、
    WINDOW 的校验、version 5 模型的 LSTM 推进、相对位置 MHA 窗口、Why/by
    logit、稳定 softmax、线性温度退火、唯一随机源与有限性失败契约均完全沿
    用 sample-lstm-mha-relative-anneal；模型装载不经随机初始化，整次调用
    仅初始化一次 r=random.Random(int(SEED))，不写任何文件。
    ...
    每步先按原顺序求温度缩放后的 a[k]、m=max(a)、e[k]=exp(a[k]-m)，d 自
    0.0 依 k 升序累加。再将索引按 (-e[k], k) 升序排列（e 降序、并列时 k
    升序），候选恰为该序前 K 项；s 自 0.0 按候选序累加 e。令
    u=random()*s，再从 0.0 依候选序累加 e，选择首个累计值严格大于 u 的
    索引；无则取候选末项。以该字符更新 x 并向 memory 追加 h 的 float 副
    本、向 pos 追加 t+1。任一新增运算非有限均抛 ValueError。LENGTH 为 0
    时不计算温度且仅输出 LF；为 1 时仅用 START_T。成功返回 LENGTH 个码
    点再加一个 LF。
    """
    if not _WINDOW_RE.match(window_text):
        raise ValueError("WINDOW must match [1-9][0-9]*")
...
def _sample_lstm_mha_topp_run(model_path, start, seed_text, start_t_text,
    ...
    失败契约均完全沿用 sample-lstm-mha-anneal；模型装载不经随机初始化，整
    次调用仅初始化一次 r=random.Random(int(SEED))，不写任何文件。TOP_P 经
    float() 解析，结果须有限且 0<TOP_P<=1，否则抛 ValueError。
    ...
    rng = random.Random(seed)
    # 与 sample-lstm-mha-anneal 相同：h0 保留模型原值，forward_cross 会先
...
def _sample_lstm_mha_topk(model_path, start, seed_text, start_t_text,
                          end_t_text, top_k_text, length_text, window_text):
    """以线性退火温度与 top-k 抽样从 version 4 的 LSTM-MHA 模型采样。

    除 TOP_K 与选样外，MODEL、START、SEED、LENGTH、WINDOW 的校验、LSTM/MHA
    状态推进、Why/by logit、稳定 softmax、线性温度退火、唯一随机源与有限性
    失败契约均完全沿用 sample-lstm-mha-top-p；模型装载不经随机初始化，整
    次调用仅初始化一次 r=random.Random(int(SEED))，不写任何文件。
    ...
    u=random()*s，再从 0.0 依候选序累加 e，选择首个累计值严格大于 u 的
    索引；无则取候选末项。以该字符更新 x 并向 memory 追加 h 的 float 副
    本。任一新增运算非有限均抛 ValueError。LENGTH 为 0 时不计算温度且仅
    输出 LF；为 1 时仅用 START_T。成功返回 LENGTH 个码点再加一个 LF。
    """
    if not _WINDOW_RE.match(window_text):
        raise ValueError("WINDOW must match [1-9][0-9]*")
    ...
    rng = random.Random(seed)
    # 与 sample-lstm-mha-top-p 相同：h0 保留模型原值，forward_cross 会先
    # 复制再投影；各步 h 本就是 float。
    memory = [h0]
    h = list(h0)
    c = list(c0)
...
def _sample_lstm_mha_topk_topp(model_path, start, seed_text, start_t_text,
                               end_t_text, top_k_text, top_p_text,
                               length_text, window_text):
    """以线性退火温度与 top-k 后 top-p 抽样从 version 4 的 LSTM-MHA 模型采样。

    除 TOP_P 及筛选外，MODEL、START、SEED、LENGTH、WINDOW 的校验、LSTM/MHA
    状态推进、Why/by logit、稳定 softmax、线性温度退火、唯一随机源与有限性
    失败契约均完全沿用 sample-lstm-mha-top-k；模型装载不经随机初始化，整
    次调用仅初始化一次 r=random.Random(int(SEED))，不写任何文件。
    ...
    每步先按原顺序求温度缩放后的 a[k]、m=max(a)、e[k]=exp(a[k]-m)，d 自
    0.0 依 k 升序累加。再将索引按 (-e[k], k) 升序排列（e 降序、并列时 k
    升序）取前 K 项；sK 自 0.0 按该序累加 e，令 target=TOP_P*sK，再从
    0.0 按该序累加 e，保留首个使累计值 >=target 的最短前缀，s 为该前缀
    累计和。令 u=random()*s，再按前缀序从 0.0 累加 e，选首个累计值严格
    大于 u 的索引；无则取前缀末项。其字符追加到输出并作为下一输入 x，随
    后向 memory 追加 h 的 float 副本。任一新增乘加或抽样中间量非有限均
    抛 ValueError。LENGTH 为 0 时不计算温度且仅输出 LF；为 1 时仅用
    START_T。成功返回 LENGTH 个码点再加一个 LF。
    """
    if not _WINDOW_RE.match(window_text):
        raise ValueError("WINDOW must match [1-9][0-9]*")
    ...
    for t in range(length):
        ...
        # 索引按 (-e_k, k) 升序：e 降序、并列时 k 升序；候选取前 K 项。
        order = sorted(range(V), key=lambda k: (-e[k], k))
        candidates = order[:top_k]
...
def _score_lstm_mha(model_path, start, text, temperature_text, window_text):
    """确定性评分指定候选 TEXT（LSTM-MHA），返回待写出的字符串。

    MODEL、START、TEMPERATURE、WINDOW 的读取、词法、十三键、F、形状及失败
    契约均沿用 sample-lstm-mha；TEXT 为可空 Unicode 字符串且每个码点须在
    vocab，否则抛 ValueError。本命令无 SEED、无随机源且不写文件。

    置 h=h0、c=c0、x=START 索引、memory=[h0]、total=0.0。按 t 升序遍历
    TEXT：以 x 的 V 长 one-hot 调用装入 W、b 的 LSTMCell.forward 更新
    h、c；M 取 memory 末尾至多 WINDOW 项（顺序从旧到新），装入 Wq、Wk、
    Wv、Wo 构造 MHA(H, heads)，以 forward_cross([h], M, None) 返回首项
    ctx，按 i 升序令 u[i]=h[i]+ctx[0][i]；按原 Why/by 次序求 z，再计算
    a[k]=z[k]/TEMPERATURE、m=max(a)、e[k]=exp(a[k]-m)，d 自 0.0 依 k
    升序累加 e[k]；不抽样，令 y 为 TEXT[t] 索引，
    lp=a[y]-m-math.log(d)，total 按 t 升序累加 lp，置 x=y，memory 追加
    h 的 float 副本。任一计算非有限均抛 ValueError。成功返回
...
def _score_lstm_mha_relative_batch(model_path, start, candidates_path,
                                   temperature_text, window_text):
    """确定性评分 CANDIDATES 文件中的多个候选串（相对位置偏置 LSTM-MHA）。

    MODEL、START、TEMPERATURE、WINDOW 的读取、词法、十四键、bias、F、形状
    与失败契约均沿用 score-lstm-mha-relative。CANDIDATES 为严格 UTF-8 的
    JSON 文件，顶层须为非空数组；每个元素 type 恰为 str（可为空串），且
    每个码点须在 vocab，重复元素按原序保留，否则抛 ValueError。本命令无
    SEED、无随机源且不写文件。

    逐个候选独立评分：每个候选均重置 h=h0、c=c0、x=START 索引、
    memory=[h0]、pos=[0]、total=0.0，候选间不共享任何状态；逐步 one-hot
    推进、pos 切片与 forward_cross_relative、u、Why/by logit、
    a[k]=z[k]/TEMPERATURE、m=max(a)、d 自 0.0 依 k 升序累加
    exp(a[k]-m)、lp=a[y]-m-math.log(d)、total 累加与 memory、pos 更新均
    沿用 score-lstm-mha-relative，任一计算非有限均抛 ValueError；未格式
    化 total 须与单独调用原入口相同。

    成功返回单个 JSON 对象加 LF，键序 items,best。items 按 CANDIDATES
    原序，每项恰为 [text,steps,format(total,'.17g')]：text 为候选原文
    str（空串为 ""），steps 为码点数 int；best 为最高未格式化 total 的
    int 下标，按 float 比较，平分时取较小下标。序列化恰用
    json.dumps(obj,ensure_ascii=True,separators=(',',':'),
    allow_nan=False)+'\\n' 的 UTF-8 字节。
    """
    if not _WINDOW_RE.match(window_text):
    ...
    for ci, ids in enumerate(candidates_ids):
        # 每个候选独立重置 h0、c0、START、memory、pos 与 total，候选间不
        # 共享状态。
        memory = [h0]
        pos = [0]
        h = list(h0)
        ...
        for t in range(len(ids)):
            ...
            # logit 仅以 u 替代 h；下标与累加顺序同 _score_lstm_mha。
            z = _output_logits(Why, by, u)
        ...
        # best 为最高未格式化 total 的下标；仅严格大于才更新，平分保留小
        # 下标。
...
def _beam_lstm_mha(model_path, start, start_t_text, end_t_text, beam_text,
                   length_text, window_text):
    """以线性退火温度、带投影多头交叉注意力从 version 4 的 LSTM-MHA 模型
    做确定性束搜索。

    除 BEAM 与确定性选束外，MODEL、START、LENGTH、WINDOW 的校验、0/1 长度
    语义、version 4 LSTM/MHA 状态推进、Why/by logit、稳定 softmax、线性温
    度退火与有限性失败契约均沿用 sample-lstm-mha-anneal；WINDOW 词法与安
    全截取沿用 perplexity-lstm-mha。本命令无 SEED、不构造任何随机源且不
    写任何文件。

    BEAM 整串匹配 [1-9][0-9]*（任意位数均合法），否则抛 ValueError；每轮
    仅当其数学值小于候选数时转 int（此时其位数不超过候选数十进制位数，
    不触发整数文本位数上限），否则保留全部候选，超长文本不得触发转换异常。

    初始束为 (0.0, "", h0, c0, START 索引, [h0], ())。第 t 轮逐束按既有
    温度推进 h、c，M 取 memory 末尾至多 WINDOW 项（从旧到新），以装入四
    组投影的 MHA 的 forward_cross([h], M, None) 首行 ctx 与 h 逐项求和得
    u，经 Why/by 求缩放 logit a、m=max(a)、e[k]=exp(a[k]-m)，d 自 0.0
    按 k 升序累加；每个 k 生成子束：分数加 a[k]-m-log(d)，文本追加
    vocab[k]，置 x=k，索引元组追加 k，memory 追加 h 的 float 副本。全部
    子束按 (-分数, 生成索引元组) 升序排列，保留前 min(BEAM, 候选数) 项；
    同轮等长，使用原始累计分数，长度归一化因子固定为 1。任一新增运算非
    有限均抛 ValueError。LENGTH 为 0 时仅输出 LF，否则输出最终首束文本
    加一个 LF。相同输入输出逐字节相同。
    """
    if not _WINDOW_RE.match(window_text):
        raise ValueError("WINDOW must match [1-9][0-9]*")
    ...
    # 模型 W、b 与四组投影直接装入，不经随机初始化；本入口不构造任何
    # random.Random。
    cell = _lstm_cell_loaded(V, H, W, b)
    mha = _mha_loaded(H, heads, Wq, Wk, Wv, Wo)

    # 初始束 (0.0, "", h0, c0, START 索引, [h0], ())；h0 保留模型原值，
    # forward_cross 与 LSTMCell.forward 均不修改其输入，故可直接共享。末
    # 项为生成索引元组（仅用于排序的确定性决胜）。
    beams = [(0.0, "", h0, c0, vocab.index(start), [h0], ())]

    for t in range(length):
        temperature = temperature_at(t)
        children = []
        for score, text, h, c, x, memory, base_idx in beams:
            # 当前字符的 V 长 one-hot 输入，推进 LSTM 隐状态与细胞状态。
            xvec = [0.0] * V
            xvec[x] = 1.0
            nh, nc = cell.forward(xvec, h, c)[:2]
        ...
        # 全部子束按 (-分数, 生成索引元组) 升序；同轮等长，使用原始累计
        # 分数（长度归一化因子固定为 1）。
        children.sort(key=lambda beam: (-beam[0], beam[6]))
...
def _train_lstm_mha_relative(model_path, corpus_path, out_path,
                             window_text):
    """相对位置偏置多头交叉注意力 LSTM 模型的一次全语料 SGD 更新并写 OUT。

    MODEL、CORPUS、WINDOW 的校验与前向严格沿用
    perplexity-lstm-mha-relative（version 5 十四键、bias 为
    heads×(2R+1)、pos 序列与 forward_cross_relative 调用），并逐步缓存
    LSTM cache 与各次 forward_cross_relative 的独立快照；反向、0.1
    学习率、5.0 全局裁剪、序列化、原子写盘及成败协议均沿用
    train-lstm-mha，仅投影梯度之外多一组 dbias。

    输出层以 u 替代 train-lstm 的 h：令 g_t = p_t-onehot(y_t)，按 t 升序
    累加 dWhy += g_t⊗u_t、dby += g_t，并对每个 j 自 0.0 按 k 升序求
    du_t = Whyᵀg_t。置 dhs 为全零，按 t 降序把该步保存的缓存装回同一
    MHA 后调用 backward_cross_relative([du_t])，取 dqx、dkvx、四组投影
    梯度与 dbias 步梯度：先将 du_t+dqx[0] 按 i 升序加至 dhs[t]；dkvx 按
    M_t 从旧到新映射回完整记忆 [h0, h_0, ..., h_{t-1}]——首行若为 h0
    （q=0）梯度丢弃，第 q（q>=1）行加至 dhs[q-1]；dWq、dWk、dWv、dWo
    与 dbias 自 0.0 起按 t 降序、行、列升序累加。任一累加非有限抛
    ValueError。随后调用 LSTMCell.backward_sequence(dhs, caches) 取得
    dW、db。

    梯度组序为 dW、db、dWq、dWk、dWv、dWo、dbias、dWhy、dby，依行序
    累加平方和求全局范数，超过 5.0 即统一缩放至 5.0；平方和溢出时取
    a=max|g|，同序自 0.0 累加 q+=(g/a)**2，以 (g/a)*(5.0/sqrt(q)) 为
    裁剪梯度。九组参数各减去 0.1 倍（裁剪后的）梯度，heads、h0、c0 不
    变。任一梯度或更新结果非有限均抛 ValueError。

    OUT 顶层键依次且仅为 version、vocab、W、b、Wq、Wk、Wv、Wo、heads、
    bias、Why、by、h0、c0（version 恰为 int 5，heads 仍为非 bool
    int），数值数组元素均转为 float；字节格式与 train-lstm-mha 相同
    （紧凑 ASCII 转义 JSON 加 LF，负零保留为 -0.0）。先在 OUT 同目录建
    临时文件，写全并关闭后以 os.replace 原子替换 OUT；任一阶段失败都
    保持 OUT 原状态并清理该临时文件。成功时 stdout 为空并返回 0。
    """
    if not _WINDOW_RE.match(window_text):
        raise ValueError("WINDOW must match [1-9][0-9]*")
    ...
    # 前向严格复用 perplexity-lstm-mha-relative：t 升序以 (h0, c0) 为
    # 初态逐步推进 h、c，缓存每步 cache 与该次相对交叉注意力的独立
    # 快照；M_t 为 memory 末尾至多 WINDOW 项，P 为 pos 的同一切片，
    # u_t = h + 首行上下文，随后向 memory 追加 h 的 float 副本、向
    # pos 追加 t+1。
    caches = []
    m_list = []
    u_list = []
    ...
    for t in range(T):
        ...
        # logit：仅以 u 替代 h，其余与 train-lstm 相同，自 float 偏置起
        # 依 j 升序累加。
        z = [0.0] * V
        for k in range(V):
            acc = float(by[k])
        ...
        for k in range(V):
            ...
            d += ev
            if not math.isfinite(d):
                raise ValueError("softmax denominator accumulated "
                                 "non-finitely")
        ...
        for k in range(V):
            ...
            for j in range(H):
                dw_row[j] += gk * u[j]
                if not math.isfinite(dw_row[j]):
                    raise ValueError("dWhy accumulated to a non-finite "
                                     "value")
    ...
    # 相对交叉注意力残差与记忆的反向：dhs 置零，按 t 降序把各步独立
    # 快照装回同一 MHA 调用 backward_cross_relative，并按行、列序累加
    # 四组投影梯度与 dbias。
    dhs = [[0.0] * H for _ in range(T)]
    dWq = [[0.0] * H for _ in range(H)]
    dWk = [[0.0] * H for _ in range(H)]
    dWv = [[0.0] * H for _ in range(H)]
    dWo = [[0.0] * H for _ in range(H)]
    dbias = [[0.0] * bias_width for _ in range(heads)]
    for t in range(T - 1, -1, -1):
        mha._cache = snaps[t]
        (dqx, dkvx, gWq, gWk, gWv, gWo,
         gdbias) = mha.backward_cross_relative([du_list[t]])
        ...
        # dbias 自 0.0 起按 t 降序、行 r、列 j 升序累加。
        for r in range(heads):
            db_row = dbias[r]
            gb_row = gdbias[r]
            for j in range(bias_width):
                db_row[j] += gb_row[j]
                if not math.isfinite(db_row[j]):
                    raise ValueError("dbias accumulated to a non-finite "
                                     "value")

    _dxs, _dh0, _dc0, dW, db = cell.backward_sequence(dhs, caches)

    # 依 dW、db、dWq、dWk、dWv、dWo、dbias、dWhy、dby 行序累加平方和求
    # 全局范数；任一梯度非有限即失败。平方和溢出（梯度本身均有限）时不
    # 再误报失败：取 a=max|g|，同序自 0.0 累加 q+=(g/a)**2，以
    # (g/a)*(5.0/sqrt(q)) 为裁剪梯度；未溢出路径与原先逐字节一致。
    groups = (dW, db, dWq, dWk, dWv, dWo, dbias, dWhy, dby)
    sum_sq = 0.0
    max_abs = 0.0
    overflowed = False
    for group in groups:
        rows = group if type(group[0]) is list else (group,)
        for row in rows:
            for v in row:
                if not math.isfinite(v):
                    raise ValueError("gradient is non-finite")
    ...
    if overflowed:
        # 同序自 0.0 累加 q+=(g/a)**2；max_abs>0（否则平方和不会溢出），
        # 每项 (g/a)**2<=1，q 必有限。
        q = 0.0
        for group in groups:
            rows = group if type(group[0]) is list else (group,)
            for row in rows:
                for v in row:
                    r = v / max_abs
                    q += r * r
        rescale = 5.0 / math.sqrt(q)

        def _clip(grad):
            return (grad / max_abs) * rescale
    else:
        global_norm = math.sqrt(sum_sq)
        scale = 5.0 / global_norm if global_norm > 5.0 else 1.0
    ...
    # 即失败。
    def _updated(old, grad):
        value = float(old) - 0.1 * _clip(grad)
        if not math.isfinite(value):
            raise ValueError("updated parameter became non-finite")
        return value
    ...
    new_Wo = [[_updated(Wo[a][j], dWo[a][j]) for j in range(H)]
              for a in range(H)]
    new_bias = [[_updated(bias[r][j], dbias[r][j])
                 for j in range(bias_width)] for r in range(heads)]
    new_Why = [[_updated(Why[k][j], dWhy[k][j]) for j in range(H)]
               for k in range(V)]
    new_by = [_updated(by[k], dby[k]) for k in range(V)]
    ...
    tmp_fd, tmp_path = tempfile.mkstemp(
        prefix=".tmp-train-lstm-mha-relative-", dir=out_dir)
    replaced = False
    try:
        try:
            f = os.fdopen(tmp_fd, "wb")
...
def main(argv):
    ...
    python seqmodel.py sample-transformer-window MODEL START SEED
    TEMPERATURE LENGTH WINDOW：除 WINDOW 与每步截断前向外，MODEL、START、
    SEED、TEMPERATURE、LENGTH 的词法与校验、唯一随机源、Why/by 仿射、温度
    softmax、词表升序阈值抽样、选中字符索引追加 prefix 及有限性失败契约均
    沿用 sample-transformer，不写文件。WINDOW 整串匹配 [1-9][0-9]*（任意
    位数均合法，不先转无界整数）。置 prefix=[START 索引]，每步以十进制位
    数和同长字典序安全求 n=min(WINDOW,len(prefix))，仅以末尾 n 项构造
    n×V one-hot 与局部因果 mask（mask[i][j]=(j<=i)），装参调用
    TransformerBlock.forward 仅取末行；每步独立截断，不复用任何窗外状态。
    WINDOW 数学值每步不小于 prefix 长度时与同参 sample-transformer 逐字节
    相同。LENGTH=0 不执行前向，stdout 仅 LF。输出契约与 sample 相同。
    ...
    因果 mask、TransformerBlock.forward、Why/by 仿射、稳定 softmax、词表
    升序阈值抽样与 prefix 状态更新均沿用 sample-transformer；START_T、
    END_T 各经 float() 解析，须有限且严格大于 0；整次仅初始化一次
    random.Random(int(SEED))。LENGTH 为 0 时不求温度、不执行前向，仅输出
    LF；为 1 时仅用 START_T，否则 t 自 0 升序，第 t 步温度严格按
    START_T+(END_T-START_T)*t/(LENGTH-1) 求值且每步须有限并大于 0；以该
    ...
    前向、Why/by 仿射、线性温度、稳定 softmax、唯一随机源、prefix 状态更新
    与有限性错误协议均沿用 sample-transformer-anneal。TOP_P 经 float() 解
    析，须有限且 0<TOP_P<=1，否则失败。每步先按原顺序求 a、m 与
    e_k=exp(a_k-m)，d 从 0.0 按 k 升序累加；索引按 (-e_k,k) 升序，从 0.0
    依序累加 e，保留首个使累计值 >=TOP_P*d 的最短前缀，s 为其累计和；令
    u=random()*s，按前缀序从 0.0 累加 e，选首个累计值严格大于 u 的索引，
    无则取末项；字符与索引追加到输出和 prefix。任一新增运算非有限即失败。
    输出契约与 sample 相同，不写文件。

    python seqmodel.py sample-transformer-top-k MODEL START SEED START_T
    END_T TOP_K LENGTH：除 TOP_K 与选样外，MODEL、START、SEED、START_T、
    END_T、LENGTH 的校验，以及 LENGTH 的 0/1 语义、Transformer 前向、
    Why/by 仿射、线性温度、稳定 softmax、唯一随机源、prefix 状态更新与有
    限性错误协议均沿用 sample-transformer-anneal。TOP_K 整串匹配
    [1-9][0-9]* 且数学值 K<=V=len(vocab)；先按十进制位数及同长字典序与 V
    比较，越界失败，通过后才转 int，超长文本不得触发整数转换异常。每步先
    按原顺序求 a_k、m 与 e_k=exp(a_k-m)，d 自 0.0 按 k 升序累加；索引按
    (-e_k,k) 升序，候选为前 K 项，s 从 0.0 按候选序累加。令
    u=random()*s，再依序累加 e，选首个累计值严格大于 u 的索引，无则取候
    选末项；字符及索引追加到输出和 prefix；新增运算非有限即失败。输出契约
    与 sample 相同，不写文件。

    python seqmodel.py sample-transformer-top-k-top-p MODEL START SEED
    START_T END_T TOP_K TOP_P LENGTH：除 TOP_P 与选样外，全部校验、
    LENGTH 的 0/1 语义、Transformer 前向、线性温度退火、唯一随机源、
    prefix 状态更新及有限性错误协议均沿用 sample-transformer-top-k。TOP_K
    整串匹配 [1-9][0-9]* 且数学值 K<=V，先按十进制位数及同长字典序与 V
    比较，通过后才转 int。TOP_P 经 float() 解析，须有限且 0<TOP_P<=1。
    每步沿原次序求 a_k、m=max(a)、e_k=exp(a_k-m)；按 (-e_k,k) 升序取前
    K 项，sK 从 0.0 依序累加 e，令 target=TOP_P*sK；再从 0.0 按候选序累
    加 e，保留首次使累计值 >=target 的最短前缀，s 为其累计和。令
    u=random()*s，按前缀序从 0.0 累加 e，选首个累计值严格大于 u 的索引，
    无则取前缀末项；以该字符更新输出和 prefix。新增运算非有限即失败。输
    出契约与 sample 相同，不写文件。

    python seqmodel.py sample-transformer-top-k-top-p-scored MODEL START
    SEED START_T END_T TOP_K TOP_P LENGTH：全部校验、Transformer 前向、
    温度退火、top-k 后 top-p 截取、随机消费及错误协议均沿用
    sample-transformer-top-k-top-p；同参须消费相同随机序列并生成与原入
    ...
    python seqmodel.py sample-lstm-mha-relative MODEL START SEED TEMPERATURE
    LENGTH WINDOW：MODEL、WINDOW 沿用 perplexity-lstm-mha-relative
    （version 5 十四键、bias、pos 序列与 forward_cross_relative），其余
    参数校验、唯一随机源、温度 softmax、词表升序阈值抽样、输出及错误协议
    均沿用 sample-lstm-mha。置 h=h0、c=c0、x=START 索引、memory=[h0]、
    pos=[0]，第 t 步推进 LSTM 后，M 取 memory 末尾至多 WINDOW 项（从旧到
    新），P 取 pos 的同一切片，调用 forward_cross_relative([h], M,
    [t+1], P, bias, None)，以 h 与首行上下文之和 u 经 Why/by 求 logit 并
    按原规则抽样，随后向 memory 追加 h 的 float 副本、向 pos 追加 t+1；
    模型参数直接装入，不经随机初始化，整次仅构造一次
    ...
    唯一随机源 random.Random(int(SEED)) 初始化；STATE 存在时读取 UTF-8
    JSON，顶层键依次且仅为 v,sha,args,c,x,m,r：v 为 int 1，sha 为 MODEL
    字节的小写 SHA-256，args 为原文 str 数组 [START,SEED,TEMPERATURE,
    WINDOW]，c 为 H 长有限 float 列表，x 为 [0,V) 内非 bool int，m 为旧
    到新的非空 H 列有限 float 矩阵（末行即当前 h），r 为 getstate 递归将
    tuple 转 list 所得 [3,625 个非 bool int,null]。类型、形状、有限性、
    摘要、args 或随机状态不符均失败。生成 LENGTH 步后状态写为 ASCII 转义、
    禁非有限数、保留 -0.0 的紧凑 JSON UTF-8 字节加 LF；先写全 STATE 同目录
    临时文件并 os.replace 后，stdout 才写本段文本加 LF。分段 p、q 步的文本
    与终态须与一次 p+q 步逐字节相同；LENGTH=0 创建或校验状态且 stdout 仅
    LF。失败均返回 2、stdout 空、stderr 恰为 "error\\n"，STATE 保持原字节
    或不存在，并清理临时文件。
    ...
    注意力从 version 5 的模型采样。除温度外，MODEL、START、SEED、LENGTH、
    WINDOW 的校验及 LSTM 推进、相对位置 MHA 窗口、Why/by logit、稳定
    softmax、词表升序阈值抽样、状态更新与有限性检查均沿用
    sample-lstm-mha-relative；START_T、END_T 各经 float() 解析，须有限
    且严格大于 0；整次仅构造一次 random.Random(int(SEED))。LENGTH 为 0
    时不求温度且仅输出 LF，为 1 时仅用 START_T，否则第 t 步温度严格按
    START_T+(END_T-START_T)*t/(LENGTH-1) 求值且每步须有限并大于 0，以
    该温度替换固定温度；h、c、x、memory、pos 与随机源跨步连续；输出契约
    与 sample 相同，不写文件。

    python seqmodel.py sample-lstm-mha-relative-top-p MODEL START SEED
    START_T END_T TOP_P LENGTH WINDOW：以线性退火温度与 top-p（核）抽样、
    带相对位置偏置多头交叉注意力从 version 5 的模型采样。除 TOP_P 与核选
    样外，模型与参数校验、version 5 的 LSTM/相对位置 MHA 状态推进、
    Why/by logit、稳定 softmax、线性温度、唯一随机源、pos 窗口追加与有
    限性失败契约均沿用 sample-lstm-mha-relative-anneal；TOP_P 经
    float() 解析，须有限且 0<TOP_P<=1。每步求 a[k]、m=max(a)、
    e[k]=exp(a[k]-m) 与 d（自 0.0 按 k 升序累加），索引按 (-e[k],k) 升
    序，依序累加 e 并保留首个使累计值 >=TOP_P*d 的最短前缀，s 为其和；
    u=random()*s，按前缀序累加 e 选首个累计值严格大于 u 的索引，无则取
    末项，更新 x 并追加 h 副本与 t+1 到 pos。LENGTH 为 0 时不求温度且仅
    输出 LF，为 1 时仅用 START_T。Unicode 按 UTF-8 输出，不写文件。

    python seqmodel.py sample-lstm-mha-relative-top-k MODEL START SEED
    START_T END_T TOP_K LENGTH WINDOW：以线性退火温度与 top-k 抽样、带
    相对位置偏置多头交叉注意力从 version 5 的模型采样。除 TOP_K 与选样
    外，模型与参数校验、LENGTH 为 0/1 的处理、退火、LSTM/相对位置 MHA
    状态推进及唯一随机源均沿用 sample-lstm-mha-relative-anneal。TOP_K
    整串匹配 [1-9][0-9]* 且数学值 K<=V，先按十进制位数及同长字典序与 V
    比较，越界失败，通过后才转 int。每步按原顺序求 a[k]、m=max(a)、
    e[k]=exp(a[k]-m)，索引按 (-e[k],k) 升序取前 K 项，s 自 0.0 依序累
    加；u=random()*s，再依序累加 e，选首个累计值严格大于 u 的索引，无
    则取末项，随后沿用原状态更新；任一新增运算非有限即失败。成功时
    stdout 恰为 LENGTH 个码点的 UTF-8 编码再加一个 LF，stderr 为空，
    不写文件。
    ...
    模型与参数校验、LSTM/MHA 状态推进、Why/by logit、稳定 softmax、线性
    温度、唯一随机源与有限性失败契约均沿用 sample-lstm-mha-anneal；TOP_P
    经 float() 解析，须有限且 0<TOP_P<=1。每步求 a[k]、m=max(a)、
    e[k]=exp(a[k]-m) 与 d（自 0.0 按 k 升序累加），索引按 (-e[k],k) 升
    序，依序累加 e 并保留首个使累计值 >=TOP_P*d 的最短前缀，s 为其和；
    u=random()*s，按前缀序累加 e 选首个累计值严格大于 u 的索引，无则取
    ...
    型与参数校验、LSTM/MHA 状态推进、Why/by logit、稳定 softmax、线性温
    度、唯一随机源与有限性失败契约均沿用 sample-lstm-mha-top-p；TOP_K
    整串匹配 [1-9][0-9]* 且数学值 K<=V，先按十进制位数及同长字典序与 V
    比较，越界失败，通过后才转 int。每步求 a[k]、m=max(a)、
    e[k]=exp(a[k]-m)，索引按 (-e[k],k) 升序，候选取前 K 项，s 自 0.0
    按候选序累加；u=random()*s，再从 0.0 依序累加 e，选首个累计值严格
    大于 u 的索引，无则取末项，更新 x 并追加 h 副本。LENGTH 为 0 时不
    ...
    再做 top-p（核）抽样、带投影多头交叉注意力从 version 4 的 LSTM-MHA
    模型采样。除 TOP_P 及筛选外，模型与参数校验、LSTM/MHA 状态推进、
    Why/by logit、稳定 softmax、线性温度、权重、唯一随机源与有限性失败
    契约均沿用 sample-lstm-mha-top-k；TOP_K 整串匹配 [1-9][0-9]* 且数学
    值 K<=V，先按十进制位数及同长字典序与 V 比较，通过后才转 int；TOP_P
    经 float() 解析，须有限且 0<TOP_P<=1。每步求 a[k]、m=max(a)、
    e[k]=exp(a[k]-m) 与 d（自 0.0 按 k 升序累加），索引按 (-e[k],k) 升序
    取前 K 项，sK 自 0.0 依序累加，令 target=TOP_P*sK，再自 0.0 累加 e
    保留首个使累计值 >=target 的最短前缀，s 为其和；u=random()*s，按前
    缀序累加 e 选首个累计值严格大于 u 的索引，无则取末项，更新 x 并追加
    h 副本。LENGTH 为 0 时不求温度且仅输出 LF，为 1 时仅用 START_T。
    Unicode 按 UTF-8 输出，不写文件。
    ...
    否则失败。无 SEED、无随机源，不写文件。置 h=h0、c=c0、x=START 索引、
    memory=[h0]、total=0.0，按 t 升序遍历 TEXT：每步以 x 的 one-hot 推进
    h、c，M 取 memory 末尾至多 WINDOW 项（从旧到新），以装入四组投影的
    MHA 的 forward_cross([h], M, None) 首行 ctx 与 h 逐项求和得 u，经
    Why/by 得 z，按原顺序计算 a[k]=z[k]/TEMPERATURE、m=max(a)、
    e[k]=exp(a[k]-m)，d 自 0.0 依 k 升序累加 e[k]；不抽样，令 y 为
    TEXT[t] 索引，lp=a[y]-m-math.log(d)，total 按 t 升序累加 lp，置 x=y，
    memory 追加 h 的 float 副本。任一计算非有限即失败。成功时 stdout 恰为
    ...
    python seqmodel.py score-lstm-mha-relative-batch MODEL START CANDIDATES
    TEMPERATURE WINDOW：确定性评分 CANDIDATES 文件中的多个候选串。MODEL、
    START、TEMPERATURE、WINDOW 与逐步计算、失败契约均沿用
    score-lstm-mha-relative（version 5 十四键、bias、pos 序列与
    forward_cross_relative）；CANDIDATES 为严格 UTF-8 的 JSON 文件，顶层
    须为非空数组，元素 type 恰为 str（可为空串）且每个码点须在 vocab，
    重复元素按原序保留，否则失败。无 SEED、无随机源，不写文件。每个候选
    均独立重置 h=h0、c=c0、x=START 索引、memory=[h0]、pos=[0]、
    total=0.0，候选间不共享状态；未格式化 total 须与单独调用原入口相同，
    任一计算非有限即失败。成功时 stdout 恰为单个 JSON 对象加 LF，键序
    items,best：items 按 CANDIDATES 原序，每项恰为
    [text,steps,format(total,'.17g')]，steps 为码点数 int；best 为最高
    total 的 int 下标，按未格式化 float 比较，平分取较小下标。序列化恰用
    json.dumps(obj,ensure_ascii=True,separators=(',',':'),
    ...
    python seqmodel.py beam-lstm-mha MODEL START START_T END_T BEAM LENGTH
    WINDOW：以线性退火温度、带投影多头交叉注意力从 version 4 的 LSTM-MHA
    模型做确定性束搜索。除 BEAM 与确定性选束外，MODEL、START、LENGTH、
    WINDOW 的校验、0/1 长度语义、version 4 LSTM/MHA 状态推进、Why/by
    logit、稳定 softmax、线性温度退火与有限性失败契约均沿用
    sample-lstm-mha-anneal；本命令无 SEED、无随机源且不写文件。BEAM 整串
    匹配 [1-9][0-9]*（任意位数合法），每轮仅当其数学值小于候选数时转
    int，否则保留全部候选，超长文本不得触发转换异常。初始束为
    (0.0, "", h0, c0, START 索引, [h0], ())；第 t 轮逐束推进 h、c，按温
    度求 a[k]、m=max(a)、e[k]=exp(a[k]-m)，d 从 0.0 按 k 升序累加；每个
    k 生成子束，分数加 a[k]-m-log(d)，文本追加 vocab[k]、索引元组追加
    k，memory 追加 h 的 float 副本。子束按 (-分数, 索引元组) 升序保留前
    min(BEAM, 候选数) 项，长度归一化因子固定为 1。LENGTH 为 0 时仅输出
    LF，否则输出最终首束文本加 LF。
    ...
    python seqmodel.py train-lstm-mha-relative MODEL CORPUS OUT WINDOW：
    输入校验、前向、一次全语料 SGD、0.1 学习率、5.0 全局裁剪、序列化、
    原子写盘及成败协议均沿用 train-lstm-mha，但模型与位置语义沿用
    perplexity-lstm-mha-relative（version 5 十四键、bias、pos 序列与
    forward_cross_relative），逐步缓存各步相对交叉注意力的独立快照；
    按 t 升序沿用输出层求 du、dWhy、dby；dhs 置零，按 t 降序调用
    backward_cross_relative([du_t])，将 du_t+dqx[0] 加至 dhs[t]，dkvx
    按 M_t 从旧到新映射回完整记忆（h0 项梯度丢弃），dWq、dWk、dWv、
    dWo、dbias 按 t 降序及行列序从 0.0 累加，再调用
    LSTMCell.backward_sequence 取得 dW、db。梯度组序 dW、db、dWq、
    dWk、dWv、dWo、dbias、dWhy、dby（平方和溢出时取 a=max|g|，同序累加
    q+=(g/a)**2，以 (g/a)*(5.0/sqrt(q)) 为裁剪梯度；梯度非有限仍失败），
    heads、h0、c0 不更新；OUT 顶层
    键依次且仅为 version、vocab、W、b、Wq、Wk、Wv、Wo、heads、bias、
    Why、by、h0、c0，version 为 int 5，字节格式与 train-lstm-mha
    相同。成功时 stdout 为空并返回 0。

    python seqmodel.py train-transformer MODEL CORPUS OUT：MODEL、CORPUS
    沿用 perplexity-transformer（version 6 十四键严格 JSON、F、形状、
    heads 整除 V、严格 UTF-8 语料至少 2 码点且无表外字符）；以前 T 个
    字符的 T×V one-hot 与因果 mask（mask[i][j]=(j<=i)）调用装入参数的
    TransformerBlock.forward。t 升序沿用 perplexity-transformer 的
    Why/by 仿射与 softmax 次序求 g=p-onehot(下一字符)，按 t/k/j 序累加
    dWhy、dby，并按 k 升序从 0.0 求 dy=Whyᵀg；全部 t 完成后调用
    block.backward(dy)。梯度组序为 dWq、dWk、dWv、dWo、dW1、db1、dW2、
    db2、dWhy、dby，按组、行、列序从 0.0 累加平方和，范数超过 5.0 时
    scale=5.0/norm，否则 1.0；参数更新为 float(old)-0.1*(grad*scale)，
    非有限即失败。version、heads、P 不变；OUT 十四键顺序、形状、数组转
    float、紧凑 ASCII 转义 JSON 加 LF、负零保留与同目录临时文件原子替换
    均沿用 train-lstm-mha。成功时 stdout 为空并返回 0。

    python seqmodel.py train-transformer-window MODEL CORPUS OUT WINDOW：
    MODEL、CORPUS、OUT 完全沿用 train-transformer（version 6 十四键严格
    JSON、F、形状、heads 整除 V、严格 UTF-8 语料至少 2 码点且无表外字符）；
    WINDOW 整串匹配 [1-9][0-9]*（任意位数均合法，不先转无界整数）。令
    T=语料码点数-1，以十进制位数与同长字典序安全比较：WINDOW 数学值不小
    于 T 时直接沿用 train-transformer 的整序列前向与一次 backward，OUT 与
    其逐字节相同。WINDOW<T 时 t 升序安全求 n=min(WINDOW,t+1)、s=t+1-n，
    以索引 [s:t+1] 构造 n×V one-hot 与 n×n 因果 mask（mask[i][j]=(j<=i)），
    独立调用装参 TransformerBlock.forward 取末行预测下一字符；输出层
    Why/by 仿射、softmax、g=p-onehot(下一字符)、dWhy、dby 与 dy=Whyᵀg
    沿用 train-transformer 的 t/k/j 次序；建 n×V 零 dy 仅末行写该梯度并
    调用 backward，八组块梯度按 t、组、行、列升序从 0.0 累加，每步独立
    截断、不传播或复用窗外状态。最终组序为 dWq、dWk、dWv、dWo、dW1、
    db1、dW2、db2、dWhy、dby，全局 5.0 裁剪、0.1 更新、十四键顺序、数组
    转 float、紧凑 ASCII 转义 JSON 加 LF、负零保留与原子替换均沿用
    train-transformer。成功时 stdout 为空并返回 0。
    ...
    python seqmodel.py mha-relative-bucket-stats MODEL CORPUS WINDOW：
    MODEL、CORPUS、WINDOW 的校验、每步 LSTM 推进、memory/pos 尾窗截取与
    相对 MHA 调用均沿用 perplexity-lstm-mha-relative，取其权重 w 但不计算
    Why、by 输出层。令 R=(len(bias[0])-1)//2、T=len(CORPUS)-1；每步每头
    r 置 2R+1 个 0.0，按 j 升序令 b=max(-R,min(R,P[j]-(t+1)))+R 并累加
    mass[b]+=w[r][0][j]；总量按 t、r、b 升序从 0.0 累加，非有限即失败。
    stdout 为键序 version,radius,steps,items,mean 的紧凑 JSON 加 LF：
    前三值为 int 1、R、T；items 按 t 升序，每项 [t,rows]，rows 按 r
    升序，每项 [r,masses]；mean 按 r 升序，每项 [r,masses]；两处 masses
    均按 b 升序，为单步质量或 总量/T 的 format(x,'.17g') 字符串列表。
    JSON 参数、UTF-8、尾 LF、返回码、stdout/stderr、无回溯、失败原子性
    与不写文件协议沿用 mha-head-stats。
    ...
    try:
        if len(argv) == 4 and argv[1] == "perplexity":
        ...
        elif len(argv) == 6 and argv[1] == "train-transformer-window":
            _train_transformer_window(argv[2], argv[3], argv[4], argv[5])
            ...
            sys.stdout.buffer.write(output.encode("ascii"))
        elif len(argv) == 5 and argv[1] == "perplexity-lstm-mha-relative":
            output = _perplexity_lstm_mha_relative(argv[2], argv[3],
                                                   argv[4])
            sys.stdout.buffer.write(output.encode("ascii"))
        ...
        elif len(argv) == 6 and argv[1] == "train-lstm-mha-relative":
            _train_lstm_mha_relative(argv[2], argv[3], argv[4], argv[5])
            ...
            sys.stdout.buffer.write(output.encode("utf-8"))
              ...
              and argv[1] == "sample-lstm-mha-relative-top-p"):
            output = _sample_lstm_mha_relative_topp(
                argv[2], argv[3], argv[4], argv[5], argv[6], argv[7],
                argv[8], argv[9])
            sys.stdout.buffer.write(output.encode("utf-8"))
        elif (len(argv) == 10
              and argv[1] == "sample-lstm-mha-relative-top-k"):
            output = _sample_lstm_mha_relative_topk(
                argv[2], argv[3], argv[4], argv[5], argv[6], argv[7],
                argv[8], argv[9])
            sys.stdout.buffer.write(output.encode("utf-8"))
            ...
            sys.stdout.buffer.write(output.encode("ascii"))
        elif len(argv) == 7 and argv[1] == "score-lstm-mha-relative-batch":
            output = _score_lstm_mha_relative_batch(
                argv[2], argv[3], argv[4], argv[5], argv[6])
            sys.stdout.buffer.write(output.encode("utf-8"))
            ...
            raise ValueError(
                ...
                "seqmodel.py sample-lstm-mha-top-p MODEL START SEED "
                "START_T END_T TOP_P LENGTH WINDOW | "
