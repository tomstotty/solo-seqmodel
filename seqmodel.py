...
def _sample_lstm_attn_topp(model_path, start, seed_text, start_t_text,
    ...
    """以线性退火温度、top-p（核）抽样、带注意力上下文从 LSTM 模型采样。
...
def _beam_lstm_attn(model_path, start, start_t_text, end_t_text,
    ...
    除 BEAM 及下述确定性选束外，参数校验、LENGTH 的 0/1 语义、线性温度、
    LSTM 状态、注意力记忆、Why/by logit、稳定 softmax 与有限性失败契约均
    沿用 _sample_lstm_attn_anneal；WINDOW 词法与安全截取沿用
    perplexity-lstm-attn。本命令无 SEED、不初始化任何随机源、不写任何
    文件。

    BEAM 整串匹配 [1-9][0-9]*（任意位数均合法）。每轮先以十进制位数及同
    长度字典序与该轮候选数（全部子束总数）的十进制文本比较：仅当其数学值
    小于候选数时才转 int 并保留前 BEAM 项，否则保留全部候选；任意位数文
    本都不触发整数文本位数上限。

    初始束为 (0.0, "", h0, c0, START索引, [h0], ())，七元组依次为累计分
    数、已生成文本、h、c、下一输入索引 x、注意力记忆与已生成索引元组。
    第 t 轮（t 自 0 升序，温度同 sample-lstm-attn-annel 的退火规则）逐束
    按既有顺序推进 h、c，求 u、logit z、缩放 logit a、m=max(a)、
    e_k=exp(a_k-m)，d 自 0.0 依 k 升序累加；随后对每个 k 依 k 升序生成子
    束：分数加 a[k]-m-log(d)，文本追加 vocab[k]，x 置 k，记忆为父束记忆
    追加 h 的 float 副本，索引元组追加 k。全部子束按 (-分数, 生成索引元
    组) 升序排列，保留前 min(BEAM, 候选数) 项；同轮各束等长，使用原始累
    计分数，长度归一化因子固定为 1。任一新增运算（log、减法、分数累加）
    非有限均抛 ValueError。LENGTH 为 0 时仅返回 LF，否则返回最终首束文本
    再加一个 LF；相同输入输出逐字节相同。
    """
    if not _WINDOW_RE.match(window_text):
        raise ValueError("WINDOW must match [1-9][0-9]*")
    ...
    # BEAM：整串匹配 [1-9][0-9]*，任意位数均合法；此处不转 int，仅在某轮
    # 其数学值小于该轮候选数时才转换（此时位数受候选数约束，不会触发整数
    # 文本位数上限）。
    ...
    # 束：(分数, 文本, h, c, x, memory, 生成索引元组)。初始束为
    # (0.0, "", h0, c0, START索引, [h0], ())；h0、c0 不被修改，直接共享。
    beams = [(0.0, "", h0, c0, vocab.index(start), [h0], ())]

    for t in range(length):
        temperature = temperature_at(t)
        children = []
        for score, text, h, c, x, memory, indices in beams:
            # 当前字符的 V 长 one-hot 输入，推进 LSTM 隐状态与细胞状态。
            xvec = [0.0] * V
            xvec[x] = 1.0
            ...
            if not math.isfinite(log_d):
                raise ValueError("log of softmax denominator is non-finite")

            # 子束共享父束推进后的 h、c（forward 不修改其返回值），记忆为父
            # 束记忆追加 h 的 float 副本（既有行共享，仅追加新行）。
            child_memory = memory + [[float(v) for v in nh]]
            for k in range(V):
                # 分数增量按 a[k]-m-log(d) 的运算顺序求值。
                delta = a[k] - m
                if not math.isfinite(delta):
                    raise ValueError("beam score delta became non-finite")
                delta -= log_d
                if not math.isfinite(delta):
                    raise ValueError("beam score delta became non-finite")
                ...
                children.append((new_score, text + vocab[k], nh, nc, k,
                                 child_memory, indices + (k,)))

        # 候选数即本轮子束总数。仅当 BEAM 的数学值小于候选数时才转 int：
        # 位数更少、或同位数且字典序更小即 BEAM<候选数（两者均无前导零）。
        ...
        # 按 (-分数, 生成索引元组) 升序，保留前 min(BEAM, 候选数) 项。
        children.sort(key=lambda item: (-item[0], item[6]))
        beams = children[:keep]

    # LENGTH 为 0 时首束文本为空串，即仅输出 LF。
    return beams[0][1] + "\n"
...
def main(argv):
    ...
    python seqmodel.py beam-lstm-attn MODEL START START_T END_T BEAM
    LENGTH WINDOW：以线性退火温度、带注意力上下文从 version 2 的 LSTM
    模型做确定性束搜索。除 BEAM 及确定性选束外，参数校验、LENGTH 的
    0/1 语义、线性温度、LSTM 状态、注意力记忆、Why/by logit、稳定
    softmax 与有限性失败契约均沿用 sample-lstm-attn-anneal；本命令无
    SEED、无随机源且不写文件。BEAM 整串匹配 [1-9][0-9]*（任意位数合
    法），每轮仅当其数学值小于候选数时转 int，否则保留全部候选。初始
    束为 (0.0, "", h0, c0, START索引, [h0])；第 t 轮逐束推进 h、c 并求
    缩放 logit a、m=max(a)、e_k=exp(a_k-m)，d 自 0.0 依 k 升序累加，
    每个 k 生成子束：分数加 a[k]-m-log(d)，文本追加 vocab[k]，x 置 k，
    memory 追加 h 的 float 副本。全部子束按 (-分数, 生成索引元组) 升
    序，保留前 min(BEAM, 候选数) 项；使用原始累计分数，长度归一化因
    子固定为 1。LENGTH 为 0 时仅输出 LF，否则输出最终首束文本加 LF；
    任一新增运算非有限均失败，相同输入逐字节相同。
