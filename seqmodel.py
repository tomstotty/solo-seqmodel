...
def _load_rank_inputs(models_path, list_path):
    """rank-window-* 三条 CLI 共用的 MODELS/LIST 加载核心。

    MODELS 为严格 UTF-8 的 JSON 数组，至少 2 项，每项为非空相对路径
    str，重复项按原下标各自加载并保序；LIST 为严格 UTF-8 的 JSON 非空
    数组，每项恰为 [corpus, window] 二元组：corpus 为非空相对路径
    str，window 整串匹配 [1-9][0-9]*（任意位数均合法，全程不转为
    int），重复项按序保留。模型路径相对 MODELS 文件所在目录解析，语
    料路径相对 LIST 文件所在目录解析；所有模型 vocab 须逐项同序相
    等，H 可不同。任一读取、UTF-8、JSON 或校验失败均抛 ValueError
    （或 OSError），失败时不写文件。

    返回四元组 (model_paths, entries, loaded, entry_ids)：model_paths
    为 MODELS 原文 str 列表；entries 为 LIST 原文 (corpus, window)
    二元组列表；loaded 按模型原下标依次为 (cell, Why, by, h0, c0)；
    entry_ids 按清单原下标依次为语料码点的 vocab 整数 id 列表。
    """
    with open(models_path, "rb") as f:
        model_paths = json.loads(f.read().decode("utf-8"))
    if not isinstance(model_paths, list) or len(model_paths) < 2:
...
def _total_rank_nll(cell, Why, by, h0, c0, ids, window_text):
    """rank-window-* 三条 CLI 共用的逐项总 NLL 核心。

    每次调用从该模型自身 h0、c0、memory=[h0] 独立重置，单步状态推进、
    注意力、窗口截取、logit 与稳定 log-sum-exp 的 t/j/k 浮点运算次序
    完全沿用 perplexity-lstm-attn，总量从 0.0 按 t 升序累加；任一计算
    非有限即抛 ValueError。不缓存任何跨调用状态。
    """
    V = len(by)
