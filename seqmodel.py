...
def _compare_lstm_attn(model_a_path, model_b_path, corpus_path, window_text):
    """两模型在同一语料上的逐步负对数似然之差（B 减 A），返回待写出的字符串。

    MODEL_A、MODEL_B 各自沿用 perplexity-lstm-attn 的 version 2 八键、F、
    形状及严格 UTF-8 契约；两者 vocab 须逐项同序相等（H 可不同），否则抛
    ValueError。CORPUS 沿用至少 2 码点且无表外字符契约；WINDOW 沿用
    perplexity-lstm-attn 的词法及任意位数安全截取。

    两轨各从自身 h0、c0、memory=[h0] 重置，按 t 升序以既有状态推进、注意
    力、logit 与稳定 log-sum-exp 顺序各求 T=语料码点数-1 个单步负对数似
    然，记 a、b；令 d=b-a，D 从 0.0 按 t 升序累加 d。任一计算非有限（含
    d 或 D）均抛 ValueError。

    stdout 先写 T 个 JSON 项行，每行键序恰为 t,target,a_nll,b_nll,delta，
    值依次为 int t、下一单码点 str、format(a,'.17g')、format(b,'.17g')、
    format(d,'.17g') 字符串；再写唯一汇总行，键序恰为 steps,total_delta，
    值依次为 int T、format(D,'.17g') 字符串。每行恰由
    json.dumps(obj,ensure_ascii=True,separators=(',',':'),
    allow_nan=False)+'\\n' 生成，各行直接拼接，末行保留 LF。全部行先完整
    构造再返回，故失败时不产生任何部分输出，不写文件。
    """
    if not _WINDOW_RE.match(window_text):
        raise ValueError("WINDOW must match [1-9][0-9]*")

    vocab_a, W_a, b_a, Why_a, by_a, h0_a, c0_a = \
        _load_perplexity_lstm_model(model_a_path)
    vocab_b, W_b, b_b, Why_b, by_b, h0_b, c0_b = \
        _load_perplexity_lstm_model(model_b_path)
    if vocab_a != vocab_b:
        raise ValueError("model vocabs must be identical and in the same order")
    vocab = vocab_a
    V = len(vocab)
    ...
    # 两轨独立重置：各自从自身 h0、c0、memory=[h0] 出发。
    memory_a = [h0_a]
    h_a = list(h0_a)
    c_a = list(c0_a)
    memory_b = [h0_b]
    ...
    lines = []
    for t in range(T):
        a, h_a, c_a, memory_a = _step_nll(
            cell_a, Why_a, by_a, h_a, c_a, memory_a, t)
        b, h_b, c_b, memory_b = _step_nll(
...
def main(argv):
    ...
    F、形状及严格 UTF-8 契约，且两者 vocab 须逐项同序相等（H 可不同），
    否则失败；CORPUS、WINDOW 沿用 perplexity-lstm-attn 的契约。两轨各
    从自身 h0、c0、memory=[h0] 重置，按 t 升序以既有状态推进、注意力、
    logit 与稳定 log-sum-exp 顺序各求 T=语料码点数-1 个单步负对数似然
    a、b；令 delta=b-a，total_delta 从 0.0 按 t 升序累加 delta；任一计
    算非有限即失败。stdout 先写 T 个 JSON 项行，键序
    t,target,a_nll,b_nll,delta，值依次为 int t、下一单码点 str、
    format(a,'.17g')、format(b,'.17g')、format(delta,'.17g')；再写唯一
    汇总行，键序 steps,total_delta，值依次为 int T 与
    format(total_delta,'.17g')。每行恰由 json.dumps(obj,
    ensure_ascii=True,separators=(',',':'),allow_nan=False)+'\\n'
    生成的 UTF-8 字节组成，末行保留 LF；失败时不输出任何部分行，
    不写文件。
