...
def _sample_transformer_top_k_top_p(model_path, start, seed_text,
    ...
    """以线性退火温度、top-k 截断后 top-p 核选样从单层 Transformer 块采样。

    除 TOP_P 与下述选样外，MODEL、START、SEED、START_T、END_T、TOP_K、
    LENGTH 的校验，以及 LENGTH 的 0/1 语义、prefix 的 one-hot 输入、因果
    mask、TransformerBlock.forward、Why/by 仿射、线性温度退火、稳定
    softmax、唯一随机源、prefix 状态更新与有限性失败契约均沿用
    _sample_transformer_top_k；不写任何文件。TOP_K 整串匹配
    [1-9][0-9]*，且数学值 K 不超过 V=len(vocab)：先按十进制位数及同长度
    字典序与 V 的十进制文本比较，越界即失败，仅通过后才转 int。TOP_P 经
    float() 解析，结果须有限且 0<TOP_P<=1，否则抛 ValueError。

    每步先按原顺序求 a_k=z_k/T、m=max(a)、e_k=exp(a_k-m)，d 自 0.0 依 k
    升序累加。再将索引按 (-e_k, k) 升序排列（e 降序、并列时 k 升序）并取
    前 K 项；sK 自 0.0 按该序累加 e，令 target=TOP_P*sK，再从 0.0 按该序
    累加 e，保留首个使累计值 >=target 的最短前缀，s 为此前缀累计和。令
    u=r.random()*s，再按前缀序自 0.0 累加 e，选首个累计值严格大于 u 的索
    引；无则取前缀末项。选中字符追加到输出，其索引追加到 prefix。任一新增
...
def main(argv):
    ...
    START_T END_T TOP_K TOP_P LENGTH：除 TOP_P 与下述选样外，全部校验、
    LENGTH 的 0/1 语义、Transformer 前向、线性温度退火、唯一随机源、
    prefix 状态更新与有限性错误协议均沿用 sample-transformer-top-k；
    TOP_K 仍整串匹配 [1-9][0-9]* 且数学值不超过 V，先按十进制位数及同
    长度字典序与 V 比较，通过后才转 int。TOP_P 经 float() 解析，须有限
    且 0<TOP_P<=1。每步沿原次序求 a_k、m=max(a)、e_k=exp(a_k-m)；按
    (-e_k,k) 升序取前 K 项，sK 自 0.0 依序累加，令 target=TOP_P*sK，
    再从 0.0 依序累加 e，保留首个使累计值 >=target 的最短前缀，s 为其
    累计和；令 u=random()*s，按前缀序自 0.0 累加 e，选首个累计值严格
    大于 u 的索引，无则取末项；以该字符更新输出和 prefix。任一新增运
    算非有限即失败。输出契约与 sample 相同，不写文件。
    ...
    try:
        if len(argv) == 4 and argv[1] == "perplexity":
            ...
            output = _sample_transformer_top_k(
                ...
                argv[8])
            sys.stdout.buffer.write(output.encode("utf-8"))
        elif len(argv) == 10 and argv[1] == "sample-transformer-top-k-top-p":
