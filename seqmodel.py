...
def _sample_transformer_window_top_p(model_path, start, seed_text,
    ...
    """有限上下文加线性退火、top-p 核选样的 Transformer 采样。

    除 TOP_P 与核选样外，MODEL、START、SEED、START_T、END_T、LENGTH 的词法
    与校验、模型十四键装载、装参 TransformerBlock、唯一随机源、线性温度退火
    （含 LENGTH 的 0/1 语义与 START_T+(END_T-START_T)*t/(LENGTH-1) 的严格
    求值次序）、窗口截断前向、Why/by 仿射、prefix 状态更新与有限性失败契约
    均逐项沿用 _sample_transformer_window_anneal；不写任何文件。

    WINDOW 整串匹配 [1-9][0-9]*（任意位数均合法，不先转无界整数），否则抛
    ValueError。TOP_P 经 float() 解析，结果须有限且 0<TOP_P<=1，否则抛
    ValueError。

    置 prefix=[START 索引]。每步以十进制位数和同长字典序安全求
    n=min(WINDOW,len(prefix))，仅以末尾 n 项构造 n×V one-hot 与 n×n 因果
    mask（mask[i][j]=(j<=i)），调用装参 TransformerBlock.forward 并仅取末
    行；每步重新截断、独立前向，不复用任何窗外状态。随后沿原次序求
    a_k=z_k/T、m=max(a)、e_k=exp(a_k-m)，d 自 0.0 依 k 升序累加。索引按
    (-e_k, k) 升序，自 0.0 依序累加 e，保留首个使累计值 >=TOP_P*d 的最短
    前缀，s 为其累计和。令 u=r.random()*s，按前缀序自 0.0 累加 e，选首个
    累计值严格大于 u 的索引，无则取前缀末项；选中字符追加到输出、其索引追
    加到完整 prefix。任一中间量非有限即失败。

    LENGTH=0 时不求温度、不执行任何前向，仅返回 LF。当 WINDOW 每步都不小
    于 prefix 长度时，输出与同参 sample-transformer-top-p 逐字节相同。成功
...
def main(argv):
    ...
    python seqmodel.py sample-transformer-window-top-p MODEL START SEED
    START_T END_T TOP_P LENGTH WINDOW：除 TOP_P 与核选样外，全部校验、
    LENGTH 的 0/1 语义、线性温度退火、窗口截断前向、Why/by 仿射、唯一
    随机源、prefix 状态更新与有限性失败协议均沿用
    sample-transformer-window-anneal。WINDOW 整串匹配 [1-9][0-9]*（任意
    位数均合法，安全截取）。TOP_P 经 float() 解析，须有限且 0<TOP_P<=1。
    每步以 prefix 尾部 min(WINDOW,len(prefix)) 项构造 one-hot 与因果
    mask，沿原次序求 a_k、m=max(a)、e_k=exp(a_k-m)，d 自 0.0 按 k 升序
    累加；索引按 (-e_k,k) 升序，保留累加 e 首次达到 TOP_P*d 的最短前
    缀，s 为其和。令 u=random()*s，按前缀序累加 e，选首个累计值严格大
    于 u 的索引，无则取末项，更新 prefix；任一新增运算非有限即失败。
    LENGTH=0 不求温度或前向，仅输出 LF；窗口覆盖 prefix 时与同参
    sample-transformer-top-p 逐字节相同。输出契约与 sample 相同，不写文
    件。
