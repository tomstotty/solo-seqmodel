...
def _sample_lstm_mha_relative_topp(model_path, start, seed_text,
                                   ...
                                   length_text, window_text):
    """以线性退火温度与 top-p（核）抽样从 version 5 相对位置模型采样。

    除 TOP_P 与选样外，MODEL、START、SEED、LENGTH、WINDOW 的校验、
    LENGTH 的 0/1 语义、version 5 状态推进、相对位置 MHA 窗口、线性温度
    退火、唯一随机源与有限性失败契约均完全沿用
    sample-lstm-mha-relative-anneal；模型装载不经随机初始化，整次调用仅
    初始化一次 r=random.Random(int(SEED))，不写任何文件。TOP_P 经
    ...
    到输出并作为下一输入 x，随后向 memory 追加 h 的 float 副本、向 pos
    追加 t+1。任一新增运算非有限均抛 ValueError。成功返回 LENGTH 个码点
    再加一个 LF。
    """
    if not _WINDOW_RE.match(window_text):
        raise ValueError("WINDOW must match [1-9][0-9]*")
...
def main(argv):
    ...
    带相对位置偏置多头交叉注意力从 version 5 的模型采样。除 TOP_P 与选样
    外，全部输入校验、LENGTH 的 0/1 语义、version 5 状态推进、相对位置
    MHA 窗口、退火、唯一随机源、有限性与成败协议均沿用
    sample-lstm-mha-relative-anneal；TOP_P 经 float() 解析，须有限且
    0<TOP_P<=1，否则失败。每步沿原顺序求 a[k]、m=max(a)、
    e[k]=exp(a[k]-m)，d 自 0.0 按 k 升序累加；索引按 (-e[k],k) 升序，依
    序累加 e，保留首个使累计值 >=TOP_P*d 的最短前缀，s 为其和；令
    u=random()*s，按前缀序自 0.0 累加 e，选首个累计值严格大于 u 的索引，
    无则取末项，以该字符更新 x，再向 memory、pos 追加 h 的 float 副本与
    t+1。任一新增运算非有限即失败。输出契约与 sample 相同，不写文件。
